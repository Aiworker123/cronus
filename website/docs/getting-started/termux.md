...
description: "Run Cronus Agent directly on an Android phone with Termux"
---

# Cronus on Android with Termux

This is the tested path for running Cronus Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Cronus from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Cronus now ships a Termux-aware installer path:
...
- links `cronus` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `cronus` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/cronus" "$PREFIX/bin/cronus"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `cronus` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.cronus/.env`.
...
### `cronus doctor` says ripgrep or Node is missing
