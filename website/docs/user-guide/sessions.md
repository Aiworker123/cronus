...
Cronus Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.cronus/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Cronus stores session history so it can resume conversations, but it does not
...
Cronus explicitly injects for that turn.
...
For example, if a user sends an image and asks Cronus to make a meme from it,
Cronus may inspect that image once with vision and run an image-processing
...
`cronus sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`cronus` or `cronus chat`) |
...
cronus -c "my project"   # → resumes "my project #3"
```
...
cronus --resume 20250305_091523_a1b2c3d4
cronus -r 20250305_091523_a1b2c3d4

# Resume by title
cronus --resume "refactoring auth"

# Or with the chat subcommand
cronus chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `cronus sessions list`.

### Conversation Recap on Resume

When you resume a session, Cronus displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Cronus session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.cronus/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `cronus -r "<title>"` from the shell) and pick up where the platform left off.
...
Cronus automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `cronus sessions list` or `cronus sessions browse`.
...
```bash
cronus sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Cronus creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`cronus -c "my project"`), it automatically picks the most recent session in the lineage.
...
Cronus provides a full set of session management commands via `cronus sessions`:
...
# Filter by platform
cronus sessions list --source telegram

# Show more sessions
cronus sessions list --limit 50
```
...
cronus sessions export backup.jsonl

# Export sessions from a specific platform
cronus sessions export telegram-history.jsonl --source telegram

# Export a single session
cronus sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
cronus sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
cronus sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
cronus sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
cronus sessions prune

# Custom age threshold
cronus sessions prune --older-than 30

# Only prune sessions from a specific platform
cronus sessions prune --source telegram --older-than 60

# Skip confirmation
cronus sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`cronus insights`](/reference/cli-commands#cronus-insights).
...
When Cronus cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Cronus uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Cronus in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.cronus/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.cronus/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.cronus/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.cronus/sessions/`. They are no longer written or
read by Cronus. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Cronus process in the same `CRONUS_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.cronus/config.yaml`:
...
cronus sessions delete <session_id>
