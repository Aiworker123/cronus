...
Cronus Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Cronus checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.cronus/config.yaml`:
...
| `timeout` | `60` | Seconds Cronus waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `cronus --yolo` or `cronus chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `CRONUS_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `CRONUS_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Cronus shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Cronus refuses to run them **regardless** of:
...
Configure the timeout in `~/.cronus/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.cronus/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.cronus/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` cronus/gateway | Self-termination prevention |
...
The `CRONUS_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.cronus/config.yaml`:
...
:::tip
Use `cronus config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Cronus controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.cronus/.env`:
...
For more flexible authorization, Cronus includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `cronus pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.cronus/config.yaml`:
...
cronus pairing approve telegram ABC12DEF

# Revoke a user's access
cronus pairing revoke telegram 123456789

# Clear all pending codes
cronus pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.cronus/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Cronus applies strict security hardening to every container.
...
Container resources are configurable in `~/.cronus/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.cronus/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `CRONUS_HOME`. Skills declare these in frontmatter:
...
When loaded, Cronus checks if these files exist in the active profile's `CRONUS_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Cronus infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Cronus infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Cronus integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Cronus under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.cronus/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.cronus/logs/` for unauthorized access attempts
10. **Keep updated** — run `cronus update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.cronus/.env
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.cronus/.env`:
...
TERMINAL_SSH_USER=cronus
TERMINAL_SSH_KEY=~/.ssh/cronus_agent_key
```
...
Cronus ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `cronus_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `cronus doctor` for the full remediation.
- **`cronus doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Cronus installs these **lazily** on first use rather than eagerly under `cronus-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `cronus tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `cronus tools`.
