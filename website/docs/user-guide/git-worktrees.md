...
title: "Git Worktrees"
description: "Run multiple Cronus agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Cronus Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Cronus so each session has a clean, isolated working directory.

## Why Use Worktrees with Cronus?

Cronus treats the **current working directory** as the project root:

- CLI: the directory where you run `cronus` or `cronus chat`
...
git worktree add ../repo-feature feature/cronus-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/cronus-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Cronus there:

```bash
cd ../repo-feature

# Start Cronus in the worktree
cronus
```
...
git worktree add ../repo-experiment-a feature/cronus-a
git worktree add ../repo-experiment-b feature/cronus-b
```
...
- Works on its own branch (`feature/cronus-a` vs `feature/cronus-b`).
...
- Cronus checkpoint data under `~/.cronus/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Cronus experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/cronus-checkpoints-docs`, `feature/cronus-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Cronus from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `cronus -w` (Automatic Worktree Mode)

Cronus has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `cronus/cronus-<hash>`).
...
```bash
cronus -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `cronus -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Cronus session its own clean checkout.
