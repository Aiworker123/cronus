...
Type `@` followed by a reference to inject content directly into your message. Cronus expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Cronus env: `$CRONUS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CRONUS_HOME/skills/.hub/`
