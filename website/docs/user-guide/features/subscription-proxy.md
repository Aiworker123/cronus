...
chat completions — use your Cronus-managed provider subscription as their
...
| Use case | "Use Cronus as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Cronus stores
the refresh token in `~/.cronus/auth.json` — the same place all Cronus
...
Model:      Cronus-4-70B    # or Cronus-4.3-36B, Cronus-4-405B
```
...
interface in `cronus_cli/proxy/adapters/`.
...
If you see `not logged in`, run `cronus auth add nous`. If you see
...
`cronus_cli/proxy/adapters/<provider>.py` and registering it in
