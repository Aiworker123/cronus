...
description: "Durable SQLite-backed task board for coordinating multiple Cronus profiles"
---
...
Cronus Kanban is a durable task board, shared across all your Cronus profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.cronus/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.cronus/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `cronus kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `cronus kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/cronus-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `CRONUS_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
cronus kanban boards list

# Create a new board.
cronus kanban boards create atm10-server \
    --name "ATM10 Server" \
...
cronus kanban --board atm10-server list
cronus kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
cronus kanban boards switch atm10-server
cronus kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
cronus kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `CRONUS_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.cronus/kanban/current` — the slug persisted by `cronus kanban
   boards switch`.
4. `default`.
...
`cronus dashboard` → Kanban tab shows a board switcher at the top as soon
...
cronus gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
cronus kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `cronus kanban show t_abcd`.
...
Override the config flag at runtime via `CRONUS_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `cronus gateway
...
until one comes up — `cronus kanban create` warns about this at creation
time.

Running `cronus kanban daemon` as a separate process is **deprecated**;
...
cronus kanban create "nightly ops review" \
    --assignee ops \
...
```bash
cronus kanban complete t_abc t_def t_hij --result "batch wrap"
cronus kanban archive  t_abc t_def t_hij
cronus kanban unblock  t_abc t_def
cronus kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `cronus kanban`.** When the dispatcher spawns a worker it sets `CRONUS_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `cronus kanban` CLI.
...
kanban_show()                                     # no args — uses CRONUS_KANBAN_TASK
...
### Why tools instead of shelling to `cronus kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `cronus kanban complete` *inside* the container, where `cronus` isn't installed and `~/.cronus/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.cronus/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `cronus chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `CRONUS_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/cronus_cli/test_kanban_db.py -q"],
...
2. `cd $CRONUS_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
cronus -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
cronus -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
cronus kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

cronus kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
cronus -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
cronus -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `cronus kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `cronus kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `cronus profile create --description "..."`, `cronus profile describe <name> --text "..."`, `cronus profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).

Config knobs (all under `kanban:` in `~/.cronus/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `cronus profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.cronus/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Cronus dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
cronus kanban init                                     # create kanban.db + print daemon hint
cronus kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
cronus kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
cronus kanban show <id> [--json]
cronus kanban assign <id> <profile>                    # or 'none' to unassign
cronus kanban link <parent_id> <child_id>
cronus kanban unlink <parent_id> <child_id>
cronus kanban claim <id> [--ttl SECONDS]
cronus kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
cronus kanban complete <id>... [--result "..."]
cronus kanban block <id> "<reason>" [--ids <id>...]
cronus kanban unblock <id>...
cronus kanban archive <id>...

cronus kanban tail <id>                                # follow a single task's event stream
cronus kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
cronus kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
cronus kanban runs <id> [--json]                       # attempt history (one row per run)
cronus kanban assignees [--json]                       # profiles on disk + per-assignee task counts
cronus kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
cronus kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `cronus gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
cronus kanban stats [--json]                           # per-status + per-assignee counts
cronus kanban log <id> [--tail BYTES]                  # worker log from ~/.cronus/kanban/logs/
cronus kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
cronus kanban notify-list [<id>] [--json]
cronus kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
cronus kanban context <id>                             # what a worker sees
cronus kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
cronus kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
cronus kanban create "nightly backup audit" \
...
Every `cronus kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `cronus chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `cronus_cli.kanban.run_slash()` entry point that reuses the `cronus kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `cronus kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.cronus/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`cronus kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `cronus kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/cronus-kanban-v1-spec.pdf`.
...
```bash
cronus kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$CRONUS_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
cronus kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
cronus kanban notify-list
cronus kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
cronus kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
cronus kanban runs t_abcd
...
**Bulk close caveat.** `cronus kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`cronus kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `cronus kanban heartbeat $TASK` to signal liveness during long operations. |
...
`cronus kanban tail <id>` shows these for a single task. `cronus kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.cronus/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/cronus-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
