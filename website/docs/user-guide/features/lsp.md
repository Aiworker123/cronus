...
Cronus runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Cronus
...
1. Cronus captures a baseline of current diagnostics for the file.
...
…). Cronus auto-detects the binary on PATH or in
`<CRONUS_HOME>/lsp/bin/`.
...
`node_modules` tree — Cronus installs both packages together when you
run `cronus lsp install typescript` or auto-install fires on first
use.

## CLI

```
cronus lsp status          # service state + per-server install status
cronus lsp list            # registry, optionally --installed-only
cronus lsp install <id>    # eagerly install one server
cronus lsp install-all     # try every server with a known recipe
cronus lsp restart         # tear down running clients
cronus lsp which <id>      # print resolved binary path
```

`cronus lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CRONUS_HOME>/lsp/bin
...
When `install_strategy: auto`, Cronus installs binaries into
`<CRONUS_HOME>/lsp/bin/`. NPM packages land in
`<CRONUS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Cronus-owned and is
...
Servers are kept alive for the life of the Cronus process. There's
...
**`cronus lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CRONUS_HOME>/lsp/bin/`. Run
`cronus lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `cronus lsp status`**
...
When `cronus lsp status` shows a `Backend warnings` section, install
...
`~/.cronus/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.cronus/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `cronus lsp restart` to clear the set;
