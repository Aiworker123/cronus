...
Schedule tasks to run automatically with natural language or cron expressions. Cronus exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Cronus itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
Cron jobs use whatever provider `cronus model` selected. `cronus setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Cronus disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
cronus cron create "every 2h" "Check server status"
cronus cron create "every 1h" "Summarize new feed items" --skill blogwatcher
cronus cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Cronus will use the unified `cronjob` tool internally.
...
cronus cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever Cronus profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `CRONUS_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
cronus cron create "every 1d at 03:00" \
...
Use `--profile default` to explicitly pin to the root Cronus profile. The named profile must already exist; the scheduler refuses to create profiles on the fly. To clear a profile pin during `cron edit`, pass an empty string (`--profile ""` or `profile=""`) — the job reverts to running in whatever profile the scheduler itself is in.
...
:::note Serialization
Jobs with a `profile` set also run sequentially, for the same reason as `workdir`-pinned jobs: switching `CRONUS_HOME` is a process-global mutation, so two profile-pinned jobs running in parallel would race each other. Unpinned jobs still run in the normal parallel pool.
:::
...
```bash
cronus cron edit <job_id> --schedule "every 4h"
cronus cron edit <job_id> --prompt "Use the revised task"
cronus cron edit <job_id> --skill blogwatcher --skill maps
cronus cron edit <job_id> --add-skill maps
cronus cron edit <job_id> --remove-skill blogwatcher
cronus cron edit <job_id> --clear-skills
```
...
```bash
cronus cron list
cronus cron pause <job_id_or_name>
cronus cron resume <job_id_or_name>
cronus cron run <job_id_or_name>
cronus cron remove <job_id_or_name>
cronus cron edit <job_id_or_name> [...flags]
cronus cron status
cronus cron tick
```
...
```bash
cronus gateway install     # Install as a user service
sudo cronus gateway install --system   # Linux: boot-time system service for servers
cronus gateway             # Or run in foreground
...
1. loads jobs from `~/.cronus/cron/jobs.json`
...
A file lock at `~/.cronus/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.cronus/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.cronus/cron/output/`), but no message is sent to the delivery target.
...
Or set the `CRONUS_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.cronus/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Cronus directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Cronus will write the check script to `~/.cronus/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.cronus/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.cronus/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.cronus/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.cronus/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Cronus reads Job 1's most recent output from `~/.cronus/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Cronus skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `cronus tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `cronus tools` cron-platform config wins; otherwise Cronus falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Cronus should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.cronus/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.cronus/scripts/.feed-changed.last"
...
:::tip
Cronus's own `~/.cronus/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/cronus-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.cronus/cron/jobs.json`. Output from job runs is saved to `~/.cronus/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Cronus resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
