...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Cronus's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Cronus instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
cronus memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$CRONUS_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `cronus` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Cronus syncs them back at session init.
...
The `cronus honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `cronus memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
cronus honcho status          # Connection status, config, and key settings
cronus honcho setup           # Redirects to `cronus memory setup`
cronus honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
cronus honcho peer            # Show or update peer names + dialectic reasoning level
cronus honcho mode            # Show or set recall mode (hybrid/context/tools)
cronus honcho tokens          # Show or set token budget for context and dialectic
cronus honcho identity        # Seed or show the AI peer's Honcho identity
cronus honcho sync            # Sync Honcho config to all existing profiles
cronus honcho peers           # Show peer identities across all profiles
cronus honcho sessions        # List known Honcho session mappings
cronus honcho map             # Map current directory to a Honcho session name
cronus honcho enable          # Enable Honcho for the active profile
cronus honcho disable         # Disable Honcho for the active profile
cronus honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `cronus honcho`

If you previously used the standalone `cronus honcho setup`:
...
No re-login or re-setup needed. Run `cronus memory setup` and select "honcho" — the wizard detects your existing config.
