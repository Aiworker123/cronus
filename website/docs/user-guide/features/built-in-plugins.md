...
description: "Plugins shipped with Cronus Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Cronus ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.cronus/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Cronus Plugin](/guides/build-a-cronus-plugin) to write your own.
...
2. **User** — `~/.cronus/plugins/<name>/`
3. **Project** — `./.cronus/plugins/<name>/` (requires `CRONUS_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `cronus_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `cronus memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `cronus plugins list` and the interactive `cronus plugins` UI), but none load until you explicitly enable them:

```bash
cronus plugins enable disk-cleanup
```

Or via `~/.cronus/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Cronus. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
cronus plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `cronus plugins enable <name>`.
...
| `cronus-achievements` | dashboard tab | Steam-style collectible badges generated from your real Cronus session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `cronus memory` and `cronus plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CRONUS_HOME` or `/tmp/cronus-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under CRONUS_HOME | always | Never |
...
**State** — everything lives at `$CRONUS_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `CRONUS_HOME` or `/tmp/cronus-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `cronus plugins enable disk-cleanup` (or check the box in `cronus plugins`).

**Disabling again:** `cronus plugins disable disk-cleanup`.
...
**Enabling:** `cronus plugins enable security-guidance` (or check the box in `cronus plugins`).

**Disabling again:** `cronus plugins disable security-guidance`.
...
Traces Cronus turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Cronus' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `cronus logs`.
...
Or check the box in the interactive `cronus plugins` UI. Then put the credentials in `~/.cronus/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Cronus turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `CRONUS_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Cronus session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `cronus chat` session lives under one Langfuse session.

**Verify:**

```bash
cronus plugins list                 # observability/langfuse should show "enabled"
cronus chat -q "hello"              # check the Langfuse UI for a "Cronus turn" trace
```
...
| `CRONUS_LANGFUSE_RELEASE` | — | Release/version tag |
| `CRONUS_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `CRONUS_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `CRONUS_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Cronus-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Cronus-prefixed wins when both are set.
...
**Disabling:** `cronus plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.cronus/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
cronus plugins enable google_meet
...
**Disabling:** `cronus plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.cronus/cache/google_meet/` until you remove them.

### cronus-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Cronus session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Cronus feature changes.

**How it works:**

- Scans your entire `~/.cronus/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$CRONUS_HOME/plugins/cronus-achievements/state.json`
...
| Secret | Hidden until Cronus detects the first related signal in your history |

**API** — routes mount under `/api/plugins/cronus-achievements/`:
...
**State files** — live under `$CRONUS_HOME/plugins/cronus-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Cronus updates. |
...
**Enabling:** Nothing to enable — `cronus-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `cronus dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/cronus-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.cronus/plugins/cronus-achievements/` that ships no dashboard. The plugin's state files under `$CRONUS_HOME/plugins/cronus-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Cronus plugin — see [Build a Cronus Plugin](/guides/build-a-cronus-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.cronus/plugins/<name>/`
- Manifest source is reported as `bundled` in `cronus plugins list`
