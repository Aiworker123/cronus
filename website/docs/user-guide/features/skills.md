...
All skills live in **`~/.cronus/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Cronus at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Cronus to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.cronus/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
cronus chat --toolsets skills -q "What skills do you have?"
cronus chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.cronus/cache/chart-q4-2025.png
...
When a missing value is encountered, Cronus asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `cronus setup` or `~/.cronus/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `cronus config migrate` prompts for unconfigured settings, and `cronus config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.cronus/skills/                  # Single source of truth
...
If you maintain skills outside of Cronus — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Cronus to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.cronus/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.cronus/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Cronus process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Cronus ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.cronus/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.cronus/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `cronus bundles list`.
...
cronus bundles create research

# Overwrite an existing bundle
cronus bundles create backend-dev --skill ... --force

# Delete a bundle
cronus bundles delete backend-dev

# Re-scan ~/.cronus/skill-bundles/ and report changes
cronus bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.cronus/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.cronus/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
cronus skills browse                              # Browse all hub skills (official first)
cronus skills browse --source official            # Browse only official optional skills
cronus skills search kubernetes                   # Search all sources
cronus skills search react --source skills-sh     # Search the skills.sh directory
cronus skills search https://mintlify.com/docs --source well-known
cronus skills inspect openai/skills/k8s           # Preview before installing
cronus skills install openai/skills/k8s           # Install with security scan
cronus skills install official/security/1password
cronus skills install skills-sh/vercel-labs/json-render/json-render-react --force
cronus skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
cronus skills list --source hub                   # List hub-installed skills
cronus skills check                               # Check installed hub skills for upstream updates
cronus skills update                              # Reinstall hub skills with upstream changes when needed
cronus skills audit                               # Re-scan all hub skills for security
cronus skills uninstall k8s                       # Remove a hub skill
cronus skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
cronus skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
cronus skills publish skills/my-skill --to github --repo owner/repo
cronus skills snapshot export setup.json          # Export skill config
cronus skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Cronus. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `cronus skills search <query> --source skills-sh`. Cronus resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Cronus currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Cronus repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Cronus can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
cronus skills search https://mintlify.com/docs --source well-known
cronus skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
cronus skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Cronus can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
cronus skills install openai/skills/k8s
cronus skills tap add myorg/skills-repo
```
...
- Cronus source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Cronus supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Cronus source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Cronus can search and convert agent entries from LobeHub's public catalog into installable Cronus skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Cronus source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Cronus integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Cronus' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Cronus source id: `browse-sh`
- Trust level: `community`

```bash
cronus skills search airbnb --source browse-sh
cronus skills inspect browse-sh/airbnb.com/search-listings-ddgioa
cronus skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Cronus fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Cronus source id: `url`
...
```bash
cronus skills install https://sharethis.chat/SKILL.md
cronus skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
cronus skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `cronus skills update` re-fetches from the same URL automatically when you want to refresh.
...
`cronus skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Cronus | Always trusted |
...
```bash
cronus skills check          # Report which installed hub skills changed upstream
cronus skills update         # Reinstall only the skills with updates available
cronus skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Cronus users add with `cronus skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.cronus.tags`, `version`, `author`, `platforms`, `metadata.cronus.config`).
...
Cronus discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/cronus-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Cronus user can subscribe and install:

```bash
cronus skills tap add my-org/cronus-skills
cronus skills search deploy
cronus skills install my-org/cronus-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.cronus/.hub/taps.json`:
...
The `cronus skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `cronus skills tap list` shows the effective path per tap.
...
```bash
cronus skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.cronus/.hub/taps.json` (created on demand).

## Bundled skill updates (`cronus skills reset`)

Cronus ships with a set of bundled skills in `skills/` inside the repo. On install and on every `cronus update`, a sync pass copies those into `~/.cronus/skills/` and records a manifest at `~/.cronus/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Cronus recomputes the hash of your local copy and compares it to the origin hash:
...
cronus skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `CRONUS_HOME`, so `cronus -p coder skills reset <name>` only affects that profile.
:::
