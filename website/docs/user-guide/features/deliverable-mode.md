...
When Cronus Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.cronus/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.cronus/config.yaml` (switchable per session
via `/personality`).
...
If you use Cronus' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.cronus/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Cronus Agent's deliverable
