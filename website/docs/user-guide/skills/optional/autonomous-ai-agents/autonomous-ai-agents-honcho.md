...
description: "Configure and use Honcho memory with Cronus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Cronus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cronus skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Cronus

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Cronus profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/cronus#running-honcho-locally-with-cronus

### Verify

```bash
cronus honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Cronus profile's AI peer
...
Honcho models conversations as interactions between **peers**. Cronus creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Cronus instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Cronus run |
| `global` | Single session across all directories |

Manual override: `cronus honcho map my-project-name`
...
Each Cronus profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block cronus.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `cronus.coder` host block in `honcho.json`
...
```bash
cronus honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Cronus when Honcho memory is active.
...
Config file: `$CRONUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `cronus honcho setup`. Ensure `memory.provider: honcho` is in `~/.cronus/config.yaml`.

### Memory not persisting across sessions
Check `cronus honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `cronus profile create <name> --clone`. For existing profiles: `cronus honcho sync`.
...
| Command | Description |
|---------|-------------|
| `cronus honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `cronus honcho status` | Show resolved config, connection test, peer info for active profile |
| `cronus honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `cronus honcho disable` | Disable Honcho for the active profile |
| `cronus honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `cronus honcho peers` | Show peer identities across all profiles |
| `cronus honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `cronus honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `cronus honcho sessions` | List known directory-to-session-name mappings |
| `cronus honcho map <name>` | Map current working directory to a Honcho session name |
| `cronus honcho identity` | Seed AI peer identity or show both peer representations |
| `cronus honcho sync` | Create host blocks for all Cronus profiles that don't have one yet |
| `cronus honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Cronus + Honcho |
| `cronus memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `cronus memory status` | Show active memory provider and config |
| `cronus memory off` | Disable external memory provider |
