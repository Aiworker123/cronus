...
| | |
|---|---|
| Source | Optional — install with `cronus skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Cronus Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Cronus terminal calls (desktop app flow only)
...
### Service Account (recommended for Cronus)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.cronus/.env` (the skill will prompt for this on first load).
...
## Cronus Execution Pattern (desktop app flow)

Cronus terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/cronus-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/cronus-op.sock"
