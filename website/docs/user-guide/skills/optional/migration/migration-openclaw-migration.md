---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Cronus Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Cronus Agent"
---
...
Migrate a user's OpenClaw customization footprint into Cronus Agent. Imports Cronus-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cronus skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Cronus Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Cronus`, `Memory`, `Persona`, `Import` |
| Related skills | [`cronus-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-cronus-agent) |
...
# OpenClaw -> Cronus Migration

Use this skill when a user wants to move their OpenClaw setup into Cronus Agent with minimal manual cleanup.
...
```bash
cronus claw migrate              # Full interactive migration
...
cronus claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `cronus setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_cronus.py` to:

- import `SOUL.md` into the Cronus home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Cronus memory entries
- merge OpenClaw command approval patterns into Cronus `command_allowlist`
- migrate Cronus-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.cronus/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Cronus workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.cronus/tts/`
- archive non-secret docs that do not have a direct Cronus destination
...
- `scripts/openclaw_to_cronus.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py`

Do not guess a shorter path like `~/.cronus/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.cronus/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Cronus-compatible secrets, currently:
...
Cronus CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Cronus. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Cronus workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Cronus skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.cronus/skills/migration/openclaw-migration/scripts/openclaw_to_cronus.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Cronus unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Cronus targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Cronus destination. Unsupported auth blobs must still be reported as skipped.
...
- Cronus persona state imported
- Cronus memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.cronus/skills/openclaw-imports/`
