...
| | |
|---|---|
| Source | Optional — install with `cronus skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Cronus Agent |
| License | MIT |
...
import upstream classes into Cronus itself.
...
```bash
mkdir -p ~/.cronus/cache/darwinian-evolver && cd ~/.cronus/cache/darwinian-evolver
...
```bash
cd ~/.cronus/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.cronus/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.cronus/skills/research/darwinian-evolver
DE_DIR=~/.cronus/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Cronus core.
   Custom driver scripts under `~/.cronus/skills/...` are user-side and fine.
