...
| | |
|---|---|
| Source | Optional — install with `cronus skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CRONUS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CRONUS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CRONUS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client cronus
```
...
```bash
PUBLISH="${CRONUS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client cronus
```
...
```bash
DRIVE="${CRONUS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `cronus`)    |
