...
| | |
|---|---|
| Source | Optional — install with `cronus skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Cronus Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.cronus/.env`:
...
```bash
CANVAS="python $CRONUS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
