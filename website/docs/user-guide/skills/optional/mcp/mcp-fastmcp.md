...
| | |
|---|---|
| Source | Optional — install with `cronus skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Cronus or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Cronus. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cronus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cronus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Cronus integration, either:

- configure the server in `~/.cronus/config.yaml` using the `native-mcp` skill, or
...
### Cronus cannot see the deployed server

The server-building part may be correct while the Cronus config is not. Load the `native-mcp` skill and configure the server in `~/.cronus/config.yaml`, then restart Cronus.
