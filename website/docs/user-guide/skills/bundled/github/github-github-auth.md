...
- Give it a name like "cronus-agent"
- Select scopes:
...
- Give it a title like "cronus-agent-&lt;machine-name>"
...
elif [ -f ~/.cronus/.env ] && grep -q "^GITHUB_TOKEN=" ~/.cronus/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.cronus/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
