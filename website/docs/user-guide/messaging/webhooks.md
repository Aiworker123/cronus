...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Cronus Agent — Webhooks Tutorial"
...
1. Enable via `cronus gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `cronus webhook subscribe`
...
Add the `github-pr` route to your `~/.cronus/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Cronus processes the event, and posts a review comment on the PR.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `cronus webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
cronus webhook remove github-issues
```

### Test a subscription

```bash
cronus webhook test github-issues
cronus webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.cronus/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `cronus webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `cronus gateway run`
