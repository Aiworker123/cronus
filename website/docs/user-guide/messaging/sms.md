...
title: "SMS (Twilio)"
description: "Set up Cronus Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Cronus connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `pip install 'cronus-agent[sms]'`
...
If you're running Cronus locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Cronus will respond via SMS.
...
Cronus validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Cronus gateway logs for Twilio API errors
