...
Connect Cronus to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Cronus supports two WeCom integration modes:
...
> Run `cronus gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Cronus decrypts the message, queues it for the agent
...
(Use `cronus gateway start` only after `cronus gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Cronus and the token the
...
in `~/.cronus/.env` values around `=` will also break signature checks. After
fixing, restart `cronus gateway run`.
...
Check `cronus gateway run` logs for the bound host/port. If the adapter bound to
