...
sidebar_position: 12
title: "Google Chat"
description: "Set up Cronus Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Cronus Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Cronus
...
> Run `cronus gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Cronus can read (e.g.,
`~/.cronus/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a cronus restart)
...
- Principal: `cronus-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Cronus calls
...
- **App name**: whatever you want users to see ("Cronus" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/cronus-chat-events`.
...
`ADDED_TO_SPACE` event that Cronus uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Cronus

Add the Google Chat section to `~/.cronus/.env`:
...
Install the dependencies the Google Chat adapter needs (no Cronus extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Cronus is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Cronus detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Cronus session.
...
3. Download the JSON. Move it onto the host that runs Cronus.
4. On the host, register the client with Cronus:
...
That writes `~/.cronus/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.cronus/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.cronus/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Cronus isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `cronus gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Cronus will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.cronus/google_chat_user_tokens/<sanitized_email>.json` (filesystem
