...
description: "Chat with Cronus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Cronus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Cronus](/guides/use-voice-mode-with-cronus).
...
```mermaid
flowchart TB
    subgraph Gateway["Cronus Gateway"]
...
```bash
cronus gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
cronus gateway              # Run in foreground
cronus gateway setup        # Configure messaging platforms interactively
cronus gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo cronus gateway install --system   # Linux only: install a boot-time system service
cronus gateway start        # Start the default service
cronus gateway stop         # Stop the default service
cronus gateway status       # Check default service status
cronus gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Cronus Agent to the latest version |
...
Configure per-platform overrides in `~/.cronus/gateway.json`:
...
cronus pairing approve telegram XKGH5N7P

# Other pairing commands:
cronus pairing list          # View pending + approved users
cronus pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Cronus appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.cronus/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.cronus/config.yaml`:
...
```bash
CRONUS_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
cronus gateway install               # Install as user service
cronus gateway start                 # Start the service
cronus gateway stop                  # Stop the service
cronus gateway status                # Check status
journalctl --user -u cronus-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo cronus gateway install --system
sudo cronus gateway start --system
sudo cronus gateway status --system
journalctl -u cronus-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Cronus will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Cronus installations on the same machine (with different `CRONUS_HOME` directories), each gets its own systemd service name. The default `~/.cronus` uses `cronus-gateway`; other installations use `cronus-gateway-<hash>`. The `cronus gateway` commands automatically target the correct service for your current `CRONUS_HOME`.
:::

### macOS (launchd)

```bash
cronus gateway install               # Install as launchd agent
cronus gateway start                 # Start the service
cronus gateway stop                  # Stop the service
cronus gateway status                # Check status
tail -f ~/.cronus/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.cronus.gateway.plist`. It includes three environment variables:
...
- **CRONUS_HOME** — scopes the gateway to your Cronus installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `cronus gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CRONUS_HOME` directory gets its own launchd label. The default `~/.cronus` uses `ai.cronus.gateway`; other installations use `ai.cronus.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `cronus-cli` | Full access |
| Telegram | `cronus-telegram` | Full tools including terminal |
| Discord | `cronus-discord` | Full tools including terminal |
| WhatsApp | `cronus-whatsapp` | Full tools including terminal |
| Slack | `cronus-slack` | Full tools including terminal |
| Google Chat | `cronus-google_chat` | Full tools including terminal |
| Signal | `cronus-signal` | Full tools including terminal |
| SMS | `cronus-sms` | Full tools including terminal |
| Email | `cronus-email` | Full tools including terminal |
| Home Assistant | `cronus-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `cronus-mattermost` | Full tools including terminal |
| Matrix | `cronus-matrix` | Full tools including terminal |
| DingTalk | `cronus-dingtalk` | Full tools including terminal |
| Feishu/Lark | `cronus-feishu` | Full tools including terminal |
| WeCom | `cronus-wecom` | Full tools including terminal |
| WeCom Callback | `cronus-wecom-callback` | Full tools including terminal |
| Weixin | `cronus-weixin` | Full tools including terminal |
| BlueBubbles | `cronus-bluebubbles` | Full tools including terminal |
| QQBot | `cronus-qqbot` | Full tools including terminal |
| Yuanbao | `cronus-yuanbao` | Full tools including terminal |
| Microsoft Teams | `cronus-teams` | Full tools including terminal |
| API Server | `cronus-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `cronus-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.cronus/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
