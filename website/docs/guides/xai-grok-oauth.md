...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Cronus Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Cronus Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Cronus automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Cronus — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Cronus opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.cronus/auth.json` and refreshed automatically before they expire.
...
```bash
cronus auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Cronus detects the remote environment and prints the authorization URL instead of opening a browser.
...
If you don't have a regular SSH client (e.g. you're running Cronus inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Cronus skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
cronus auth add xai-oauth --manual-paste
# Or via the model picker:
cronus model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/NousResearch/cronus-agent/issues/26923).

## How the Login Works

1. Cronus opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Cronus and the tokens are saved to `~/.cronus/auth.json`.
4. From then on, Cronus refreshes the access token in the background — you stay signed in until you `cronus auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
cronus config set model.default grok-4.3
cronus config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.cronus/config.yaml` will contain:
...
```bash
cronus --provider xai-oauth        # canonical
cronus --provider grok-oauth       # alias
cronus --provider x-ai-oauth       # alias
cronus --provider xai-grok-oauth   # alias
```
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `cronus setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

Cronus refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Cronus surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Cronus marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `cronus auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Cronus raises a timeout error.

**Fix:** re-run `cronus auth add xai-oauth` (or `cronus model`). The flow starts fresh.

### State mismatch (possible CSRF)

Cronus detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Cronus prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `cronus model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/cronus-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
cronus config set model.provider xai
```
...
**Fix:** run `cronus model` and pick the xAI Grok OAuth provider, or run `cronus auth add xai-oauth`.
...
```bash
cronus auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `cronus auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `cronus auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Cronus is on a different machine than your browser
