---
sidebar_position: 7
title: "Use SOUL.md with Cronus"
description: "How to use SOUL.md to shape Cronus Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Cronus

`SOUL.md` is the **primary identity** for your Cronus instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Cronus to feel like the same assistant every time you talk to it — or if you want to replace the Cronus persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Cronus should be
- what Cronus should avoid stylistically
- how Cronus should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Cronus is and how Cronus speaks
...
Cronus now uses only the global SOUL file for the current instance:

```text
~/.cronus/SOUL.md
```

If you run Cronus with a custom home directory, it becomes:
...
Cronus automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Cronus does not overwrite it
- if the file exists but is empty, Cronus adds nothing from it to the prompt

## How Cronus uses it

When Cronus starts a session, it reads `SOUL.md` from `CRONUS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Cronus feels.
...
Cronus already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Cronus should sound.

# Avoid
What Cronus should not do.

# Defaults
How Cronus should behave when ambiguity appears.
```
...
Then restart Cronus or start a new session.
...
### I edited SOUL.md but Cronus still sounds the same

Check:
- you edited `~/.cronus/SOUL.md` or `$CRONUS_HOME/SOUL.md`
...
### Cronus is ignoring parts of my SOUL.md
