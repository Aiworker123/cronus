---
sidebar_position: 8
title: "Use Voice Mode with Cronus"
description: "A practical guide to setting up and using Cronus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Cronus
...
- you want Cronus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Cronus.
...
## Step 1: make sure normal Cronus works first
...
Cronus supports both local and cloud speech stacks.
...
### If you use `cronus setup`

If you choose NeuTTS in the setup wizard, Cronus checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Cronus transcribes and responds
...
- asking Cronus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Cronus loop.

## Tuning CLI behavior

### Silence threshold

If Cronus starts/stops too aggressively, tune:
...
Cronus stays a normal chat bot, but can speak replies.
...
- you want Cronus to function like a portable research or ops assistant
...
Cronus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Cronus detects speech boundaries
- transcripts are posted in the associated text channel
- Cronus responds in text and audio
...
1. get text Cronus working
2. install `cronus-agent[voice]`
