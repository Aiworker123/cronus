...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `cronus send`."
---

# Pipe Script Output to Messaging Platforms

`cronus send` is a small, scriptable CLI that pushes a message to any
messaging platform Cronus is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | cronus send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `cronus
...
cronus send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
cronus send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
cronus send --list

# Filter by platform
cronus send --list telegram
```
...
Any platform Cronus ships adapters for works as a target:
...
`cronus send` resolves the message body in this order:

1. **Positional argument** — `cronus send --to telegram "hi"`
2. **`--file PATH`** — `cronus send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | cronus send --to telegram`

When stdin is a TTY (no pipe), Cronus does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  cronus send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `cronus send` reuses your Cronus config, the same script works on
any host where Cronus is installed — no need to export bot tokens into
...
`cronus send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  cronus send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | cronus send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.cronus/bin/cronus send \
...
```bash
./train.py --epochs 200 && \
  cronus send --to telegram "training done" || \
  cronus send --to telegram "training failed (exit $?)"
```
...
cronus send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(cronus send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `cronus send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `cronus send` calls
the platform's REST endpoint directly using credentials from
`~/.cronus/.env` and `~/.cronus/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `cronus gateway start` and retry.
...
# Machine-readable
cronus send --list --json
```

The listing is built from `~/.cronus/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`cronus gateway
...
| Approach | Multi-platform | Reuses Cronus creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `cronus send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`cronus send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `cronus send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `cronus send` shares with cron delivery.
