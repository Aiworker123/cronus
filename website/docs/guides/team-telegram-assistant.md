...
This tutorial walks you through setting up a Telegram bot powered by Cronus Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Cronus Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.cronus/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Cronus itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Cronus Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_cronus_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Cronus Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Cronus Gateway...
...
# View live logs
journalctl --user -u cronus-gateway -f
...
cronus gateway start
cronus gateway stop
tail -f ~/.cronus/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `cronus gateway install` to update the plist.
:::
...
```bash
cronus gateway stop && cronus gateway start
```
...
cronus pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.cronus/.env`:
...
Control how much detail the bot shows when using tools. In `~/.cronus/config.yaml`:
...
Customize how the bot communicates by editing `~/.cronus/SOUL.md`:

For a full guide, see [Use SOUL.md with Cronus](/guides/use-soul-with-cronus).
...
```bash
# From the CLI
cronus cron list          # View all scheduled jobs
cronus cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u cronus-gateway` (Linux) or `~/.cronus/logs/gateway.log` (macOS) |
| Cron job output | `~/.cronus/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.cronus/cron/jobs.json` |
| Pairing data | `~/.cronus/pairing/` |
| Session history | `~/.cronus/sessions/` |
