...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Cronus Agent — no API key required"
---

# MiniMax OAuth

Cronus Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Cronus automatically refreshes your session.
...
# → Cronus opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.cronus/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.cronus/.env
```
...
```bash
cronus auth add minimax-oauth --no-browser
```

Cronus will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Cronus implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Cronus generates a PKCE verifier / challenge pair and a random state value.
...
4. Cronus polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.cronus/auth.json` under the `minimax-oauth` key.
...
```bash
cronus config set model.default MiniMax-M2.7
cronus config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.cronus/config.yaml` will contain entries similar to:
...
```bash
cronus --provider minimax-oauth    # canonical
cronus --provider minimax-portal   # alias
cronus --provider minimax-global   # alias
cronus --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `cronus setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
cronus --provider minimax-oauth
```
...
Cronus refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Cronus marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Cronus marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `cronus auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Cronus raises a timeout error.

**Fix:** re-run `cronus auth add minimax-oauth` (or `cronus model`). The flow starts fresh.

### State mismatch (possible CSRF)

Cronus detected that the `state` value returned by the authorization server does not match what it sent.
...
If `cronus` cannot open a browser window, use `--no-browser`:

```bash
cronus auth add minimax-oauth --no-browser
```

Cronus prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `cronus model` and select MiniMax (OAuth), or run `cronus auth add minimax-oauth`.
...
```bash
cronus auth remove minimax-oauth
```
...
- [cronus doctor](../reference/cli-commands.md)
