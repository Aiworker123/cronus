...
description: "Diagnose and fix common Cronus cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`cronus gateway` for foreground, or `cronus gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `cronus cron tick`.
...
```bash
date
cronus cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.cronus/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.cronus/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.cronus/.env` |
...
| `local` | Write access to `~/.cronus/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `cronus cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `cronus skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `cronus skills list`.
...
2. `~/.cronus/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `cronus cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Cronus config directory). Verify:
```bash
ls ~/.cronus/scripts/your-script.py   # Must exist
cronus cron edit <job_id> --script ~/.cronus/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `cronus skills install <skill-name>`.
...
Jobs are stored in `~/.cronus/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.cronus/cron/jobs.json
chmod 600 ~/.cronus/cron/jobs.json   # Your user should own it
```
...
```bash
cronus cron list                    # Show all jobs, states, next_run times
cronus cron run <job_id>            # Schedule for next tick (for testing)
cronus cron edit <job_id>           # Fix configuration issues
cronus logs                         # View recent Cronus logs
cronus skills list                  # Verify installed skills
```
...
1. Run the job with `cronus cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.cronus/logs/agent.log` for scheduler messages and `~/.cronus/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/cronus-agent](https://github.com/NousResearch/cronus-agent) with:
