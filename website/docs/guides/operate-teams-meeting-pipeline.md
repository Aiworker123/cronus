...
```bash
cronus teams-pipeline validate
```
...
```bash
cronus teams-pipeline token-health
cronus teams-pipeline token-health --force-refresh
```
...
```bash
cronus teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
cronus teams-pipeline maintain-subscriptions
cronus teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Cronus cron (recommended if you already run the Cronus gateway)

Cronus ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.cronus/scripts/`. First create the script:

```bash
mkdir -p ~/.cronus/scripts
cat > ~/.cronus/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec cronus teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.cronus/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
cronus cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/cronus-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Cronus Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=cronus
EnvironmentFile=/etc/cronus/env
ExecStart=/usr/local/bin/cronus teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/cronus-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Cronus Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now cronus-teams-pipeline-maintain.timer
systemctl list-timers cronus-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/cronus teams-pipeline maintain-subscriptions >> /var/log/cronus/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.cronus/.env` at the top of a wrapper script that crontab calls.
...
```bash
cronus teams-pipeline subscriptions   # should show expirationDateTime advanced
cronus teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
cronus teams-pipeline list
cronus teams-pipeline list --status failed
cronus teams-pipeline show <job-id>
```

### Replay a stored job

```bash
cronus teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
cronus teams-pipeline fetch --meeting-id <meeting-id>
cronus teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `cronus teams-pipeline maintain-subscriptions --dry-run`
- inspect `cronus teams-pipeline list --status failed`
...
- run `cronus teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `cronus teams-pipeline run`
...
- [ ] `cronus teams-pipeline validate` returns an OK snapshot
- [ ] `cronus teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Cronus cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
