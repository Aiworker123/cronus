---
sidebar_position: 6
title: "Use MCP with Cronus"
description: "A practical guide to connecting MCP servers to Cronus Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Cronus

This guide shows how to actually use MCP with Cronus Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Cronus tool
- you want Cronus to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Cronus to internal APIs, databases, or company systems without modifying Cronus core

Do not use MCP when:
- a built-in Cronus tool already solves the job well
...
- Cronus remains the agent
- MCP servers contribute tools
- Cronus discovers those tools at startup or reload time
...
If you installed Cronus with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Cronus banner/status should show MCP integration when configured
- ask Cronus what tools it has available
...
## WSL2: bridge Cronus in WSL to Windows Chrome

This is the practical setup when:

- Cronus runs inside WSL2
...
In this setup, Cronus does **not** connect to Chrome directly. Instead:

- Cronus runs in WSL
- Cronus starts a local stdio MCP server
...
```text
Cronus (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Cronus stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Cronus core browser transport
...
```bash
cronus mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
cronus mcp test chrome-devtools-win
```

Then start a fresh Cronus session or run:
...
Once loaded, Cronus can use the MCP-prefixed browser tools directly. For example:
...
If Cronus runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Cronus from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Cronus from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Cronus:
...
2. Cronus-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Cronus will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Cronus to reason over a bounded workspace.
...
Now Cronus can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Cronus core.
...
Because Cronus now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
