...
description: "How Cronus builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Cronus deliberately separates:
...
1. agent identity — `SOUL.md` from `CRONUS_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.cronus/SOUL.md)
You are Cronus, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.cronus/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_cronus_md(cwd_path)       # 1. .cronus.md / CRONUS.md (walks to git root)
    ...
    # SOUL.md from CRONUS_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.cronus.md`, `CRONUS.md` | CWD up to git root | Cronus-native project config |
...
- **YAML frontmatter stripped** — `.cronus.md` frontmatter is removed (reserved for future config overrides)
...
1. `.cronus.md` / `CRONUS.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Cronus already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.cronus/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.cronus/MEMORY.md` and `~/.cronus/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.cronus.md`, `CRONUS.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Cronus.
- Ephemeral overlays such as `CRONUS_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Cronus assembles prompts for everyone, change Python and treat it as a code contribution
