...
The messaging gateway is the long-running process that connects Cronus to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `cronus_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.cronus/.env` | API keys, bot tokens, platform credentials |
| `~/.cronus/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`cronus send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.cronus/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `cronus gateway start` / `cronus gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.cronus/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `cronus gateway stop` stops only the current profile's gateway. `cronus gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
