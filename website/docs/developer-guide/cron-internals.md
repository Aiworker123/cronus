...
description: "How Cronus stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `cronus_cli/cron.py` | CLI `cronus cron` subcommands |
...
Jobs are stored in `~/.cronus/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `cronus cron` commands are run or during active CLI sessions.
...
```python
# ~/.cronus/scripts/check_competitors.py
...
2. **Environment variable** — `CRONUS_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.cronus/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `cronus cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `cronus cron` CLI provides direct job management:

```bash
cronus cron list                    # Show all jobs
cronus cron create                  # Interactive job creation (alias: add)
cronus cron edit <job_id>           # Edit job configuration
cronus cron pause <job_id>          # Pause a running job
cronus cron resume <job_id>         # Resume a paused job
cronus cron run <job_id>            # Trigger immediate execution
cronus cron remove <job_id>         # Delete a job
```
