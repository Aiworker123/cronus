...
The ACP adapter wraps Cronus' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
cronus acp / cronus-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.cronus/.env
  -> configure stderr logging
  -> construct CronusACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'cronus-agent[acp]==<version>' cronus-acp`, pointed at the `cronus-agent` PyPI release.
...
- `allow_once` -> Cronus `once`
- `allow_always` -> Cronus `always`
- reject options -> Cronus `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Cronus tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["cronus-acp"])
...
Instead it reuses Cronus' runtime resolver:

- `acp_adapter/auth.py`
- `cronus_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Cronus provider/credentials. It also always advertises a terminal setup auth method (`cronus-setup`, args `--setup`) so first-run registry clients can open Cronus' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.cronus/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `cronus-acp` toolset definition
- `cronus_cli/main.py` — `cronus acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `cronus-acp` script
