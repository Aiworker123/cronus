...
sidebar_position: 1
title: "Architecture"
description: "Cronus Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Cronus Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ cronus_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CronusCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── cronus_state.py           # SQLite session/state database with FTS5
├── cronus_constants.py       # CRONUS_HOME, profile-aware paths
...
│
├── cronus_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `cronus` subcommands (large file)
...
│   ├── skills_config.py      # cronus skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # cronus tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # cronus gateway start/stop
│
...
```text
User input → CronusCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .cronus.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.cronus/plugins/` (user), `.cronus/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `cronus plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-cronus-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Cronus as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`cronus -p <name>`) gets its own CRONUS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
