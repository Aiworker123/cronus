...
This page covers all commands related to [Cronus profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `cronus profile` without a subcommand shows help.
...
```bash
cronus profile use <name>
```

Sets `<name>` as the active profile. All subsequent `cronus` commands (without `-p`) will use this profile.
...
cronus profile use default
```

## `cronus profile create`

```bash
cronus profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `cronus profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `cronus update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
cronus profile create mybot

# Clone config only from current profile
cronus profile create work --clone

# Clone everything from current profile
cronus profile create backup --clone-all

# Clone config from a specific profile
cronus profile create work2 --clone --clone-from work
```

## `cronus profile describe`

```bash
cronus profile describe [<name>] [options]
```
...
cronus profile describe researcher

# Set it explicitly
cronus profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
cronus profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
cronus profile describe --all --auto
```

## `cronus profile delete`

```bash
cronus profile delete <name> [options]
```
...
```bash
cronus profile delete mybot
cronus profile delete mybot --yes
```
...
```bash
cronus profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Cronus home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ cronus profile show work
Profile: work
Path:    ~/.cronus/profiles/work
...
## `cronus profile alias`

```bash
cronus profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Cronus installation.
...
```bash
cronus profile alias work
# Creates/updates ~/.local/bin/work

cronus profile alias work --name mywork
# Creates ~/.local/bin/mywork

cronus profile alias work --remove
# Removes the wrapper script
```

## `cronus profile rename`

```bash
cronus profile rename <old-name> <new-name>
```
...
```bash
cronus profile rename mybot assistant
# ~/.cronus/profiles/mybot → ~/.cronus/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `cronus profile export`

```bash
cronus profile export <name> [options]
```
...
```bash
cronus profile export work
# Creates work.tar.gz in the current directory

cronus profile export work -o ./work-2026-03-29.tar.gz
```

## `cronus profile import`

```bash
cronus profile import <archive> [options]
```
...
```bash
cronus profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

cronus profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`cronus profile export` / `import` are still the right commands for
...
### `cronus profile install`

```bash
cronus profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `cronus -p telemetry`). |
...
cronus profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
cronus profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
cronus profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
cronus profile install ./telemetry/
```

### `cronus profile update`

```bash
cronus profile update <name> [--force-config] [--yes]
```
...
```bash
cronus profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Cronus version, author, env var requirements, the source URL/path, and
...
`cronus profile list` also shows the distribution name and version in a
`Distribution` column, and `cronus profile show <name>` / `delete <name>`
...
cronus profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
cronus profile install https://github.com/your-org/internal-assistant.git
```
...
`cronus_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Cronus version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Cronus can clone from.
3. Tell recipients to run `cronus profile install <your-repo-url>`.
...
## `cronus -p` / `cronus --profile`

```bash
cronus -p <name> <command> [options]
cronus --profile <name> <command> [options]
```

Global flag to run any Cronus command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
cronus -p work chat -q "Check the server status"
cronus --profile dev gateway start
cronus -p personal skills list
cronus -p work config edit
```

## `cronus completion`

```bash
cronus completion <shell>
```
...
- `cronus profile <TAB>` — subcommands (list, use, create, etc.)
- `cronus profile use <TAB>` — profile names
- `cronus -p <TAB>` — profile names
