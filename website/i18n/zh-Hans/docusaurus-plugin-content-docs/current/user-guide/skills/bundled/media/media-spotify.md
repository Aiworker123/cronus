...
通过 Cronus Spotify 工具集（7 个工具）控制用户的 Spotify 账户。设置指南：https://cronus-agent.nousresearch.com/docs/user-guide/features/spotify
...
**`401 Unauthorized` 重试后仍出现** — 刷新令牌已被撤销。告知用户重新运行 `cronus auth spotify`。
