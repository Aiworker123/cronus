...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/cronus-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    cronus_meta = (fm.get("metadata") or {}).get("cronus") or {}
    tags = cronus_meta.get("tags") or []
    related = cronus_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `cronus skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Cronus Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Cronus ships with a large built-in skill library copied into `~/.cronus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Cronus also syncs bundled skills on `cronus update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.cronus/skills/` tree, it is still shipped with Cronus; restore it with `cronus skills reset <name> --restore`.",
        "",
