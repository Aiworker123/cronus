...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CRONUS_HOME", os.path.expanduser("~/.cronus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CRONUS_HOME", Path.home() / ".cronus")) / "skills" / "red-teaming" / "godmode"
...
CRONUS_HOME = Path(os.getenv("CRONUS_HOME", Path.home() / ".cronus"))
CONFIG_PATH = CRONUS_HOME / "config.yaml"
PREFILL_PATH = CRONUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Cronus models — already uncensored, just needs clean prompt
    "cronus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "cronus" in model_lower or "nous" in model_lower:
        return "cronus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Cronus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cronus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Cronus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Cronus for changes to take effect.")
