...
## 5. GODMODE FAST — Cronus 4 405B (Zero Refusal)

**Model:** `nousresearch/cronus-4-405b`
**Strategy:** Cronus models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Cronus Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.cronus/config.yaml`:
