#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Cronus Agent.
...
# Ensure sibling modules (_cronus_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _cronus_home import display_cronus_home, get_cronus_home

CRONUS_HOME = get_cronus_home()
TOKEN_PATH = CRONUS_HOME / "google_token.json"
CLIENT_SECRET_PATH = CRONUS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = CRONUS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Cronus profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'cronus-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Cronus home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_cronus_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Cronus")
