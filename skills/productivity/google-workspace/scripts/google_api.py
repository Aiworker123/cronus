#!/usr/bin/env python3
"""Google Workspace API CLI for Cronus Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Cronus-facing JSON contract and falls back to the Python client
...
from _cronus_home import get_cronus_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CRONUS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Cronus Agent")
