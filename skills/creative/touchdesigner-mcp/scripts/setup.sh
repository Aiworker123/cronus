...
CRONUS_HOME_DIR="${CRONUS_HOME:-$HOME/.cronus}"
CRONUS_CFG="${CRONUS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Cronus config has twozero_td MCP entry ──
if [[ ! -f "$CRONUS_CFG" ]]; then
    echo -e " ${FAIL} Cronus config not found at ${CRONUS_CFG}"
    manual_steps+=("Create ${CRONUS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CRONUS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Cronus config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Cronus config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CRONUS_CFG} manually"); }
    manual_steps+=("Restart Cronus session to pick up config change")
fi
