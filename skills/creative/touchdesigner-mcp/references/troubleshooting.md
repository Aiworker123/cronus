...
## 5. Cronus Configuration

### Config location

`$CRONUS_HOME/config.yaml` (defaults to `~/.cronus/config.yaml` when `CRONUS_HOME` is unset)
...
Restart the Cronus session for changes to take effect. The MCP connection is
