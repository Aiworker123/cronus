# OpenAI-Compatible API Server for Cronus Agent
...
SSE streaming. By exposing this endpoint, cronus-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  cronus-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.cronus/.env`
2. Run `cronus gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with cronus-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns cronus-agent as a model) |
...
Authorization: Bearer cronus-api-key-here
...
But cronus-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "cronus-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured cronus-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of cronus-agent's core prompt
...
Once implemented, cronus-agent works as a drop-in backend for:
