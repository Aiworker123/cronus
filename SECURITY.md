# Cronus Agent Security Policy

This document describes Cronus Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/cronus-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Cronus Agent does not operate a bug
bounty program.**
...
- Environment details (`cronus version`, commit SHA, OS, Python
  version).
...
Cronus Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Cronus Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Cronus Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Cronus Agent's documentation
...
Cronus Agent supports two OS-level isolation postures. They address
...
Cronus Agent supports this in two ways:

- **Cronus Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Cronus Agent's in-process heuristics
...
Cronus Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Cronus Agent
itself. Bugs in Cronus Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Cronus Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Cronus Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Cronus Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  cronus-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Cronus Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Cronus Agent. Bugs in Cronus
...
- Cronus Agent includes supply-chain guards for MCP server
