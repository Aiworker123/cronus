...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    cronus_state = ModuleType("cronus_state")
    cronus_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "cronus_state", cronus_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Cronus"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Cronus"
