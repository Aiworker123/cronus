...
3. Multi-turn replay of ``codex_reasoning_items`` (with
   ...
   reverted shortly after: xAI confirmed they explicitly want Cronus to
...
# Cronus (https://x.ai/news/grok-cronus), making that hint actively wrong:
...
def test_summarize_api_error_surfaces_xai_entitlement_body_verbatim():
    """xAI's OAuth 403 body must surface as-is, with no Cronus-side hint."""
    from run_agent import AIAgent
    ...
    # No stale claim that X Premium is incompatible with Cronus.
...
def test_codex_reasoning_replay_includes_encrypted_content_for_xai():
    ...
    want Cronus to thread encrypted reasoning back across turns for
...
def test_grok_4_3_context_length_is_1m():
    ...
    Cronus' substring-match fallback used to return 256k (from the
...
def test_transport_round_trip_drops_foreign_reasoning():
    ...
    transport = ResponsesApiTransport()
    messages = [
        {"role": "system", "content": "you are cronus"},
        {"role": "user", "content": "hi"},
        _stamped_assistant_msg("xai_responses", encrypted="grok_blob"),
