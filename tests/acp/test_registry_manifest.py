"""Tests for ACP Registry metadata shipped with Cronus."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "cronus-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Cronus Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/cronus-agent"
    assert data["website"].startswith("https://cronus-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"cronus-agent[acp]=={data['version']}"
    assert uvx["args"] == ["cronus-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"cronus-agent[acp]=={_pyproject_version()}"
    )
