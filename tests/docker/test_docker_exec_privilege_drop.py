...
The shim (docker/cronus-exec-shim.sh, installed at /opt/cronus/bin/cronus)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> cronus login` would write /opt/data/auth.json as
...
failed: Cronus is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> cronus …`` (defaulting to root) gets dropped to the
   cronus user before the real binary runs.
2. ``docker exec --user cronus <c> cronus …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $CRONUS_HOME from a ``docker exec`` session land
   as cronus:cronus — the actual user-visible invariant.
4. The CRONUS_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def test_shim_drops_root_to_cronus_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `cronus` with a Python-style `-c` shim equivalent — there's no
    pure-cronus "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `cronus` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `cronus`
    do anything that writes to disk, then check the file's owner.

    Use `cronus config set` which writes config.yaml under CRONUS_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "cronus", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by cronus, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "cronus:cronus", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected cronus:cronus. "
        "The shim did not drop privileges before invoking cronus."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user cronus already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "cronus", sleep_container,
         "cronus", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user cronus failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up cronus:cronus — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """CRONUS_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "CRONUS_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "cronus", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With CRONUS_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``CRONUS_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``CRONUS_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"CRONUS_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "cronus", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "cronus:cronus", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected cronus:cronus"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/cronus/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid cronus cronus <args>` which resolves `cronus`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> cronus login`` (root) wrote
    ...
    "Cronus is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `cronus config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is cronus-readable.

    Specifically: pretend the operator ran `cronus login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the cronus UID. We use `cronus auth list` since that touches the
    ...
    # `cronus auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `cronus config set` first to
    # provoke a write into CRONUS_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "cronus", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # CRONUS_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "cronus", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the cronus user "
