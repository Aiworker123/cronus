"""Harness: dashboard opt-in via CRONUS_DASHBOARD.

Today (tini): dashboard starts once when CRONUS_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``cronus`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without CRONUS_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "cronus dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without CRONUS_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without CRONUS_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `cronus doctor` and any other health check).
    ...
    CRONUS_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without CRONUS_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with CRONUS_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "CRONUS_DASHBOARD=1", built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With CRONUS_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "CRONUS_DASHBOARD=1", built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'cronus dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with CRONUS_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """CRONUS_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "CRONUS_DASHBOARD=1", "-e", "CRONUS_DASHBOARD_PORT=9120",
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "cronus dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as cronus, so the
    # cronus user can kill it (same UID).
