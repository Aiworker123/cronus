...
Override the image with ``CRONUS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CRONUS_TEST_IMAGE", "cronus-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CRONUS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CRONUS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged cronus user
# ---------------------------------------------------------------------------
#
# Background: every Cronus runtime path inside the container drops to UID
# 10000 (the ``cronus`` user) via ``s6-setuidgid cronus``. ``docker exec``
...
# fails (PermissionError swallowed) for cronus, so a test that ran as root
...
# helpers below run every probe under ``-u cronus`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: cronus).
