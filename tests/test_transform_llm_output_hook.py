...
from cronus_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(cronus_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <cronus_home>/plugins/<name> and opt it in."""
    plugin_dir = cronus_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    cronus_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        cronus_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path / "cronus_empty"))
