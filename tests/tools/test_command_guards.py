...
def _clean_state():
    ...
    saved = {}
    for k in ("CRONUS_INTERACTIVE", "CRONUS_GATEWAY_SESSION", "CRONUS_EXEC_ASK", "CRONUS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CRONUS_INTERACTIVE"] = "1"
        session_key = os.getenv("CRONUS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CRONUS_INTERACTIVE or CRONUS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
