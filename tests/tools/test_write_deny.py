...
class TestWriteDenyExactPaths:
    ...
    def test_cronus_env(self):
        # ``.env`` under the active CRONUS_HOME (profile-aware, not just
        # ``~/.cronus``) must be write-denied. The hermetic test conftest
        # points CRONUS_HOME at a tempdir — resolve via get_cronus_home()
        # to match the denylist.
        from cronus_constants import get_cronus_home
        path = str(get_cronus_home() / ".env")
        assert _is_write_denied(path) is True

    def test_cronus_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.cronus/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "cronus_root"
        ...
        # Sanity check: CRONUS_HOME does point to the profile dir, not the root.
        from cronus_constants import get_cronus_home, get_default_cronus_root
        assert get_cronus_home() == profile_home
        assert get_default_cronus_root() == root
...
class TestWriteAllowed:
    ...
    def test_cronus_config_not_env(self):
        path = os.path.join(str(Path.home()), ".cronus", "config.yaml")
