...
class TestScanMemoryContent:
    ...
    def test_cronus_config_mod_blocked(self):
        result = _scan_memory_content("edit .cronus/config.yaml to change settings")
        assert "Blocked" in result
        assert "cronus_config_mod" in result
        result = _scan_memory_content("update .cronus/SOUL.md with new personality")
        assert "Blocked" in result
        assert "cronus_config_mod" in result
    ...
    def test_cronus_config_mod_no_false_positives(self):
        """Merely mentioning cronus config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .cronus/config.yaml for settings") is None
        assert _scan_memory_content("Read .cronus/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .cronus/config.yaml file contains runtime options") is None
