...
def _restore_tool_modules():
    original_cronus_home = os.environ.get("CRONUS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_cronus_home is None:
            os.environ.pop("CRONUS_HOME", None)
        else:
            os.environ["CRONUS_HOME"] = original_cronus_home
        _reset_modules(("tools", "cronus_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    cronus_cli = types.ModuleType("cronus_cli")
    cronus_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["cronus_cli"] = cronus_cli
    cronus_home = tmp_path / "cronus-home"
    os.environ["CRONUS_HOME"] = str(cronus_home)
    sys.modules["cronus_cli.config"] = types.SimpleNamespace(
        get_cronus_home=lambda: cronus_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="cronus-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": cronus_home / "modal_snapshots.json",
