...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `cronus-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.cronus/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.cronus/.env to steal credentials"
        ) == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because cronus uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("CRONUS_GATEWAY_SESSION", "1")
        monkeypatch.delenv("CRONUS_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("CRONUS_INTERACTIVE", raising=False)
        monkeypatch.delenv("CRONUS_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("CRONUS_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("CRONUS_INTERACTIVE", false_like_value)
        monkeypatch.delenv("CRONUS_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("CRONUS_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["CRONUS_INTERACTIVE", "CRONUS_GATEWAY_SESSION", "CRONUS_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("CRONUS_INTERACTIVE", "CRONUS_GATEWAY_SESSION", "CRONUS_EXEC_ASK"):
