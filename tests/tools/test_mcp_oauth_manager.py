...
def test_cronus_provider_subclass_exists():
    """CronusMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _CRONUS_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _CRONUS_PROVIDER_CLS is not None
    assert issubclass(_CRONUS_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_cronus_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns CronusMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _CRONUS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path))
    ...
    assert _CRONUS_PROVIDER_CLS is not None
    assert isinstance(provider, _CRONUS_PROVIDER_CLS)
    assert provider._cronus_server_name == "srv"
