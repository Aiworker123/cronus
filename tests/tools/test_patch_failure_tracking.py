...
def cronus_home(monkeypatch, tmp_path):
    """Isolate CRONUS_HOME and clear module-level caches afterward so the
...
class TestPatchFailureEscalation:
    def test_first_two_failures_use_normal_hint(self, cronus_home, tmp_path, fresh_tracker):
    ...
    def test_third_consecutive_failure_escalates(self, cronus_home, tmp_path, fresh_tracker):
        from tools.file_tools import _handle_patch
    ...
    def test_success_clears_failure_counter(self, cronus_home, tmp_path, fresh_tracker):
        from tools.file_tools import _handle_patch
    ...
    def test_different_paths_have_independent_counters(
        self, cronus_home, tmp_path, fresh_tracker
    ):
        from tools.file_tools import _handle_patch
