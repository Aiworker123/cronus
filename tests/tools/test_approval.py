...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("cronus_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("cronus_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"CRONUS_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_cronus_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.cronus/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_cronus_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $CRONUS_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_cronus_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$CRONUS_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_cronus_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $CRONUS_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.cronus/cronus-agent && source venv/bin/activate && python -m cronus_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m cronus_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m cronus_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "cronus_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart cronus-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_cronus_detected(self):
        """pkill targeting cronus/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_cronus_detected(self):
        cmd = "killall cronus"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep cronus) bypasses the pkill/killall name-matching
    ...
    def test_pkill_cronus_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("CRONUS_GATEWAY_SESSION", "CRONUS_YOLO_MODE",
                      "CRONUS_SESSION_KEY", "CRONUS_INTERACTIVE")
        }
        os.environ.pop("CRONUS_YOLO_MODE", None)
        os.environ.pop("CRONUS_INTERACTIVE", None)
        os.environ["CRONUS_GATEWAY_SESSION"] = "1"
        os.environ["CRONUS_SESSION_KEY"] = self.SESSION_KEY
