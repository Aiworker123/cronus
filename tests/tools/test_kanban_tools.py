...
Verifies:
  - Tools are gated on CRONUS_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `cronus chat` sessions (no CRONUS_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("CRONUS_KANBAN_TASK", raising=False)
    ...
    schema = registry.get_definitions(set(resolve_toolset("cronus-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CRONUS_KANBAN_TASK", "t_fake")
    home = tmp_path / ".cronus"
    home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the CRONUS_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: CRONUS_HOME isolated, CRONUS_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".cronus"
    home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(home))
    monkeypatch.setenv("CRONUS_PROFILE", "test-worker")
    monkeypatch.delenv("CRONUS_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("CRONUS_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["cronus_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CRONUS_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to CRONUS_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``CRONUS_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``cronus-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "cronus-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from CRONUS_PROFILE in the fixture, not the
        # caller-supplied "cronus-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CRONUS_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from cronus_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no CRONUS_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (CRONUS_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has CRONUS_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no CRONUS_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # CRONUS_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    monkeypatch.setenv("CRONUS_KANBAN_CRASH_GRACE_SECONDS", "0")
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("CRONUS_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("CRONUS_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no CRONUS_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via CRONUS_KANBAN_BOARD env var,
...
# restarting Cronus. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Cronus home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    CRONUS_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".cronus"
    home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(home))
    # Make sure neither CRONUS_KANBAN_DB nor CRONUS_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("CRONUS_KANBAN_DB", raising=False)
    monkeypatch.delenv("CRONUS_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("CRONUS_KANBAN_TASK", raising=False)
    monkeypatch.setenv("CRONUS_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning CRONUS_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".cronus"
    home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(home))
    monkeypatch.setenv("CRONUS_PROFILE", "alt-worker")
    monkeypatch.delenv("CRONUS_KANBAN_DB", raising=False)
    monkeypatch.delenv("CRONUS_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
