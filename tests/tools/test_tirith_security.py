...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._cronus_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(cronus_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(cronus_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_cronus_bin(self):
        """After _INSTALL_FAILED, manual install in CRONUS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        cronus_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(cronus_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(cronus_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._cronus_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == cronus_bin
            assert _tirith_mod._resolved_path == cronus_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# CRONUS_HOME isolation
# ---------------------------------------------------------------------------

class TestCronusHomeIsolation:
    def test_cronus_bin_dir_respects_cronus_home(self):
        """_cronus_bin_dir must use CRONUS_HOME, not hardcoded ~/.cronus."""
        from tools.tirith_security import _cronus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CRONUS_HOME": tmpdir}):
            result = _cronus_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_cronus_home(self):
        """_failure_marker_path must use CRONUS_HOME, not hardcoded ~/.cronus."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CRONUS_HOME": "/custom/cronus"}):
            result = _failure_marker_path()
        assert result == "/custom/cronus/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CRONUS_HOME; verify it's active."""
        cronus_home = os.getenv("CRONUS_HOME")
        assert cronus_home is not None, "CRONUS_HOME should be set by conftest"
        assert "cronus_test" in cronus_home, "Should point to test temp dir"

    def test_get_cronus_home_fallback(self):
        """Without CRONUS_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_cronus_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CRONUS_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".cronus")
            result = _get_cronus_home()
