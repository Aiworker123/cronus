...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        cronus_home.mkdir()
        (cronus_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(cronus_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.cronus/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        (cronus_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        (cronus_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        (cronus_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.cronus/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        cronus_home = tmp_path / ".cronus"
        (cronus_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"CRONUS_HOME": str(cronus_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.cronus")

        assert mounts[0]["container_path"] == "/home/user/.cronus/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.cronus/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.cronus/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape CRONUS_HOME."""
        monkeypatch.setenv("CRONUS_HOME", str(tmp_path / ".cronus"))
        (tmp_path / ".cronus").mkdir()

        # Create a sensitive file one level above cronus_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside cronus_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside CRONUS_HOME must still be registered."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
        (cronus_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_cronus_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of CRONUS_HOME must be allowed."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        subdir = cronus_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside CRONUS_HOME pointing outside must be rejected."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

        # Create a sensitive file outside cronus_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside cronus_home pointing outside
        symlink = cronus_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes CRONUS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, cronus_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape CRONUS_HOME."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
        ...
        sensitive.write_text("{}")
        self._write_config(cronus_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(cronus_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside CRONUS_HOME via config must still mount."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

        (cronus_home / "oauth.json").write_text("{}")
        self._write_config(cronus_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        (cronus_home / "cache" / "documents").mkdir(parents=True)
        (cronus_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.cronus/cache/documents" in paths
        assert "/root/.cronus/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.cronus/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        # Use legacy dir name — get_cronus_dir prefers old if it exists
        (cronus_home / "document_cache").mkdir()
        (cronus_home / "image_cache").mkdir()
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(cronus_home / "document_cache") in host_paths
        assert str(cronus_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.cronus/cache/documents" in container_paths
        assert "/root/.cronus/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        doc_dir = cronus_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        ss_dir = cronus_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.cronus/cache/screenshots/session_abc/screen1.png"
