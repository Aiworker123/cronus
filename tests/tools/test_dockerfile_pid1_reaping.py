...
- Signal forwarding runs through the init so ``docker stop`` triggers
  cronus's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, cronus accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, cronus accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into cronus,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, cronus still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``cronus-ink`` workspace package is
    ...
    assert "ui-tui/packages/cronus-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``cronus-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@cronus/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@cronus/ink``)
    ...
    assert "ui-tui/packages/cronus-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled cronus-ink workspace package "
        "so ``await import('@cronus/ink')`` resolves at runtime."
    )
