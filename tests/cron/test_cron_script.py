...
- Path resolution (absolute, relative to CRONUS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CRONUS_HOME."""
    cronus_home = tmp_path / ".cronus"
    cronus_home.mkdir()
    (cronus_home / "cron").mkdir()
    (cronus_home / "cron" / "output").mkdir()
    (cronus_home / "scripts").mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CRONUS_DIR", cronus_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", cronus_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", cronus_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", cronus_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.cronus/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CRONUS_SESSION_PLATFORM") is None
        assert os.environ.get("CRONUS_SESSION_CHAT_ID") is None
        assert os.environ.get("CRONUS_SESSION_CHAT_NAME") is None
