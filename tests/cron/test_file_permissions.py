...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("cronus_cli.config.get_config_path", return_value=config_path), \
             patch("cronus_cli.config.ensure_cronus_home"):
    ...
    def test_ensure_cronus_home_sets_0700(self):
        home = Path(self.tmpdir) / ".cronus"
        with patch("cronus_cli.config.get_cronus_home", return_value=home):
