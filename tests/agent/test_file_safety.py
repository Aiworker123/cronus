...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_cronus_env(self):
        """Cronus' own .env inside CRONUS_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: cronus internal .env is in ~/.cronus/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.cronus/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.cronus/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Cronus cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        cronus_home = tmp_path / ".cronus"
        cache = cronus_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._cronus_home_path", return_value=cronus_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Cronus cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        cronus_home = tmp_path / ".cronus"
        hub = cronus_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_cronus_home(self, tmp_path):
        """The env basename guard does not depend on CRONUS_HOME resolution."""
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        cronus_home = tmp_path / ".cronus"
        cache = cronus_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
