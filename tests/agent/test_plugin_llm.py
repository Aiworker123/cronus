...
""",
        ...
        from cronus_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir()
        (cronus_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
        from cronus_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from cronus_cli.plugins import PluginContext, PluginManifest, PluginManager
