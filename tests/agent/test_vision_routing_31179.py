...
def isolated_home(monkeypatch):
    """Temp CRONUS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="cronus_test_31179_")
    cronus_home = os.path.join(test_home, ".cronus")
    os.makedirs(cronus_home)
...
def _fresh_modules():
    """Drop cached cronus modules so each test reloads against current env."""
