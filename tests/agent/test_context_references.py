...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Cronus Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_cronus_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path / ".cronus"))

    cronus_env = tmp_path / ".cronus" / ".env"
    cronus_env.parent.mkdir(parents=True)
    cronus_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.cronus/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
