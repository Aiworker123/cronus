...
to a generic "Your API key was rejected... run cronus setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "cronus setup".
    assert "cronus auth add nous --type oauth" in source
