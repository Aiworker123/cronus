...
def test_cronus_client_tag_includes_current_version():
    """The client tag must reflect cronus_cli.__version__ verbatim."""
    from cronus_cli import __version__
    from agent.portal_tags import cronus_client_tag

    assert cronus_client_tag() == f"client=cronus-client-v{__version__}"


def test_cronus_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import cronus_client_tag

    tag = cronus_client_tag()
    assert tag.startswith("client=cronus-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import cronus_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=cronus-agent" in tags
    assert cronus_client_tag() in tags
    assert len(tags) == 2
