...
engine plugins (e.g. cronus-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``cronus plugins`` picker."""
    ...
    monkeypatch.setattr(
        "cronus_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "cronus_cli.plugins.get_plugin_context_engine",
