...
def backup_env(monkeypatch, tmp_path):
    """Isolate CRONUS_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_cronus_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd cronus_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under CRONUS_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level CRONUS_DIR picks up the tmp HOME."""
    import cronus_constants
    importlib.reload(cronus_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.cronus/cron/jobs.json
