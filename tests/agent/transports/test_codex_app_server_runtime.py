...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Cronus passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CRONUS_HOME", "/users/alice/.cronus/profiles/backend-worker")
        monkeypatch.setenv("CRONUS_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CRONUS_KANBAN_DB",
            "/users/alice/.cronus/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.cronus/kanban/boards/smoke"]'
            in cmd
        )
