...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CRONUS_HOME under tmp_path."""
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path / ".cronus"))
    (tmp_path / ".cronus").mkdir()

    # Force the constants/image cache helpers to re-read CRONUS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("cronus_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_cronus_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CRONUS_HOME — gateway cleanup
