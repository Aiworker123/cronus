...
def server():
    with patch.dict(
        "sys.modules",
        {
            "cronus_constants": MagicMock(
                get_cronus_home=MagicMock(return_value="/tmp/cronus_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'cronus-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'cronus-release' patched"
    }
