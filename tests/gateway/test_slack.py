...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.cronus."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /cronus, /btw, /stop,
        ...
        for expected in ("/cronus", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@cronus`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@cronus"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@cronus")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /cronus subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /cronus btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_cronus_prefix_still_works(self, adapter):
        """Backward compat: /cronus btw foo must still route to /btw foo.

        Old workspace manifests only declared /cronus as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_cronus_freeform_question(self, adapter):
        """/cronus <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/cronus",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_cronus_slash_stashes_context(self, adapter):
        """Legacy /cronus <subcommand> also stashes context."""
        command = {
            "command": "/cronus",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_cronus_question_does_not_stash_context(self, adapter):
        """Free-form /cronus <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/cronus",
