...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "cronus_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_cronus_home() / config.yaml`` — point it here.
    monkeypatch.setattr("cronus_constants.get_cronus_home", lambda: cronus_home)
    monkeypatch.setattr("cronus_cli.config.get_cronus_home", lambda: cronus_home)
    return cfg_path
