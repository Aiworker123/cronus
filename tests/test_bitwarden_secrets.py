...
binary download are exercised manually by `cronus secrets bitwarden
...
def cronus_home(tmp_path, monkeypatch):
    """Point Cronus at an isolated home directory."""
    home = tmp_path / ".cronus"
    home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(home))
    # Some modules cache get_cronus_home; clear if needed.
    import cronus_constants
    if hasattr(cronus_constants, "_CRONUS_HOME_CACHE"):
        cronus_constants._CRONUS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(cronus_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(cronus_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(cronus_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from cronus_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under cronus_home/cache/."""
    home = tmp_path / ".cronus"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
