"""
Tests for mcp_serve — Cronus MCP server.
...
def _isolate_cronus_home(tmp_path, monkeypatch):
    """Redirect CRONUS_HOME to a temp directory."""
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path))
    try:
        import cronus_constants
        monkeypatch.setattr(cronus_constants, "get_cronus_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking cronus_state schema."""
