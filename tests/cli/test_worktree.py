...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("cronus/cronus-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned cronus/* and pr-* branches."""

    def test_prunes_orphaned_cronus_branch(self, git_repo):
        """cronus/cronus-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "cronus/cronus-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "cronus/cronus-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "cronus/cronus-deadbeef" in result.stdout
        ...
        assert "cronus/cronus-deadbeef" in orphaned
        ...
        assert "cronus/cronus-deadbeef" not in result.stdout
