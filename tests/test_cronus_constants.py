"""Tests for cronus_constants module."""
...
class TestGetDefaultCronusRoot:
    """Tests for get_default_cronus_root() — Docker/custom deployment awareness."""

    def test_no_cronus_home_returns_native(self, tmp_path, monkeypatch):
        """When CRONUS_HOME is not set, returns ~/.cronus."""
        monkeypatch.delenv("CRONUS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_cronus_root() == tmp_path / ".cronus"

    def test_cronus_home_is_native(self, tmp_path, monkeypatch):
        """When CRONUS_HOME = ~/.cronus, returns ~/.cronus."""
        native = tmp_path / ".cronus"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(native))
        assert get_default_cronus_root() == native

    def test_cronus_home_is_profile(self, tmp_path, monkeypatch):
        """When CRONUS_HOME is a profile under ~/.cronus, returns ~/.cronus."""
        native = tmp_path / ".cronus"
    ...
    def test_cronus_home_is_docker(self, tmp_path, monkeypatch):
        """When CRONUS_HOME points outside ~/.cronus (Docker), returns CRONUS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(docker_home))
        assert get_default_cronus_root() == docker_home

    def test_cronus_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any CRONUS_HOME outside ~/.cronus is treated as the root."""
        custom = tmp_path / "my-cronus-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(custom))
        assert get_default_cronus_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (CRONUS_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(profile))
        assert get_default_cronus_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(cronus_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(cronus_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".cronus"
