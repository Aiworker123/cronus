...
Run from the PR worktree:
    ...
    cd ~/.cronus/cronus-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.cronus/cronus-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.cronus/cronus-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a cronus-agent checkout"
)
...
# Isolated CRONUS_HOME for the config write.
