"""Tests for cronus_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Cronus entry point
(cronus, cronus-agent, cronus-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Cronus entry point imports cronus_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported cronus_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("cronus_bootstrap", None)
    import cronus_bootstrap  # noqa: WPS433
    return cronus_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Cronus CLI
...
class TestEntryPointsImportBootstrap:
    """Every Cronus entry point must import cronus_bootstrap as its
    ...
    # Entry points that invoke Cronus as a process.  Each one must
    # import cronus_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "cronus_cli/main.py",   # cronus CLI (console_script)
        "run_agent.py",          # cronus-agent (console_script)
        "acp_adapter/entry.py",  # cronus-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import cronus_bootstrap' and that
        ...
        half-finished ``cronus update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``cronus_bootstrap`` as a top-level module) leaves cronus
        ...
        """
        # Resolve relative to the cronus-agent repo root.  Tests live
        # at tests/test_cronus_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # cronus start even when cronus_bootstrap hasn't been
        ...
        assert first_import_name == "cronus_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'cronus_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import cronus_bootstrap' line to be the first import."
        )
