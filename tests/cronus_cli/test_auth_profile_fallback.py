...
When ``CRONUS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.cronus/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.cronus            (has its own auth.json fixture)
    * Profile     -> tmp_path/.cronus/profiles/coder   (active, CRONUS_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Cronus is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from cronus_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    cronus_home = tmp_path / "classic"
    cronus_home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

    _write(cronus_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CRONUS_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.cronus as the "real home" — which is NOT equal
    # to the CRONUS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    cronus_home = tmp_path / "classic"
    ...
    _write(cronus_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from cronus_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: CRONUS_HOME is set to a custom path that is NOT under
    # ~/.cronus/profiles/ — get_default_cronus_root() returns CRONUS_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from cronus_cli.auth import read_credential_pool, write_credential_pool
