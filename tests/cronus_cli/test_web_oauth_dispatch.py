"""Regression tests for the OAuth dispatcher in cronus_cli.web_server.
...
from cronus_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Cronus-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "cronus_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "cronus_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "cronus_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_honors_legacy_scope_override(monkeypatch):
    ...
    from cronus_cli import web_server as ws
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "cronus_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "cronus_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "cronus auth add hypothetical-pkce-provider",
