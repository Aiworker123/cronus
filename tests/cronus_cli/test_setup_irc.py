"""Tests for IRC gateway configuration via `cronus setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Cronus install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Cronus install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#cronus")
            monkeypatch.setenv("IRC_NICKNAME", "cronus-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `cronus setup gateway` experience with IRC present."""
