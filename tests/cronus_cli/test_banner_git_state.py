...
def test_format_banner_version_label_without_git_state():
    from cronus_cli import banner

    with patch.object(banner, "get_git_banner_state", return_value=None):
        value = banner.format_banner_version_label()

    assert value == f"Cronus Agent v{banner.VERSION} ({banner.RELEASE_DATE})"
...
def test_get_git_banner_state_reads_origin_and_head(tmp_path):
    ...
    with patch("cronus_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_get_git_banner_state_falls_back_to_build_sha_when_no_repo():
    ...
    parent nor ``$CRONUS_HOME/cronus-agent/`` is a git repo (the canonical
...
def test_get_git_banner_state_falls_back_when_live_git_returns_nothing(tmp_path):
    ...
    with patch("cronus_cli.banner.subprocess.run", return_value=failed), \
