...
from cronus_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def test_full_login_round_trip_unlocks_api_status(gated_app):
    ...
    assert any("cronus_session_at" in c for c in set_cookies)
    assert any("cronus_session_rt" in c for c in set_cookies)
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("cronus_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("cronus_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
