"""Tests for cronus_cli.mcp_catalog and cronus_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("CRONUS_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_cronus_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp CRONUS_HOME."""
    hh = tmp_path / "cronus-home"
    hh.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(hh))
    monkeypatch.setattr(
        "cronus_cli.config.get_cronus_home", lambda: hh
    )
    monkeypatch.setattr(
        "cronus_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "cronus_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_cronus_home() lazily through cronus_constants
    monkeypatch.setattr(
        "cronus_constants.get_cronus_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from cronus_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from cronus_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from cronus_cli.mcp_catalog import install_entry, get_entry
    ...
    def test_install_with_install_dir_substitution(self, catalog_dir, tmp_path):
        ...
        from cronus_cli import mcp_catalog
        from cronus_cli.mcp_catalog import install_entry, get_entry
        from cronus_cli.config import load_config
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from cronus_cli import mcp_catalog
        from cronus_cli.mcp_catalog import install_entry, get_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from cronus_cli.mcp_catalog import install_entry, get_entry, uninstall_entry
        from cronus_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from cronus_cli.mcp_catalog import uninstall_entry
...
class TestToolSelection:
    ...
    def test_probe_fail_no_default_writes_no_filter(self, catalog_dir):
        body = _basic_manifest()
        _write_manifest(catalog_dir, "demo", body)
        from cronus_cli.mcp_catalog import install_entry
        from cronus_cli.config import load_config
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from cronus_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        entries so users running headless or after `cronus setup` know to update."""
        body = _basic_manifest()
        ...
        assert "requires a newer Cronus" in out
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from cronus_cli import mcp_catalog
        from cronus_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from cronus_cli import mcp_catalog
        from cronus_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        import cronus_cli.tools_config as tc
        ...
        monkeypatch.setattr(
            "cronus_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no CRONUS_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("CRONUS_OPTIONAL_MCPS", raising=False)
        from cronus_cli.mcp_catalog import _catalog_root, _parse_manifest
