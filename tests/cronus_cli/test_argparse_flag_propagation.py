...
subparser, placing the flag BEFORE the subcommand (e.g. 'cronus --yolo chat')
...
def _build_parser():
    """Build the cronus argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("cronus_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "cronus_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets CRONUS_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        args = parser.parse_args(["chat"])
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("CRONUS_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `cronus gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `cronus <argv>` must exit 0 (help) rather than
