"""Tests for cronus_cli.service_manager — the abstract ServiceManager
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from cronus_cli.service_manager import _s6_running
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the cronus user. The new
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "cronus_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "cronus_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "cronus_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "cronus_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "cronus_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "cronus_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "cronus_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "cronus_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `cronus_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import cronus_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("cronus_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "cronus_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "cronus_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "cronus_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    from cronus_cli.service_manager import S6ServiceManager
    monkeypatch.setattr(
        "cronus_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
# unprivileged cronus user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from cronus_cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "cronus -p coder gateway run" in run_text
    assert "s6-setuidgid cronus" in run_text
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid cronus cronus -p coder gateway run" in run_text
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "cronus profile create typo" in msg
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from cronus_cli.service_manager import S6CommandError, S6ServiceManager
