"""Tests for the kanban CLI surface (cronus_cli.kanban)."""
...
from cronus_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`cronus kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from cronus_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("CRONUS_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("CRONUS_KANBAN_BOARD") == "beta"
