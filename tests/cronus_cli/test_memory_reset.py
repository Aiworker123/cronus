"""Tests for the `cronus memory reset` CLI command.
...
- Profile-scoped reset (uses CRONUS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CRONUS_HOME with memory files."""
    cronus_home = tmp_path / ".cronus"
    memories = cronus_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nCronus repo is at ~/.cronus/cronus-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `cronus memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        (cronus_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CRONUS_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / ".cronus"
        cronus_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("CRONUS_HOME", str(cronus_home))

        # The memories dir won't exist; get_cronus_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
