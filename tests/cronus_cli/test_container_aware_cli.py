...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CRONUS_HOME with .container-mode file."""
    cronus_home = tmp_path / ".cronus"
    cronus_home.mkdir()
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
    monkeypatch.delenv("CRONUS_DEV", raising=False)

    container_mode = cronus_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=cronus-agent\n"
        "exec_user=cronus\n"
        "cronus_bin=/data/current-package/bin/cronus\n"
    )
    return cronus_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("cronus_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "cronus-agent"
    assert info["exec_user"] == "cronus"
    assert info["cronus_bin"] == "/data/current-package/bin/cronus"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("cronus_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_cronus_dev(container_env, monkeypatch):
    """Returns None when CRONUS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("CRONUS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_cronus_dev_zero(container_env, monkeypatch):
    """CRONUS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("CRONUS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        cronus_home = Path(tmpdir) / ".cronus"
        cronus_home.mkdir()
        (cronus_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("cronus_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_cronus_home": lambda: cronus_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CRONUS_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=cronus-custom\n"
        "exec_user=myuser\n"
        "cronus_bin=/opt/cronus/bin/cronus\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "cronus-custom"
    assert info["exec_user"] == "myuser"
    assert info["cronus_bin"] == "/opt/cronus/bin/cronus"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from cronus_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "cronus"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "cronus-agent" in cmd
    assert "/data/current-package/bin/cronus" in cmd
    assert "chat" in cmd
