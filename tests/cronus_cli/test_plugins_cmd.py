"""Tests for cronus_cli.plugins_cmd — the ``cronus plugins`` CLI subcommand."""
...
class TestResolveGitExecutable:
    ...
    def test_prefers_shutil_which(self):
        import cronus_cli.plugins_cmd as pc
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="cronus_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from cronus_cli.plugins_cmd import cmd_install
        import argparse

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("cronus_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("cronus_cli.plugins_cmd._display_after_install")
    @patch("cronus_cli.plugins_cmd.shutil.move")
    @patch("cronus_cli.plugins_cmd.shutil.rmtree")
    @patch("cronus_cli.plugins_cmd._plugins_dir")
    @patch("cronus_cli.plugins_cmd._read_manifest")
    @patch("cronus_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("cronus_cli.plugins_cmd._sanitize_plugin_name")
    @patch("cronus_cli.plugins_cmd._plugins_dir")
    @patch("cronus_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from cronus_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from cronus_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from cronus_cli.plugins_cmd import cmd_list
...
class TestDiscoverAllPlugins:
    """Exercise the recursive scan that powers ``cronus plugins list``.
    ...
    def _entries_by_key(self, tmp_path, monkeypatch) -> dict:
        from cronus_cli import plugins_cmd
        bundled = tmp_path / "bundled"
        user = tmp_path / "user"
        bundled.mkdir()
        user.mkdir()
        monkeypatch.setattr(
            "cronus_cli.plugins.get_bundled_plugins_dir", lambda: bundled
        )
    ...
    def test_user_memory_subdir_is_still_scanned(self, tmp_path, monkeypatch):
        ...
        plugin at ``~/.cronus/plugins/memory/<x>/`` should still be discovered
...
class TestCopyExampleFiles:
    ...
    def test_copies_example_files(self, tmp_path):
        from cronus_cli.plugins_cmd import _copy_example_files
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "cronus_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_skips_when_no_requires_env(self):
        from cronus_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock
    ...
    def test_skips_already_set_vars(self, monkeypatch):
        ...
        console = MagicMock()
        with patch("cronus_cli.config.get_env_value", return_value="already-set"):
    ...
    def test_prompts_for_missing_var_simple_format(self):
        ...
        with patch("cronus_cli.config.get_env_value", return_value=None), \
             patch("builtins.input", return_value="sk-test-123"), \
             patch("cronus_cli.config.save_env_value") as mock_save:
    ...
    def test_secret_uses_masked_prompt(self):
        ...
        with patch("cronus_cli.config.get_env_value", return_value=None), \
             patch("cronus_cli.plugins_cmd.masked_secret_prompt", return_value="s3cret") as mock_prompt, \
             patch("cronus_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
...
class TestProviderDiscovery:
    ...
    def test_get_current_memory_provider_default(self, tmp_path, monkeypatch):
        ...
        from cronus_cli.plugins_cmd import _get_current_memory_provider
    ...
    def test_get_current_context_engine_default(self, tmp_path, monkeypatch):
        ...
        from cronus_cli.plugins_cmd import _get_current_context_engine
    ...
    def test_save_memory_provider(self, tmp_path, monkeypatch):
        ...
        config_file = tmp_path / "config.yaml"
        config_file.write_text("memory:\n  provider: ''\n")
        from cronus_cli.plugins_cmd import _save_memory_provider
    ...
    def test_save_context_engine(self, tmp_path, monkeypatch):
        ...
        config_file = tmp_path / "config.yaml"
        config_file.write_text("context:\n  engine: compressor\n")
        from cronus_cli.plugins_cmd import _save_context_engine
    ...
    def test_discover_memory_providers_empty(self):
        ...
        with patch("plugins.memory.discover_memory_providers",
            ...
            from cronus_cli.plugins_cmd import _discover_memory_providers
    ...
    def test_discover_context_engines_empty(self):
        ...
        with patch("plugins.context_engine.discover_context_engines",
                    side_effect=ImportError("no module")):
            from cronus_cli.plugins_cmd import _discover_context_engines
