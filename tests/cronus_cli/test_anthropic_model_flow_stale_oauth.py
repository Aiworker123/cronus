"""Tests for Bug #12905 fix — stale OAuth token detection in cronus model flow.

Bug 3: `cronus model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.cronus/.env but no valid
...
from cronus_cli.config import load_env, save_env_value
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("cronus_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from cronus_cli.main import _model_flow_anthropic
        cfg = {}
