...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("cronus_cli.config.get_managed_system", return_value=None), \
         patch("cronus_cli.config.get_cronus_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When CRONUS_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("cronus_cli.config.get_managed_system", return_value="NixOS"), \
         patch("cronus_cli.config.get_cronus_home", return_value=tmp_path):
        from cronus_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from cronus_cli.config import recommended_update_command_for_method
...
def test_docker_detected_via_dockerenv(tmp_path):
    with patch("cronus_cli.config.get_managed_system", return_value=None), \
         patch("cronus_cli.config.get_cronus_home", return_value=tmp_path), \
         patch("cronus_constants.is_container", return_value=True):
        from cronus_cli.config import detect_install_method
        assert detect_install_method(project_root=tmp_path) == "docker"
