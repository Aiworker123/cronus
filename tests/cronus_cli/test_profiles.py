"""Comprehensive tests for cronus_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and CRONUS_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.cronus/profiles)
    * CRONUS_HOME  -> tmp_path/.cronus  (so get_cronus_home() agrees)
    * Creates the bare-minimum ~/.cronus directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["cronus", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Cronus install itself or with
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.cronus must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".cronus"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes cronus-agent, .worktrees,
        ...
        (default_home / "cronus-agent" / ".git").mkdir(parents=True)
        (default_home / "cronus-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "cronus-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "cronus-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `cronus profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `cronus update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("cronus_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".cronus" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_cronus_home_returns_default(self, profile_env):
        # CRONUS_HOME points to tmp_path/.cronus which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".cronus" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom CRONUS_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("CRONUS_HOME", str(custom))
        # With Docker-aware roots, a custom CRONUS_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("cronus")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "cronus.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["cronus.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["cronus.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "cronus.ssi_health": {"workspace": "cronus", "enabled": True}
            }
        }))
        ...
        assert "cronus.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["cronus.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["cronus.heimdall"]["workspace"] == "cronus"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "cronus.ssi_health": {"aiPeer": "ssi_health"},
                "cronus.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["cronus.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["cronus.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("cronus-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".cronus_history",
        ...
        excluded_prefixes = [
            "default/cronus-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.cronus_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultCronusHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_cronus_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".cronus" / "profiles"

    def test_default_cronus_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_cronus_home()
        assert home == tmp_path / ".cronus"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (CRONUS_HOME outside ~/.cronus), profiles go under CRONUS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_cronus_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_cronus_home() returns CRONUS_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (CRONUS_HOME under ~/.cronus), profiles root is still ~/.cronus/profiles."""
        native = tmp_path / ".cronus"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under CRONUS_HOME."""
        from cronus_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under CRONUS_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRONUS_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".cronus" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from cronus_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".cronus"
