...
empty `env_vars` — causing `cronus tools` to write the toolset to config
...
class TestPostSetupGate:
    def test_cua_driver_missing_forces_setup(self, monkeypatch, tmp_path):
        ...
        from cronus_cli import tools_config
    ...
    def test_cua_driver_installed_skips_setup(self, monkeypatch, tmp_path):
        ...
        so a re-save through `cronus tools` doesn't re-prompt the user."""
        from cronus_cli import tools_config
