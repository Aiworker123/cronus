"""Tests for Codex auth — tokens stored in Cronus auth store (~/.cronus/auth.json)."""
...
def _setup_cronus_auth(cronus_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Cronus auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_cronus_auth(cronus_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_cronus_auth(cronus_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
    ...
    monkeypatch.setattr("cronus_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    cronus_home = tmp_path / "cronus"
    _setup_cronus_auth(cronus_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((cronus_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``cronus auth add openai-codex`` as a workaround, leaving
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Cronus auth store, not ~/.codex/."""
    cronus_home = tmp_path / "cronus"
    codex_home = tmp_path / "codex-cli"
    cronus_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "cronus-at", "refresh_token": "cronus-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Cronus store
    assert not (codex_home / "auth.json").exists()

    # Cronus auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "cronus-at"


def test_resolve_returns_cronus_auth_store_source(tmp_path, monkeypatch):
    cronus_home = tmp_path / "cronus"
    _setup_cronus_auth(cronus_home)
    monkeypatch.setenv("CRONUS_HOME", str(cronus_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run cronus auth".
    """
    from cronus_cli.auth import (
    ...
    assert "cronus auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from cronus_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from cronus_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "cronus_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("cronus_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("cronus_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
