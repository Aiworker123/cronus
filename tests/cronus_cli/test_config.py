"""Tests for cronus_cli configuration management."""
...
class TestGetCronusHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CRONUS_HOME", None)
            home = get_cronus_home()
            assert home == Path.home() / ".cronus"

    def test_env_override(self):
        with patch.dict(os.environ, {"CRONUS_HOME": "/custom/path"}):
            home = get_cronus_home()
            assert home == Path("/custom/path")


class TestEnsureCronusHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path)}):
            ensure_cronus_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``cronus logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from cronus_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="cronus_cli.config"):
                config = load_config()
            ...
            # stands out at cronus startup before logging is configured)
            captured = capsys.readouterr()
            assert "cronus config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path)}):
            ...
            assert "cronus config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CRONUS_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from cronus_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``CRONUS_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Cronus
    process on next startup via the dotenv → ``os.environ`` chain in
    ``cronus_cli/env_loader.py``.
    ...
    def _cronus_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "CRONUS_SPOTIFY_CLIENT_ID",
            "CRONUS_QWEN_BASE_URL",
            "CRONUS_MAX_ITERATIONS",
        ],
    )
    def test_cronus_integration_keys_still_writable(self, allowed_key):
        """``CRONUS_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``CRONUS_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``CRONUS_*``) keep working. The denylist
