...
`CRONUS_REDACT_SECRETS` at module-import time. `cronus_cli/main.py` at
...
in .env), the toggle was silently ignored in both `cronus chat` and
`cronus gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `CRONUS_REDACT_SECRETS`
env var in `cronus_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports cronus_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("CRONUS_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import cronus_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('CRONUS_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["CRONUS_HOME"] = str(cronus_home)
    env.pop("CRONUS_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env CRONUS_REDACT_SECRETS takes precedence over config.yaml."""
    cronus_home = tmp_path / ".cronus"
    cronus_home.mkdir()
    (cronus_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (cronus_home / ".env").write_text("CRONUS_REDACT_SECRETS=true\n")
