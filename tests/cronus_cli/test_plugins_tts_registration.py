...
``$CRONUS_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/cronus_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        from cronus_cli.plugins import PluginManager

        from agent import tts_registry
        tts_registry._reset_for_tests()

        cronus_home = Path(os.environ["CRONUS_HOME"])
        _write_plugin(
            cronus_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(cronus_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(cronus_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(cronus_home, "shadow-tts-plugin")
