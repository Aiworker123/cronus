"""Tests for cronus_cli.gateway_windows."""

from pathlib import Path

import pytest

import cronus_cli.gateway as gateway
import cronus_cli.gateway_windows as gateway_windows
import cronus_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda cronus_home: "")
    monkeypatch.setattr("cronus_cli.config.get_cronus_home", lambda: str(tmp_path / "cronus-home"))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "cronus_cli.main"]
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Cronus_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Cronus_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Cronus_Gateway_alice")
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Cronus\\cronus-agent\\venv\\Scripts\\python.exe",
        r"C:\\Cronus\\cronus-agent",
        r"C:\\CronusHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows, "_derive_venv_pythonw", lambda exe: exe.replace("python.exe", "pythonw.exe"))
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Cronus\venv\Scripts\python.exe")
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Cronus_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Cronus_Gateway_alice")
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Cronus_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Cronus gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "cronus --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Cronus gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `cronus gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("cronus_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("cronus_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `cronus gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("cronus_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
