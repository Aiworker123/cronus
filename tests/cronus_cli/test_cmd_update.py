...
from cronus_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/cronus-agent silently misses updates.
        """
        from cronus_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/cronus-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "cronus_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "cronus_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("cronus_cli.config.check_config_version", return_value=(1, 2)), patch(
            "cronus_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("cronus_cli.main.sys") as mock_sys:
            ...
            from cronus_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.cronus"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.cronus/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.cronus/profiles/work"))
        ...
        with (
            patch("cronus_cli.profiles.list_profiles", return_value=all_profiles),
            patch("cronus_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("cronus_cli.profiles.list_profiles", return_value=[default_p]),
            patch("cronus_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``cronus update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``cronus update --check --branch <name>`` honors the branch override.
    ...
    @patch("cronus_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("cronus_cli.config.detect_install_method", return_value="pip")
    @patch("cronus_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``cronus update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from cronus_cli.main import _update_via_zip
