"""Tests for cronus_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $CRONUS_HOME under tmp_path; no real
...
from cronus_cli.container_boot import (
    ReconcileAction,
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under cronus_home/profiles/<name>/."""
    p = cronus_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `cronus profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    CRONUS_HOME root (the implicit default profile)."""
    if state is not None:
        (cronus_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (cronus_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (cronus_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        cronus_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from cronus_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        cronus_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $CRONUS_HOME/profiles doesn't exist (fresh install), the
    ...
    so `cronus gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        cronus_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare CRONUS_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $CRONUS_HOME/profiles/default/ instead of
    the root profile. It must call `cronus gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the CRONUS_HOME root (left
