"""Tests for the Phase 4 s6 dispatch helper in cronus_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`cronus gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from cronus_cli import gateway as gw
    monkeypatch.setattr(
        "cronus_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "cronus_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "cronus_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "cronus_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`cronus -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from cronus_cli import gateway as gw
    from cronus_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "cronus_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "cronus profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    from cronus_cli import gateway as gw
    from cronus_cli.service_manager import S6CommandError
    ...
    monkeypatch.setattr(
        "cronus_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "cronus_cli.service_manager.detect_service_manager",
...
def _stub_execvp(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    ...
    monkeypatch.setattr("cronus_cli.gateway.os.execvp", fake_execvp)
    return calls
...
def test_redirect_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "cronus_cli.gateway.os.execvp",
        lambda *a, **kw: pytest.fail("execvp should not be called on host"),
    )
    monkeypatch.delenv("CRONUS_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("CRONUS_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("cronus_cli.gateway._profile_suffix", lambda: "")
    ...
    assert "CRONUS_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `cronus gateway run --replace`, the
    CRONUS_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
    ...
    monkeypatch.setenv("CRONUS_S6_SUPERVISED_CHILD", "1")
    monkeypatch.delenv("CRONUS_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_respects_no_supervise_env(
    ...
    """`CRONUS_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.
    ...
    monkeypatch.delenv("CRONUS_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("CRONUS_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of CRONUS_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `CRONUS_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        monkeypatch.setenv("CRONUS_GATEWAY_NO_SUPERVISE", falsy)
