"""Tests for the Cronus plugin system (cronus_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<cronus_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<cronus_home>/plugins/``; we derive
    ``<cronus_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <CRONUS_HOME>/config.yaml.
        # Config is always read from CRONUS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        cronus_home_str = os.environ.get("CRONUS_HOME")
        if cronus_home_str:
            cronus_home = Path(cronus_home_str)
        else:
            cronus_home = base.parent
        cronus_home.mkdir(parents=True, exist_ok=True)
        cfg_path = cronus_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.cronus/plugins/ are discovered."""
        plugins_dir = tmp_path / "cronus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("CRONUS_HOME", str(tmp_path / "cronus_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRONUS_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".cronus" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        cronus_home = tmp_path / "cronus_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under cronus_plugins.<name>."""
        plugins_dir = tmp_path / "cronus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("CRONUS_HOME", str(tmp_path / "cronus_test"))

        # Clean up any prior namespace module
        sys.modules.pop("cronus_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "cronus_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="cronus_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "cronus_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import cronus_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("cronus_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        cronus_home = tmp_path / "cronus_test"
        plugins_dir = cronus_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "cronus_test" / "plugins"
        # Set CRONUS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("cronus_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("cronus_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("cronus_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("cronus_cli.plugins.PluginContext.dispatch_tool.__module__", "cronus_cli.plugins"):
...
class TestPluginDebugLogging:
    """CRONUS_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("CRONUS_PLUGINS_DEBUG", raising=False)
        from cronus_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With CRONUS_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("CRONUS_PLUGINS_DEBUG", "1")
        from cronus_cli import plugins as plugins_mod
