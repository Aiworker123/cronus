...
Before the fix, ``cronus whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``cronus gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    cronus = home / ".cronus"
    cronus.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("CRONUS_HOME", str(cronus))
...
def _env_value(cronus_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("cronus_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``cronus whatsapp`` with an existing paired session and
