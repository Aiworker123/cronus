"""Tests for the top-level `./cronus` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./cronus` should use `cronus_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "cronus"
    called = []

    fake_main_module = types.ModuleType("cronus_cli.main")

    def fake_main():
        called.append("cronus_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "cronus_cli.main", fake_main_module)
