...
``cronus_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``cronus --help``, ``cronus version``,
``cronus logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``cronus --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from cronus_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["cronus", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["cronus", "-w", "--tui"], None),
        (["cronus", "version"], "version"),
        (["cronus", "--tui", "chat"], "chat"),
        (["cronus", "-w", "logs"], "logs"),
        (["cronus", "chat", "hello world"], "chat"),
        (["cronus", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["cronus", "-m", "gpt5", "chat"], "chat"),
        (["cronus", "--model", "gpt5", "chat", "hi"], "chat"),
        (["cronus", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["cronus", "-z", "hello world"], None),
        (["cronus", "-z", "hello", "chat"], "chat"),
        (["cronus", "--model=gpt5", "chat"], "chat"),     # inline form
        (["cronus", "--", "chat"], "chat"),               # -- terminator
        (["cronus", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["cronus", "some-plugin-cmd"], "some-plugin-cmd"),
        (["cronus", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["cronus"],                          # bare → chat
        ["cronus", "--help"],                # top-level help
        ["cronus", "-h"],
        ["cronus", "version"],               # known built-in
        ["cronus", "logs"],
        ["cronus", "gateway", "run"],
        ["cronus", "--tui"],
        ["cronus", "-w", "--tui"],
        ["cronus", "chat", "hi"],
        ["cronus", "help"],                  # accepted built-in-ish
        ["cronus", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["cronus", "meet", "join"],          # potential google_meet plugin
        ["cronus", "honcho", "status"],      # potential memory plugin
        ["cronus", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``cronus help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"cronus_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
