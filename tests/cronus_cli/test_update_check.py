"""Tests for the update check mechanism in cronus_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "cronus-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("cronus_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path))
    with patch("cronus_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import cronus_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "cronus_cli" / "banner.py"
    ...
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path))
    with patch("cronus_cli.banner.subprocess.run") as mock_run:
        with patch("cronus_cli.banner.check_via_pypi", return_value=0):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when CRONUS_HOME has no git repo."""
    import cronus_cli.banner as banner
    ...
    # Point CRONUS_HOME at a temp dir with no cronus-agent/.git
    monkeypatch.setenv("CRONUS_HOME", str(tmp_path))
    with patch("cronus_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from cronus_cli.main import _invalidate_update_cache

    # Build a fake ~/.cronus with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"CRONUS_HOME": str(default_home)}):
