# Cronus Agent Has Had "Routines" Since March
...
Cronus equivalent — works today:
```bash
cronus webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Cronus implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Cronus Agent |
|---|---|---|
...
## Things Cronus Does That Routines Can't
...
Cronus has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Cronus Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
cronus gateway setup    # enable webhooks
cronus webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [cronus-agent.nousresearch.com/docs/guides/automation-templates](https://cronus-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [cronus-agent.nousresearch.com](https://cronus-agent.nousresearch.com)

GitHub: [github.com/NousResearch/cronus-agent](https://github.com/NousResearch/cronus-agent)

---

*Cronus Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
