# nix/nixosModules.nix — NixOS module for cronus-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: cronus runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CRONUS_HOME/.env
# and read by cronus at startup — no container recreation needed for env changes.
#
# Tool resolution: the cronus wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.cronus-agent;
    effectivePackage =
    ...
    cronus-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-cronus-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "cronus-config-attrs";
      description = "Cronus YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "cronus-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "cronus-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'CRONUS_DOC_EOF'\n${value}\nCRONUS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "cronus-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/cronus";
    ...
    # cronus user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "cronus-container-entrypoint" ''
      set -eu

      CRONUS_UID="''${CRONUS_UID:?CRONUS_UID must be set}"
      CRONUS_GID="''${CRONUS_GID:?CRONUS_GID must be set}"

      # ── Group: ensure a group with GID=$CRONUS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$CRONUS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="cronus"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$CRONUS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$CRONUS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$CRONUS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$CRONUS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="cronus"
        TARGET_HOME="/home/cronus"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$CRONUS_UID" -g "$CRONUS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$CRONUS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$CRONUS_UID:$CRONUS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure CRONUS_HOME is owned by the target user.
      ...
      if [ -n "''${CRONUS_HOME:-}" ] && [ -d "$CRONUS_HOME" ]; then
        find "$CRONUS_HOME" \! -user "$CRONUS_UID" -exec chown "$CRONUS_UID:$CRONUS_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/cronus-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/cronus-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/cronus ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/cronus
        chmod 0440 /etc/sudoers.d/cronus
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$CRONUS_UID" --regid="$CRONUS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $CRONUS_HOME/.env.
    ...
    # Default: /var/lib/cronus/workspace → /data/workspace.
  ...
  in {
    options.services.cronus-agent = with lib; {
      enable = mkEnableOption "Cronus Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = cronus-agent;
        description = "The cronus-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/cronus";
        description = "State directory. Contains .cronus/ subdir (CRONUS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Cronus config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CRONUS_HOME/.env at activation time.
          Cronus reads this file on every startup via load_cronus_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CRONUS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CRONUS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `cronus gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the cronus user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the cronus plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Cronus discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          cronus_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as cronus (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in cronus-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the cronus CLI to environment.systemPackages and export
          CRONUS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.cronus symlink to the service
            stateDir. These users are automatically added to the cronus group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.cronus-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the cronus CLI to system PATH and export CRONUS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.cronus/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CRONUS_HOME = "${cfg.stateDir}/.cronus";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.cronus-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the cronus user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.cronus-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed cronus
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure cronus is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.cronus        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cronus/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cronus/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cronus/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cronus/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cronus/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."cronus-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.cronus
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.cronus ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.cronus ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.cronus -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.cronus/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.cronus/$_subdir"
            chmod 2770 "${cfg.stateDir}/.cronus/$_subdir"
            find "${cfg.stateDir}/.cronus/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.cronus/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.cronus/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cronus/config.yaml
            chmod 0640 ${cfg.stateDir}/.cronus/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.cronus/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cronus/.managed
          chmod 0644 ${cfg.stateDir}/.cronus/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.cronus/.container-mode <<'CRONUS_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    cronus_bin=${containerDataDir}/current-package/bin/cronus
    CRONUS_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cronus/.container-mode
            chmod 0644 ${cfg.stateDir}/.cronus/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.cronus/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.cronus";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.cronus" ]; then
                  rm -f "${symlinkPath}"
                  echo "cronus-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.cronus -> stateDir/.cronus for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.cronus";
                target = "${cfg.stateDir}/.cronus";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "cronus-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.cronus/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.cronus/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.cronus/auth.json
              fi
            ''}
          ''}
          ...
          # Cronus reads $CRONUS_HOME/.env at startup via load_cronus_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.cronus/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'CRONUS_NIX_ENV_EOF'
    ${envFileContent}
    CRONUS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.cronus/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.cronus/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cronus/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.cronus-agent = {
          description = "Cronus Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            CRONUS_HOME = "${cfg.stateDir}/.cronus";
            CRONUS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $CRONUS_HOME/.env by the activation script. load_cronus_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/cronus"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the cronus group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.cronus-agent = {
          description = "Cronus Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              CRONUS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              CRONUS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env CRONUS_UID="$CRONUS_UID" \
                --env CRONUS_GID="$CRONUS_GID" \
                --env CRONUS_HOME=${containerDataDir}/.cronus \
                --env CRONUS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/cronus gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
