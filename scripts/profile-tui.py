#!/usr/bin/env python3
"""Drive the Cronus TUI under CRONUS_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.cronus/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with CRONUS_TUI_RESUME set so it
bypasses the cronus_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  CRONUS_PERF_LOG     (default ~/.cronus/perf.log)
  CRONUS_PERF_NODE    (default node from $PATH)
  CRONUS_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests CRONUS_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_cronus_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("CRONUS_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".cronus"

DEFAULT_TUI_DIR = Path(
    os.environ.get("CRONUS_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("CRONUS_PERF_LOG", str(get_cronus_home() / "perf.log")))
DEFAULT_STATE_DB = get_cronus_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — CRONUS_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["CRONUS_DEV_PERF"] = "1"
    env["CRONUS_DEV_PERF_MS"] = str(args.threshold_ms)
    env["CRONUS_DEV_PERF_LOG"] = str(log)
    env["CRONUS_TUI_RESUME"] = sid
    ...
    node = os.environ.get("CRONUS_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="CRONUS_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "cronus-ink" / "src"
