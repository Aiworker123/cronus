...
# Python-driven tool session (e.g. Cronus terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/cronus-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/cronus-agent.git"
CRONUS_HOME="${CRONUS_HOME:-$HOME/.cronus}"
...
#   code at /usr/local/lib/cronus-agent, command at /usr/local/bin/cronus,
#   data still at /root/.cronus (CRONUS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.cronus/cronus-agent"
            echo "                   default (root, Linux): /usr/local/lib/cronus-agent"
            echo "  --cronus-home PATH  Data directory (default: ~/.cronus, or \$CRONUS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Cronus installs the code under"
            echo "  /usr/local/lib/cronus-agent and links the command into"
            echo "  /usr/local/bin/cronus (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CRONUS_HOME"
            echo "  (default /root/.cronus).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CRONUS_HOME/cronus-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs cronus setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `cronus` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CRONUS_HOME/cronus-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CRONUS_HOME/cronus-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/cronus-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CRONUS_HOME/cronus-agent — then preserve it)
#
# Always no-op when the user set --dir or $CRONUS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CRONUS_HOME.
    if is_termux; then
        INSTALL_DIR="$CRONUS_HOME/cronus-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CRONUS_HOME/cronus-agent/.git" ]; then
            INSTALL_DIR="$CRONUS_HOME/cronus-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/cronus-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/cronus-agent"
        ...
        # /usr/local/bin/cronus wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/cronus"
        log_info "  Data:    $CRONUS_HOME (unchanged)"
...
get_cronus_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/cronus" ]; then
        echo "$link_dir/cronus"
    else
        echo "cronus"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/cronus-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/cronus-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/cronus-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$CRONUS_HOME/node/bin/node" ]; then
        export PATH="$CRONUS_HOME/node/bin:$PATH"
        local found_ver=$("$CRONUS_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Cronus-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.cronus/node/..."
    ...
    # Place into ~/.cronus/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$CRONUS_HOME/node"
    mkdir -p "$CRONUS_HOME"
    mv "$extracted_dir" "$CRONUS_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$CRONUS_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$CRONUS_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$CRONUS_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$CRONUS_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CRONUS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.cronus/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Cronus install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Cronus Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="cronus-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Cronus behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"cronus-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up cronus command..."

    if [ "$USE_VENV" = true ]; then
        CRONUS_BIN="$INSTALL_DIR/venv/bin/cronus"
    else
        CRONUS_BIN="$(which cronus 2>/dev/null || echo "")"
        if [ -z "$CRONUS_BIN" ]; then
            log_warn "cronus not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$CRONUS_BIN" ]; then
        log_warn "cronus entry point not found at $CRONUS_BIN"
    ...
    # Create a user-facing shim for the cronus command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CRONUS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CRONUS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/cronus"
    cat > "$command_link_dir/cronus" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CRONUS_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/cronus"
    log_success "Installed cronus launcher → $command_link_display_dir/cronus"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "cronus command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v cronus' \
                >/dev/null 2>&1; then
        ...
        log_info "cronus not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Cronus Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Cronus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Cronus Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so cronus works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.cronus directory structure (config at top level, code in subdir)
    mkdir -p "$CRONUS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.cronus/.env (top level, easy to find)
    if [ ! -f "$CRONUS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CRONUS_HOME/.env"
            log_success "Created ~/.cronus/.env from template"
        else
            touch "$CRONUS_HOME/.env"
            log_success "Created ~/.cronus/.env"
        fi
    else
        log_info "~/.cronus/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CRONUS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.cronus/config.yaml (top level, easy to find)
    if [ ! -f "$CRONUS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CRONUS_HOME/config.yaml"
            log_success "Created ~/.cronus/config.yaml from template"
        fi
    else
        log_info "~/.cronus/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$CRONUS_HOME/SOUL.md" ]; then
        cat > "$CRONUS_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.cronus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.cronus/"

    # Seed bundled skills into ~/.cronus/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.cronus/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.cronus/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CRONUS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$CRONUS_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.cronus/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$CRONUS_HOME/.env"
    ...
    {
        echo ""
        echo "# Cronus Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Cronus will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (cronus --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'cronus setup' after install."
        return 0
    fi
    ...
    # Run cronus setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m cronus_cli.main setup < /dev/tty
    else
        python -m cronus_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CRONUS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Cronus to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CRONUS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'cronus whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CRONUS_CMD="$(get_cronus_command_path)"
                $CRONUS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'cronus whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'cronus gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CRONUS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CRONUS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: cronus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: cronus gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CRONUS_CMD gateway > "$CRONUS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.cronus/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: cronus gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: cronus gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CRONUS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CRONUS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CRONUS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}cronus${NC}              Start chatting"
    echo -e "   ${GREEN}cronus setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}cronus config${NC}       View/edit configuration"
    echo -e "   ${GREEN}cronus config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}cronus gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}cronus update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'cronus' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'cronus' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'cronus' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CRONUS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CRONUS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CRONUS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$CRONUS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CRONUS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Cronus for pip install"
    ...
    CRONUS_CMD="$(command -v cronus 2>/dev/null || echo "")"
    if [ -n "$CRONUS_CMD" ]; then
        log_info "Running cronus setup..."
        "$CRONUS_CMD" setup
    else
        log_warn "cronus command not found on PATH"
        log_info "Try: python -m cronus_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$CRONUS_HOME/.install_method"
}
