...
VERSION_FILE = REPO_ROOT / "cronus_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "cronus@marian.local": "Schrotti77",
    ...
    "274096618+cronus-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "cronus@nousresearch.com": "NousResearch",
    ...
    "cronus@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (CRONUS_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (CRONUS_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: cronus-eval community link)
    ...
    "cronusagent26@gmail.com": "cronusagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+cronusagent26@users.noreply.github.com": "cronusagent26",
    ...
    "vigo@cronus": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.cronus@agentmail.to": "oseftg",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"cronus-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "cronus@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/cronus-agent",
    ...
    lines.append(f"# Cronus Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Cronus Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Cronus Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Cronus Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Cronus Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Cronus Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Cronus Agent v{new_version} ({calver_date})' "
