...
# Bootstrap Open WebUI against Cronus Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.cronus/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-cronus.sh
...
#   CRONUS_API_PORT=8642
#   CRONUS_API_HOST=127.0.0.1
#   CRONUS_API_MODEL_NAME='Cronus Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Cronus Agent WebUI}"
...
CRONUS_ENV_FILE="${CRONUS_ENV_FILE:-$HOME/.cronus/.env}"
CRONUS_API_PORT="${CRONUS_API_PORT:-8642}"
CRONUS_API_HOST="${CRONUS_API_HOST:-127.0.0.1}"
CRONUS_API_CONNECT_HOST="${CRONUS_API_CONNECT_HOST:-127.0.0.1}"
CRONUS_API_MODEL_NAME="${CRONUS_API_MODEL_NAME:-Cronus Agent}"
CRONUS_API_BASE_URL="http://${CRONUS_API_CONNECT_HOST}:${CRONUS_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-cronus.sh"
LOG_DIR="$HOME/.cronus/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$CRONUS_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$CRONUS_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.cronus.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.cronus</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.cronus"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.cronus"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-cronus.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Cronus Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-cronus.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.cronus/logs/openwebui.log
StandardError=append:%h/.cronus/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-cronus.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$CRONUS_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Cronus API server is configured...'
  upsert_env API_SERVER_ENABLED true "$CRONUS_ENV_FILE"
  upsert_env API_SERVER_HOST "$CRONUS_API_HOST" "$CRONUS_ENV_FILE"
  upsert_env API_SERVER_PORT "$CRONUS_API_PORT" "$CRONUS_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$CRONUS_API_MODEL_NAME" "$CRONUS_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$CRONUS_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Cronus gateway so API server settings take effect...'
  cronus gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${CRONUS_API_CONNECT_HOST}:${CRONUS_API_PORT}/health" >/dev/null; then
    log 'Cronus API server did not answer on the first check. Trying to start gateway in the background...'
    nohup cronus gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${CRONUS_API_CONNECT_HOST}:${CRONUS_API_PORT}/health" >/dev/null
  ...
  log "Cronus API endpoint: ${CRONUS_API_BASE_URL}"
