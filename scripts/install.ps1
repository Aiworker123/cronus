# ============================================================================
# Cronus Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$CronusHome = "$env:LOCALAPPDATA\cronus",
    [string]$InstallDir = "$env:LOCALAPPDATA\cronus\cronus-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/cronus-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/cronus-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Cronus Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $CronusHome)) {
        New-Item -ItemType Directory -Force -Path $CronusHome | Out-Null
    }
    $envFile = Join-Path $CronusHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $CronusHome "node"
...
function Install-Git {
    ...
    which Cronus uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\cronus\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Cronus needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Cronus copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\cronus\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CRONUS_GIT_BASH_PATH`` (User scope) so Cronus can find it in a fresh
    ...
    # Download PortableGit into $CronusHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $CronusHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Cronus features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Cronus needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CRONUS_GIT_BASH_PATH`` (User env scope) so Cronus can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $CronusHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $CronusHome\git\usr\bin\bash.exe
    $candidates += "$CronusHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$CronusHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CRONUS_GIT_BASH_PATH", $candidate, "User")
            $env:CRONUS_GIT_BASH_PATH = $candidate
            Write-Info "Set CRONUS_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Cronus may not find Git Bash."
    Write-Info "If needed, set CRONUS_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$CronusHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$CronusHome\node;$env:Path"
        Write-Success "Node.js $version found (Cronus-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $CronusHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $CronusHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\cronus-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$CronusHome\node") { Remove-Item -Recurse -Force "$CronusHome\node" }
                Move-Item $extractedDir.FullName "$CronusHome\node"
                ...
                $version = & "$CronusHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $CronusHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running cronus processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/cronus-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/cronus-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/cronus-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\cronus-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\cronus-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `cronus.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install cronus-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and cronus.exe broken with
    ...
    # here, not 30 seconds later when the user runs `cronus`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `cronus dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `cronus dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `cronus dashboard` should now work."
            } else {
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        $cronusBin = "$InstallDir"
    } else {
        $cronusBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so cronus is globally available
    # On Windows, the cronus.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$cronusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$cronusBin;$currentPath",
            "User"
        )
    ...
    # Set CRONUS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\cronus instead
    # of the Unix default ~/.cronus
    $currentCronusHome = [Environment]::GetEnvironmentVariable("CRONUS_HOME", "User")
    if (-not $currentCronusHome -or $currentCronusHome -ne $CronusHome) {
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.cronus directory structure
    New-Item -ItemType Directory -Force -Path "$CronusHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CronusHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$CronusHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.cronus/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.cronus/.env"
        }
    } else {
        Write-Info "~/.cronus/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$CronusHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.cronus/config.yaml from template"
        }
    } else {
        Write-Info "~/.cronus/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Cronus's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.cronus/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.cronus/"
    
    # Seed bundled skills into ~/.cronus/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.cronus/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.cronus/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.cronus/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'cronus setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\cronus-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\cronus-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\cronus-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.cronus/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'cronus setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m cronus_cli.main setup
    } else {
        python -m cronus_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $cronusCmd = "$InstallDir\venv\Scripts\cronus.exe"
    if (-not (Test-Path $cronusCmd)) {
        $cronusCmd = "cronus"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$CronusHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'cronus whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: cronus gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$CronusHome\logs\gateway.log"
            Start-Process -FilePath $cronusCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$CronusHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: cronus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: cronus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$CronusHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$CronusHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CronusHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CronusHome\cronus-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   cronus              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   cronus setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   cronus config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   cronus config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   cronus gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   cronus update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Cronus repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Cronus to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/cronus-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
