...
_CRONUS_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CRONUS_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `cronus tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CRONUS_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Cronus toolsets (CLI + messaging platforms)
    #
    ...
    "cronus-cli": {
        ...
        "tools": _CRONUS_CORE_TOOLS,
        "includes": []
    },

    "cronus-cron": {
        # Mirrors cronus-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `cronus tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as cronus-cli; gated by `cronus tools`",
        "tools": _CRONUS_CORE_TOOLS,
        "includes": []
    },
    ...
    "cronus-email": {
        "description": "Email bot toolset - interact with Cronus via email (IMAP/SMTP)",
        "tools": _CRONUS_CORE_TOOLS,
        "includes": []
    },
    ...
    "cronus-sms": {
        "description": "SMS bot toolset - interact with Cronus via SMS (Twilio)",
        "tools": _CRONUS_CORE_TOOLS,
        "includes": []
    },
    ...
    "cronus-webhook": {
        ...
        "tools": _CRONUS_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "cronus-gateway": {
        ...
        "tools": [],
        "includes": ["cronus-telegram", "cronus-discord", "cronus-whatsapp", "cronus-slack", "cronus-signal", "cronus-bluebubbles", "cronus-homeassistant", "cronus-email", "cronus-sms", "cronus-mattermost", "cronus-matrix", "cronus-dingtalk", "cronus-feishu", "cronus-wecom", "cronus-wecom-callback", "cronus-weixin", "cronus-qqbot", "cronus-webhook", "cronus-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (cronus-<name>).
        # Gives them _CRONUS_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("cronus-"):
            platform_name = name[len("cronus-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CRONUS_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
