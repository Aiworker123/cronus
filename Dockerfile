...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/cronus/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when cronus
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# cronus process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main cronus process, the dashboard,
...
# Non-root user for runtime; UID can be overridden via CRONUS_UID at runtime
RUN useradd -u 10000 -m -d /opt/data cronus
...
#
# ui-tui/packages/cronus-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/cronus-ink/ ui-tui/packages/cronus-ink/
...
# lock on the @cronus/ink entry, tripped the TUI launcher's
...
# ---------- Permissions ----------
# Make install dir world-readable so any CRONUS_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the cronus user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# cronus_cli/main.py succeeds (see #18800). /opt/cronus/web is build-time
# only (CRONUS_WEB_DIST points at cronus_cli/web_dist) and is intentionally
# not chowned here.
# The .venv MUST remain cronus-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/cronus && \
    chown -R cronus:cronus /opt/cronus/.venv /opt/cronus/ui-tui /opt/cronus/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the cronus user via
# `s6-setuidgid cronus` in its run script. If CRONUS_UID is unset, services
# run as the default cronus user (UID 10000).

# ---------- Link cronus-agent itself (editable) ----------
...
# container always returns nothing — meaning `cronus dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the CRONUS_GIT_SHA build-arg to
# /opt/cronus/.cronus_build_sha at build time, and have
# cronus_cli/build_info.py read it at runtime.  Both `cronus dump` and
...
ARG CRONUS_GIT_SHA=
RUN if [ -n "${CRONUS_GIT_SHA}" ]; then \
        printf '%s\n' "${CRONUS_GIT_SHA}" > /opt/cronus/.cronus_build_sha && \
        chown cronus:cronus /opt/cronus/.cronus_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-cronus + dashboard.
...
# `exec cronus`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $CRONUS_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/cronus/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-cronus-setup && \
    chmod +x /etc/cont-init.d/01-cronus-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV CRONUS_WEB_DIST=/opt/cronus/cronus_cli/web_dist
ENV CRONUS_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> cronus ...` they default to root, and any file the
# command writes under $CRONUS_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/cronus/bin/cronus, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid cronus`
# when invoked as root. Non-root callers (supervised processes,
# `--user cronus`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/cronus/.venv/bin/cronus). See the shim source for
# the opt-out env var (CRONUS_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/cronus-exec-shim.sh /opt/cronus/bin/cronus

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> cronus ...` and any
# subprocess that doesn't activate the venv first still find cronus.
#
# /opt/cronus/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/cronus/bin:/opt/cronus/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. cronus
# subcommand vs. no-args), drops to the cronus user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/cronus/docker/main-wrapper.sh" ]
CMD [ ]
