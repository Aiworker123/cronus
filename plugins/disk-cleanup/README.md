...
Auto-tracks and cleans up ephemeral files created during Cronus Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CRONUS_HOME` and `/tmp/cronus-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CRONUS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CRONUS_HOME` or `/tmp/cronus-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CRONUS_HOME/disk-cleanup/` is itself excluded
- `$CRONUS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
