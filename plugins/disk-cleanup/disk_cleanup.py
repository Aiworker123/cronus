"""disk_cleanup — ephemeral file cleanup for Cronus Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CRONUS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CRONUS_HOME and /tmp/cronus-*
Never touches: ~/.cronus/logs/ or any system directory.
"""
...
try:
    ...
    def get_cronus_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CRONUS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".cronus").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CRONUS_HOME/logs/``."""
    return get_cronus_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CRONUS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CRONUS_HOME or ``/tmp/cronus-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    cronus_home = get_cronus_home()
    try:
        path.resolve().relative_to(cronus_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/cronus-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("cronus-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CRONUS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CRONUS_HOME (but leave CRONUS_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "cronus-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(cronus_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == cronus_home:
                continue
            try:
                rel_parts = dirpath.relative_to(cronus_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    cronus_home = get_cronus_home()
    try:
        rel = path.resolve().relative_to(cronus_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "cronus-agent",
        }:
            return None
        ...
        # Path isn't under CRONUS_HOME (e.g. /tmp/cronus-*) — fall through.
        pass
