...
Handles: cronus honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "cronus" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Cronus profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'cronus honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'cronus honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `cronus update` -- no output, no exceptions.
    """
    try:
        from cronus_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Cronus profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CRONUS_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_cronus_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        cronus_host.get("pinUserPeer"),
        cronus_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in cronus_host:
        aliases_src = cronus_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in cronus_host:
        prefix_src = cronus_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(cronus_host: dict) -> None:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Cronus persistent cross-session memory.")
    ...
    cronus_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'cronus honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = cronus_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        cronus_host["peerName"] = new_peer

    current_ai = cronus_host.get("aiPeer") or cfg.get("aiPeer", "cronus")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        cronus_host["aiPeer"] = new_ai

    current_workspace = cronus_host.get("workspace") or cfg.get("workspace", "cronus")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        cronus_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, cronus_host)
    ...
    if current_shape == "single" and new_shape == "multi":
        peer_target = cronus_host.get("peerName") or current_peer or "user"
        print(
    ...
    if new_shape == "single":
        _scrub_identity_mapping(cronus_host)
        cronus_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{cronus_host.get('peerName', '?')}'.")
        ...
        if prior_aliases:
            cronus_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            cronus_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'cronus honcho status' to inspect.")
        ...
        if existing_aliases:
            cronus_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = cronus_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        cronus_host["observationMode"] = new_obs
    else:
        cronus_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(cronus_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        cronus_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        cronus_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = cronus_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        cronus_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = cronus_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        cronus_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                cronus_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(cronus_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            cronus_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        cronus_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        cronus_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        cronus_host["dialecticReasoningLevel"] = new_reasoning
    else:
        cronus_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = cronus_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        cronus_host["sessionStrategy"] = new_strat

    cronus_host["enabled"] = True
    cronus_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        cronus_config = load_config()
        cronus_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(cronus_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: cronus config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    cronus honcho status     -- show full config")
    print("    cronus honcho mode       -- change recall/observation mode")
    print("    cronus honcho tokens     -- tune context and dialectic budgets")
    print("    cronus honcho peer       -- update peer names")
    print("    cronus honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Cronus profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: cronus honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'cronus honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: cronus honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        cronus = hosts.get(_host_key(), {})
        user = cronus.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = cronus.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = cronus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = cronus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Cronus' identity in Honcho. Seed with 'cronus honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: cronus honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: cronus honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = cronus.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = cronus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = cronus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Cronus asks Honcho's AI peer a question")
        ...
        print("\n  Set with: cronus honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'cronus honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'cronus honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    cronus honcho identity --show        — show both peer representations")
        print("    cronus honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Cronus + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Cronus\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Cronus persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  cronus honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'cronus honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'cronus honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  cronus honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'cronus'")
        print("  with Honcho configured and no prior session history.")
        print("  (Cronus calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    cronus honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'cronus honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Cronus, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'cronus honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    cronus honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  cronus honcho identity <path/to/SOUL.md>")
    ...
    print("    Cronus:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Cronus:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Cronus:   Honcho observes every message and updates representations")
    ...
    print("    Cronus:   per-session by default — each run gets its own session")
    print("              Map a custom name:  cronus honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. cronus honcho setup              — configure API key (required)")
        print("  2. cronus honcho migrate            — re-run this walkthrough")
    else:
        print("  1. cronus honcho status             — verify Honcho connection")
        print("  2. cronus                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. cronus honcho identity --show    — verify AI peer representation")
        print("  4. cronus honcho tokens             — tune context and dialectic budgets")
        print("  5. cronus honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'cronus memory setup'...\n")
        from cronus_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``cronus honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``cronus honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to cronus memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Cronus Honcho",
    )
