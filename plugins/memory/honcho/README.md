...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/cronus>
...
```bash
cronus honcho setup    # full interactive wizard (cloud or local)
cronus memory setup    # generic picker, also works
```

Or manually:
```bash
cronus config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.cronus/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CRONUS_HOME/honcho.json` | Profile-local (isolated Cronus instances) |
| 2 | `~/.cronus/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Cronus profile: `cronus` (default) or `cronus.<profile>`.
...
**Deployment shapes** (`cronus honcho setup` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Cronus to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Cronus session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`cronus-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`cronus`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-cronus-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening cronus in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Cronus session ID (timestamp + hex). Every `cronus` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Cronus profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`cronus`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Cronus profile: `cronus` (default) or `cronus.<profile>` (e.g. `cronus -p coder` → host key `cronus.coder`).
...
| `CRONUS_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `cronus honcho setup` | Full interactive setup wizard |
| `cronus honcho status` | Show resolved config for active profile |
| `cronus honcho enable` / `disable` | Toggle Honcho for active profile |
| `cronus honcho mode <mode>` | Change recall or observation mode |
| `cronus honcho peer --user <name>` | Update user peer name |
| `cronus honcho peer --ai <name>` | Update AI peer name |
| `cronus honcho tokens --context <N>` | Set context token budget |
| `cronus honcho tokens --dialectic <N>` | Set dialectic max chars |
| `cronus honcho map <name>` | Map current directory to a session name |
| `cronus honcho sync` | Create host blocks for all Cronus profiles |
