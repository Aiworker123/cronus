...
def _load_supermemory_config(cronus_home: str) -> dict:
    config = _default_config()
    config_path = Path(cronus_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, cronus_home: str) -> None:
    config_path = Path(cronus_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `cronus memory setup`.
        # All other options are documented for $CRONUS_HOME/supermemory.json
    ...
    def save_config(self, values, cronus_home):
        ...
        _save_supermemory_config(sanitized, cronus_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from cronus_constants import get_cronus_home
        self._cronus_home = kwargs.get("cronus_home") or str(get_cronus_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._cronus_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "cronus", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "cronus_memory", "target": target, "type": "explicit_memory"},
