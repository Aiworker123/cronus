...
Config file: `$CRONUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `cronus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `cronus-{identity}` → `cronus-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Cronus profile:

```json
{
  "container_tag": "cronus-{identity}"
}
```

For a profile named `coder`, this resolves to `cronus-coder`. The default profile resolves to `cronus-default`. Without `{identity}`, all profiles share the same container.
