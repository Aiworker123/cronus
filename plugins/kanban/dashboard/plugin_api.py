...
``cronus_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__CRONUS_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``cronus_cli/web_server.py``.

This means ``cronus dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from cronus_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from cronus_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``cronus_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from cronus_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from cronus_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``CRONUS_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from cronus_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``cronus kanban
...
# spawn failures, stuck-blocked). See cronus_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``cronus kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``cronus kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``cronus kanban specify <task_id>``.
    ...
    try:
        os.environ["CRONUS_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from cronus_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``cronus kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.cronus/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Cronus profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.cronus/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from cronus_constants import get_cronus_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_cronus_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``cronus profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from cronus_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``cronus kanban decompose <task_id>``.
    ...
    try:
        os.environ["CRONUS_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from cronus_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.cronus/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in cronus_cli/web_server.py.
