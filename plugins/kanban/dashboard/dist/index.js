/**
 * Cronus Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.cronus/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CRONUS_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CRONUS_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "cronus-kanban-dot-triage",
    todo: "cronus-kanban-dot-todo",
    ready: "cronus-kanban-dot-ready",
    running: "cronus-kanban-dot-running",
    blocked: "cronus-kanban-dot-blocked",
    done: "cronus-kanban-dot-done",
    archived: "cronus-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-cronus-task";
  ...
  const DOCS_URL = "https://cronus-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://cronus-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "cronus.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="cronus-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "cronus-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "cronus-kanban-md",
  ...
  // standard `drop` event and our `cronus-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("cronus-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("cronus-kanban-column--drop");
          if (target) target.classList.add("cronus-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("cronus-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("cronus-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("cronus-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__CRONUS_SESSION_TOKEN__ || "";
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "cronus-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "cronus-kanban-attention",
        "cronus-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "cronus-kanban-attention-bar" },
        h("span", { className: "cronus-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "cronus-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "cronus-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "cronus-kanban-attention-row",
                  "cronus-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "cronus-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "cronus-kanban-attention-row-id" }, task.id),
                h("span", { className: "cronus-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "cronus-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "cronus-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "cronus-kanban-diag-action-btn",
      action.suggested ? "cronus-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " cronus-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".cronus-kanban-drawer-comment-row input, .cronus-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "cronus-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("cronus-kanban-diag", sevClass) },
      h("div", { className: "cronus-kanban-diag-header" },
        h("span", { className: "cronus-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "cronus-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "cronus-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "cronus-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "cronus-kanban-diag-data-row" },
                  h("span", { className: "cronus-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "cronus-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "cronus-kanban-diag-data-row" },
                h("span", { className: "cronus-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "cronus-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "cronus-kanban-diag-reassign-row" },
            h("span", { className: "cronus-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "cronus-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "cronus-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "cronus-kanban-diag-msg",
              msg.ok ? "cronus-kanban-diag-msg--ok" : "cronus-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "cronus-kanban-section" },
      h("div", { className: "cronus-kanban-section-head-row" },
        h("span", { className: "cronus-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "cronus-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "cronus-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "cronus-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "cronus-kanban-docs-link",
      title: "Open Cronus Kanban docs in a new tab",
      "aria-label": "Cronus Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `cronus kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "cronus-kanban-msg-ok" : "cronus-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "cronus-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "cronus-kanban-boardswitcher" },
      h("div", { className: "cronus-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "cronus-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "cronus-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "cronus-kanban-dialog-title" },
        ...
        h("div", { className: "cronus-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Cronus profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "cronus-kanban-bulk" },
      h("span", { className: "cronus-kanban-bulk-count" },
      ...
      h("div", { className: "cronus-kanban-bulk-priority",
      ...
      h("div", { className: "cronus-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Cronus profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "cronus-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cronus-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("cronus-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "cronus-kanban-trash",
        dragOver ? "cronus-kanban-trash--drop" : "",
        props.draggingTaskId ? "cronus-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "cronus-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "cronus-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".cronus-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "cronus-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cronus-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("cronus-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "cronus-kanban-column",
        dragOver ? "cronus-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "cronus-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "cronus-kanban-col-check",
        ...
        h("span", { className: cn("cronus-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "cronus-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "cronus-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "cronus-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "cronus-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "cronus-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "cronus-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "cronus-kanban-lane" },
                  h("div", { className: "cronus-kanban-lane-head" },
                    h("span", { className: "cronus-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "cronus-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "cronus-kanban-card--stale-red";
    if (age >= tier.amber) return "cronus-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".cronus-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "cronus-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "cronus-kanban-card",
        props.selected ? "cronus-kanban-card--selected" : "",
        props.failed ? "cronus-kanban-card--failed" : "",
        props.draggingSource ? "cronus-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "cronus-kanban-card-content" },
          h("div", { className: "cronus-kanban-card-row" },
            h("label", {
              className: "cronus-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "cronus-kanban-card-check",
            ...
            h("span", { className: "cronus-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or cronus kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "cronus-kanban-warning-badge",
                    "cronus-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "cronus-kanban-needs-assignee",
          ...
          h("div", { className: "cronus-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "cronus-kanban-card-row cronus-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "cronus-kanban-assignee",
                            title: `Assigned to Cronus profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "cronus-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "cronus-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "cronus-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "cronus-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Cronus profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Cronus profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "cronus-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "cronus-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "cronus-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "cronus-kanban-drawer-close",
        ...
        data ? h("div", { className: "cronus-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "cronus-kanban-drawer-body" },
      h("div", { className: "cronus-kanban-drawer-title" },
        h("span", { className: cn("cronus-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "cronus-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "cronus-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "cronus-kanban-section" },
        h("div", { className: "cronus-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "cronus-kanban-section" },
        h("div", { className: "cronus-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "cronus-kanban-comment" },
                h("div", { className: "cronus-kanban-comment-head" },
                  h("span", { className: "cronus-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "cronus-kanban-comment-ago" },
      ...
      h("div", { className: "cronus-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "cronus-kanban-event",
              isDiag ? "cronus-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "cronus-kanban-event-header" },
                  h("span", { className: "cronus-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "cronus-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "cronus-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "cronus-kanban-event-header-plain" },
                  h("span", { className: "cronus-kanban-event-kind" }, e.kind),
                  h("span", { className: "cronus-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "cronus-kanban-event-phantom-row" },
                  h("span", { className: "cronus-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "cronus-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "cronus-kanban-event-payload" },
    ...
    return h("div", { className: "cronus-kanban-section" },
      h("div", { className: "cronus-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "cronus-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `cronus-kanban-run--${r.outcome || r.status || "ended"}`
          : "cronus-kanban-run--active";
        return h("div", { key: r.id, className: cn("cronus-kanban-run", outcomeClass) },
          h("div", { className: "cronus-kanban-run-head" },
            h("span", { className: "cronus-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "cronus-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "cronus-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "cronus-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "cronus-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "cronus-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "cronus-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "cronus-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "cronus-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "cronus-kanban-pre cronus-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "cronus-kanban-meta-row" },
      h("span", { className: "cronus-kanban-meta-label" }, props.label),
      h("span", { className: "cronus-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "cronus-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cronus-kanban-meta-row" },
        h("span", { className: "cronus-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "cronus-kanban-meta-value cronus-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cronus-kanban-meta-row" },
        h("span", { className: "cronus-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "cronus-kanban-meta-value cronus-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "cronus-kanban-section" },
      h("div", { className: "cronus-kanban-section-head-row" },
        h("span", { className: "cronus-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "cronus-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "cronus-kanban-section" },
      h("div", { className: "cronus-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "cronus-kanban-deps-row" },
        h("span", { className: "cronus-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "cronus-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "cronus-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "cronus-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "cronus-kanban-dep-chip-x",
      ...
      h("div", { className: "cronus-kanban-deps-row" },
        h("span", { className: "cronus-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "cronus-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "cronus-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "cronus-kanban-msg-ok"
          : "cronus-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "cronus-kanban-section" },
      ...
      h("div", { className: "cronus-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "cronus-kanban-home-sub cronus-kanban-home-sub--on"
              : "cronus-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CRONUS_PLUGINS__ && typeof window.__CRONUS_PLUGINS__.register === "function") {
    window.__CRONUS_PLUGINS__.register("kanban", KanbanPage);
  }
})();
