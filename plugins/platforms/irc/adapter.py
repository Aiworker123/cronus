"""
IRC Platform Adapter for Cronus Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Cronus agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "cronus-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Cronus Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Cronus Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: cronus_, cronus_1, cronus_2...
...
def check_requirements() -> bool:
    ...
    # cronus setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `cronus gateway setup` flow for the IRC platform.

    Lazy-imports ``cronus_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Cronus to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #cronus — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.cronus/.env")
    print_info("Restart the gateway for changes to take effect: cronus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "cronus-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "cronus-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Cronus Agent (cron)")
