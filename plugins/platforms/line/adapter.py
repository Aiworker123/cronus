"""
LINE Messaging API platform adapter for Cronus Agent.
...
to Cronus Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CRONUS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            cronus_home = Path.home().joinpath(".cronus").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``cronus status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``cronus status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cronus setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.cronus/.env`` via ``cronus_cli.config``.
    """
    print()
    ...
    try:
        from cronus_cli.config import get_env_var, set_env_var
    except ImportError:
        print("cronus_cli.config not available; set LINE_* vars manually in ~/.cronus/.env")
        return
