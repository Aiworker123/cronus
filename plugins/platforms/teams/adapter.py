"""
Microsoft Teams platform adapter for Cronus Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Cronus"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        cronus_action = data.get("cronus_action", "")
        session_key = data.get("session_key", "")

        if not cronus_action or not session_key:
        ...
        choice = choice_map.get(cronus_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="cronus_approve",
                    data={**btn_data_base, "cronus_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="cronus_approve",
                    data={**btn_data_base, "cronus_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="cronus_approve",
                    data={**btn_data_base, "cronus_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="cronus_approve",
                    data={**btn_data_base, "cronus_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Cronus\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.cronus/.env")
