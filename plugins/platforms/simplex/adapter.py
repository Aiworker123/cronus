"""SimpleX Chat platform adapter (Cronus plugin).
...
This adapter ships as a Cronus platform plugin under
``plugins/platforms/simplex/``. The Cronus plugin loader scans the
...
discoverable and `cronus setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Cronus — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"cronus-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cronus setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.cronus/.env`` via ``cronus_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("cronus_cli.config not available; set SIMPLEX_* vars manually in ~/.cronus/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `cronus gateway status` without
