...
# ``model_tools`` import time, so ``cronus status``, ``cronus chat``, etc.
...
# ``cronus_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Cronus is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a cronus MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CRONUS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _cronus_home = _get_cronus_home()
        except (ModuleNotFoundError, ImportError):
            _cronus_home = _Path.home() / ".cronus"
        self._thread_count_store = _ThreadCountStore(
            _cronus_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Cronus is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CRONUS_HOME", str(_Path.home() / ".cronus"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CRONUS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a cronus MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Cronus is thinking…' marker message.
        ...
        "Cronus is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Cronus is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Cronus is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Cronus is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Cronus is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``cronus setup``.

    The setup wizard at ``cronus_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.cronus/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. cronus-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.cronus/.env")
    print_info("Restart the gateway: cronus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``cronus cron`` running as a
    separate process from ``cronus gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Cronus plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cronus-agent[google_chat]'",
        ...
        # the "Cronus is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Cronus is thinking…' marker message "
