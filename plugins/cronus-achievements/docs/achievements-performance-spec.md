# Cronus Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: cronus-achievements plugin
...
### `GET /api/plugins/cronus-achievements/achievements`
...
### `POST /api/plugins/cronus-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/cronus-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/cronus-achievements/overview`
...
- Persist plugin data under:
  - `~/.cronus/plugins/cronus-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.cronus/plugins/cronus-achievements/scan_checkpoint.json`
...
- `~/.cronus/plugins/cronus-achievements/`
