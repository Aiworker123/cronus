...
gateway (which Cronus uses to bill Browser Use sessions to a Nous
...
Auth env vars (one of)::
    ...
    # OR a managed Nous gateway entry (configured via 'cronus setup')
"""
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    def _get_config(self) -> Dict[str, Any]:
        ...
        if config is None:
            raise ValueError(
                ...
                "Set BROWSER_USE_API_KEY and restart."
            )
        return config
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Cronus only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"cronus_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
