...
``{title, url, description, position}`` rows every other Cronus web
provider produces.
...
prefers Cronus-managed xAI Grok OAuth (via ``cronus auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.cronus/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``cronus tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `cronus auth` to sign in with "
