...
session-scoped state file under ``$CRONUS_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CRONUS_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_cronus_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this cronus install, leave it first —
    ...
    env = os.environ.copy()
    env["CRONUS_MEET_URL"] = url
    env["CRONUS_MEET_OUT_DIR"] = str(out)
    env["CRONUS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CRONUS_MEET_HEADED"] = "1"
    if auth_state:
        env["CRONUS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CRONUS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CRONUS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CRONUS_MEET_MODE"] = mode
    if realtime_model:
        env["CRONUS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CRONUS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CRONUS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["CRONUS_MEET_REALTIME_KEY"] = realtime_api_key
