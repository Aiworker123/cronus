"""`cronus meet node ...` subcommand tree.

Wired into the existing ``cronus meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``cronus meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="cronus-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             cronus meet node approve <name> ws://<host>:{args.port} {token}")
        try:
