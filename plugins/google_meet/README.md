...
Let the cronus agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where cronus runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `cronus meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `cronus meet` CLI |
...
| `cli.py` | `cronus meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CRONUS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `cronus meet node {run,list,approve,remove,status,ping}` |
...
cronus meet install                                      # pip + Chromium
cronus meet setup                                        # preflight
cronus meet auth                                         # optional
cronus meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
cronus meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.cronus/.env
cronus meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
cronus meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
cronus meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.cronus/.env
cronus meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, cronus will **not** switch your system audio input automatically — the
...
cronus plugins enable google_meet
cronus meet node run --display-name my-mac --host 0.0.0.0 --port 18789
