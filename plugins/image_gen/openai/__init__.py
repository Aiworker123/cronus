...
three virtual model IDs so the ``cronus tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `cronus tools` → Image "
                    "Generation → OpenAI to configure, or `cronus setup` "
                    "to add the key."
                ),
