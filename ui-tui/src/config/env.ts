import type { MouseTrackingMode } from '@cronus/ink'
...
export const STARTUP_RESUME_ID = (process.env.CRONUS_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.CRONUS_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.CRONUS_TUI_IMAGE ?? '').trim()
...
//
// - CRONUS_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - CRONUS_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.CRONUS_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.CRONUS_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.CRONUS_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.CRONUS_TUI_INLINE)
...
// less fragile. Override explicitly with CRONUS_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.CRONUS_TUI_FPS)
