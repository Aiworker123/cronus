...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CRONUS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Cronus on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CRONUS_TUI_TRUECOLOR=1.
