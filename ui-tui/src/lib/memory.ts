...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.CRONUS_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.cronus', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `cronus-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
