...
/**
 * Return true when Cronus should enable Termux-focused TUI defaults.
 *
 * Defaults to on in Termux, with an explicit opt-out for debugging:
 *   CRONUS_TUI_TERMUX_MODE=0
 */
export const isTermuxTuiMode = (env: NodeJS.ProcessEnv = process.env): boolean => {
  ...
  const override = String(env.CRONUS_TUI_TERMUX_MODE ?? '').trim().toLowerCase()
