import type { RunExternalProcess } from '@cronus/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`cronus ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching cronus: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`cronus ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
