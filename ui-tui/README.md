...
React + Ink terminal UI for Cronus. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CRONUS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.cronus/.cronus_history` or under `CRONUS_HOME`.
...
```text
ui-tui/
  packages/cronus-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      cronus-ink.d.ts    type declarations for @cronus/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CronusCLI subprocess for slash commands
```
