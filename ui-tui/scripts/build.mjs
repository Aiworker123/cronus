...
await build({
  ...
  // Skip the prebuilt @cronus/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@cronus/ink': resolve(root, 'packages/cronus-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The cronus_cli launcher
