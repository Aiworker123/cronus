#!/usr/bin/env python3
"""
Cronus Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Cronus Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills cronus-agent-dev,github-auth
...
os.environ["CRONUS_QUIET"] = "1"  # Our own modules
...
try:
    from cronus_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from cronus_cli.banner import _format_context_length, format_banner_version_label
...
from cronus_cli.browser_connect import (
...
load_cronus_dotenv(cronus_home=_cronus_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.cronus/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If CRONUS_IGNORE_USER_CONFIG=1 is set (via ``cronus chat --ignore-user-config``),
    the user config at ``~/.cronus/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({CRONUS_HOME}/config.yaml)
    user_config_path = _cronus_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("CRONUS_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Cronus, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Cronus - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see cronus_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from cronus_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and CronusCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (cronus_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && cronus` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _CRONUS_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_CRONUS_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["CRONUS_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.cronus/logs/.
...
try:
    from cronus_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``cronus --help`` paths inside the chat command flow).  See
...
from cronus_cli.banner import build_welcome_banner
from cronus_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from cronus_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("CRONUS_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("CRONUS_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run cronus -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"cronus-{short_id}"
    branch_name = f"cronus/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`cronus_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from cronus_constants import get_cronus_home as _get_cronus_home
        _cronus_home_maint = _get_cronus_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_cronus_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`cronus_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``cronus/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("cronus-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``cronus/cronus-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``cronus -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("cronus/cronus-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. CRONUS_LIGHT / CRONUS_TUI_LIGHT env (true/false) — explicit override
#   2. CRONUS_TUI_THEME=light|dark — explicit theme
#   3. CRONUS_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Cronus assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("CRONUS_LIGHT", "CRONUS_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("CRONUS_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("CRONUS_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from cronus_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_cronus_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._cronus_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from cronus_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Cronus output it's common to display cron schedules verbatim.
    #
...
from cronus_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_cronus_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_cronus_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._cronus_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_cronus_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._cronus_bp_start = now
        ...
        _vt100_mod._cronus_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``CronusCLI._busy_command()``.
        """
        yield self

# ASCII Art - CRONUS-AGENT logo (full width, single line - requires ~95 char terminal)
CRONUS_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Cronus Caduceus (compact, fits in left panel)
CRONUS_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS CRONUS - AI Agent Framework"
        tiny_line = "⚕ NOUS CRONUS"
    else:
        agent_name = _skin.get_branding("agent_name", "Cronus Agent") if _skin else "Cronus Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("CRONUS_FAST_STARTUP_BANNER") == "1":
        from cronus_cli import __release_date__ as _release_date
        from cronus_cli import __version__ as _version

        version_line = f"Cronus Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from cronus_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.cronus/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.cronus/config.yaml on first use)
...
class CronusCLI:
    """
    Interactive CLI for the Cronus Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from cronus_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("CRONUS_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("CRONUS_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("CRONUS_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `cronus chat --ignore-rules` in cronus_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("CRONUS_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("CRONUS_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Cronus processes for this CRONUS_HOME.
        ...
        self._history_file = _cronus_home / ".cronus_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from cronus_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("CRONUS_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Cronus'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from cronus_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from cronus_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from cronus_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from cronus_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Cronus")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - CronusCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: cronus setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: cronus setup")
            return False
        ...
        # `cronus chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `cronus auth add openai-codex`
        # without `cronus model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (cronus sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (cronus sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``cronus doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``cronus doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Cronus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Cronus LLM (not agentic)
        from cronus_cli.model_switch import is_nous_cronus_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_cronus_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Cronus 3 & 4 models are NOT agentic and are not "
                "designed for use with Cronus Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Cronus: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Cronus: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.cronus/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from cronus_cli.clipboard import save_clipboard_image

        img_dir = get_cronus_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: cronus --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Cronus config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_cronus_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from cronus_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run cronus chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'cronus setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from cronus_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from cronus_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Cronus!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in cronus_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from cronus_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("CRONUS_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `cronus gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `cronus sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from cronus_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `cronus sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.cronus/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``cronus --resume <id>``
        ...
        saved_dir = get_cronus_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"cronus_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: cronus --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from cronus_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `cronus model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from cronus_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from cronus_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cronus_cli.model_switch import switch_model, parse_model_flags
        ...
        # clobber disk config.
        from cronus_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Cronus default (chat_completions)
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from cronus_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to cronus_cli.curator so the CLI and the `cronus curator`
        ...
        try:
            from cronus_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to cronus_cli.skills_hub."""
        from cronus_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_cronus_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_cronus_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in cronus_cli/commands.py instead of touching every dispatch site.
        from cronus_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from cronus_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from cronus_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_cronus_home()}/plugins/ to get started.")
                else:
            ...
            # Check for plugin-registered slash commands
                ...
                from cronus_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from cronus_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``cronus bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: cronus bundles create "
        ...
        _cprint(
            ...
            f"Manage with `cronus bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Cronus-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Cronus keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from cronus_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_cronus_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from cronus_cli.config import load_config
        from cronus_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("CRONUS_YOLO_MODE"))
        if current:
            os.environ.pop("CRONUS_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Cronus is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Cronus is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Cronus is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from cronus_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from cronus_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Cronus Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``cronus update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Cronus Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Cronus Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Cronus Agent",
            detail="This will exit the current session and run `cronus update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by cronus_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from cronus_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.cronus/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "cronus_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _cronus_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard CRONUS_HOME, so no -p needed.
            try:
                from cronus_cli.profiles import get_active_profile_name
            ...
            print(f"  cronus --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  cronus -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from cronus_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from cronus_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from cronus_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Cronus Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Cronus Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(CRONUS_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Cronus migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from cronus_cli.config import get_config_path as _get_cfg_path_resid
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Cronus binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from cronus_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Cronus Agent")
        ...
        # Dynamic prompt: shows Cronus symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Cronus needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Cronus needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Cronus needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = CronusCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_cronus_osd_patched", False):
                ...
                _pt_renderer._cronus_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``CRONUS_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("CRONUS_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the cronus process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: cronus setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                if getattr(self, '_delete_session_on_exit', False):
                    try:
                        from cronus_constants import get_cronus_home as _ghh
...
def main(
    ...
    """
    Cronus Agent CLI - Interactive AI Assistant
    ...
    os.environ["CRONUS_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Cronus Gateway (messaging platforms)...")
    ...
    # Default to cronus-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside CronusCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  CRONUS_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Cronus" box, no tool-gen
            ...
            # Single-query mode (`cronus chat -q "…"`): skip the welcome
