...
Uses a file-based lock (~/.cronus/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `cronus update` reloads
# the module) fail with ModuleNotFoundError for cronus_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from cronus_constants import get_cronus_home
from cronus_cli._subprocess_compat import windows_hide_flags
from cronus_cli.config import load_config, _expand_env_vars
from cronus_time import now as _cronus_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``cronus tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from cronus_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_cronus_home: Path | None = None


def _get_cronus_home() -> Path:
    """Resolve Cronus home dynamically while preserving test monkeypatch hooks."""
    return _cronus_home or get_cronus_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    cronus_home = _get_cronus_home()
    lock_dir = cronus_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Cronus profile.
    ...
    the scheduler's test/override hook and a context-local Cronus home override
    both point at the resolved profile directory so _get_cronus_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_cronus_home() callers agree on the same home.
    ...
    global _cronus_home
    prior_override = _cronus_home
    env_snapshot = os.environ.copy()

    from cronus_cli.profiles import normalize_profile_name, resolve_profile_env
    from cronus_constants import reset_cronus_home_override, set_cronus_home_override
    ...
    override_token = None
    try:
        override_token = set_cronus_home_override(profile_home)
        _cronus_home = profile_home
        logger.info(
            "Job '%s': using Cronus profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _cronus_home = prior_override
        if override_token is not None:
            reset_cronus_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("CRONUS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid CRONUS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within CRONUS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against CRONUS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_cronus_home() / "scripts"
    ...
    # escape — scripts MUST reside within CRONUS_HOME/scripts/.
    try:
    ...
    run_env["CRONUS_HOME"] = str(_get_cronus_home())
    try:
        from cronus_constants import get_subprocess_home
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _cronus_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_cronus_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_cronus_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["CRONUS_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed CRONUS_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads CRONUS_SESSION_PLATFORM
    #     and CRONUS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via CRONUS_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     CRONUS_SESSION_PLATFORM == "telegram".
    ...
    #     CRONUS_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read CRONUS_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the CRONUS_CRON_AUTO_DELIVER_* vars set
    # below, so clearing CRONUS_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "CRONUS_CRON_AUTO_DELIVER_PLATFORM",
        "CRONUS_CRON_AUTO_DELIVER_CHAT_ID",
        "CRONUS_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_cronus_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_cronus_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["CRONUS_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["CRONUS_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["CRONUS_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("CRONUS_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_cronus_home() / "config.yaml")
        ...
        prefill_file = os.getenv("CRONUS_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_cronus_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject CRONUS_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # CRONUS_HOME. When a workdir is configured, also inject project
        ...
        # override via CRONUS_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("CRONUS_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid CRONUS_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_cronus_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _cronus_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _cronus_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set CRONUS_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("CRONUS_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid CRONUS_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # Cronus home override, scheduler _cronus_home hook, and temporary
