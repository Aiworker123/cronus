"""Centralized logging setup for Cronus Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.cronus/logs/`` (profile-aware via ``get_cronus_home()``).
...
from cronus_constants import get_config_path, get_cronus_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_cronus_session_injector", False):
    ...
    _session_record_factory._cronus_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``cronus logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "cronus_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Cronus logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    cronus_home
        Override for the Cronus home directory.  Falls back to
        ``get_cronus_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_cronus_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._cronus_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the cronus group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from cronus_cli.config import is_managed
