"""ACP auth helpers — detect and advertise Cronus authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "cronus-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Cronus runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Cronus can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Cronus.
    ...
    not have Cronus provider credentials configured yet, so Cronus always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Cronus using the currently configured "
