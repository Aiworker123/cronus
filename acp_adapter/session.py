"""ACP session manager — maps ACP sessions to Cronus AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.cronus/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Cronus itself is running in WSL.

    Windows ACP clients can launch ``cronus acp`` inside WSL while still sending
    ...
    """
    from cronus_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Cronus from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["cronus-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Cronus agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Cronus AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Cronus runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.cronus/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``CRONUS_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_cronus_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=cronus_home / "state.db")
            return self._db_instance
        except Exception:
