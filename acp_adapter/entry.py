"""CLI entry point for the cronus-agent ACP adapter.

Loads environment variables from ``~/.cronus/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from CRONUS_HOME (default ``~/.cronus``)."""
    from cronus_cli.env_loader import load_cronus_dotenv

    cronus_home = get_cronus_home()
    loaded = load_cronus_dotenv(cronus_home=cronus_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", cronus_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="cronus-acp",
        description="Run Cronus Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Cronus version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(cronus_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import CronusACPAgent  # noqa: F401

    print("Cronus ACP check OK")


def _run_setup() -> None:
    from cronus_cli.main import main as cronus_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "cronus", "model"]
        cronus_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``cronus postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting cronus-agent ACP adapter")
    ...
    import acp
    from .server import CronusACPAgent
