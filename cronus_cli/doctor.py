"""
Doctor command for cronus CLI.

Diagnoses issues with Cronus Agent setup.
"""
...
from cronus_cli.config import get_project_root, get_cronus_home, get_env_path
...
_DHH = display_cronus_home()  # user-facing display path (e.g. ~/.cronus or ~/.cronus/profiles/coder)

# Load environment variables from ~/.cronus/.env so API key checks work
_env_path = get_env_path()
load_cronus_dotenv(cronus_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from cronus_cli.colors import Colors, color
from cronus_cli.models import _CRONUS_USER_AGENT
...
from cronus_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.cronus/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("CRONUS_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("CRONUS_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from cronus_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from cronus_cli.auth import get_minimax_oauth_auth_status
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-cronus and dashboard static services are up
    ...
    for static in ("main-cronus", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `cronus profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from cronus_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `cronus`.
    os.environ.setdefault("CRONUS_INTERACTIVE", "1")

    # Handle `cronus doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.cronus/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Cronus Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`cronus doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.cronus/.env (primary location for user config)
    env_path = CRONUS_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'cronus setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'cronus setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'cronus setup' to create one")
                issues.append("Run 'cronus setup' to create .env")
    
    # Check ~/.cronus/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = CRONUS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from cronus_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from cronus_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'cronus config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from cronus_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.cronus/.env or run 'cronus setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'cronus setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'cronus config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from cronus_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from cronus_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'cronus setup' to migrate config")
                else:
                    issues.append("Run 'cronus doctor --fix' or 'cronus setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'cronus doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Cronus' own device-code flow — the Codex CLI is
    ...
    cronus_home = CRONUS_HOME
    if cronus_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        cronus_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = cronus_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Cronus a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Cronus Agent Persona\n\n"
                "<!-- Edit this file to customize how Cronus communicates. -->\n\n"
                "You are Cronus, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = cronus_home / "memories"
    ...
    state_db_path = cronus_home / "state.db"
    ...
    wal_path = cronus_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'cronus doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "cronus"
        ...
        _cmd_link = _cmd_link_dir / "cronus"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(cronus not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/cronus → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/cronus points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/cronus → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/cronus — run 'cronus doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/cronus exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/cronus not found",
                    "(cronus command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/cronus → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/cronus symlink — run 'cronus doctor --fix'")
    ...
    try:
        from cronus_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `cronus doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'cronus config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``cronus doctor``.
    ...
    # never the legitimate source for `cronus doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'cronus setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = CRONUS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: cronus skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = CRONUS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: cronus memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: cronus memory setup",
                    "No Honcho API key — run 'cronus memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run cronus memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: cronus memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: cronus memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from cronus_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "cronus -p" in content:
                            _m = _re.search(r"cronus -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'cronus doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
