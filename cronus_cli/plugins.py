"""
Cronus Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with cronus-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.cronus/plugins/<name>/``
3. **Project plugins** – ``./.cronus/plugins/<name>/`` (opt-in via
   ``CRONUS_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``cronus_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``CRONUS_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("CRONUS_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``CRONUS_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.cronus/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("CRONUS_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When CRONUS_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Cronus loggers.
    """
    ...
    logger.debug(
        "CRONUS_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "cronus_agent.plugins"

_NS_PARENT = "cronus_plugins"
...
class PluginManifest:
    ...
    #              in ~/.cronus/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``cronus plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``cronus honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``cronus <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`cronus_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from cronus_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``cronus model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: cronus_cli.main imports plugins indirectly
        from cronus_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.cronus/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to cronus_cli/. Two layouts are
        ...
        # 2. User plugins (~/.cronus/plugins/)
        user_dir = get_cronus_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.cronus/plugins/)
        if _env_enabled("CRONUS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".cronus" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set CRONUS_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with cronus and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Cronus ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `cronus plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``cronus_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``cronus_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="cronus-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``cronus tools`` TUI so plugin-provided toolsets appear
