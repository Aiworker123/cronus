#!/usr/bin/env python3
"""
Cronus CLI - Main entry point.

Usage:
    cronus                     # Interactive chat (default)
    cronus chat                # Interactive chat
    cronus gateway             # Run gateway in foreground
    cronus gateway start       # Start gateway as service
    cronus gateway stop        # Stop gateway service
    cronus gateway status      # Show gateway status
    cronus gateway install     # Install gateway service
    cronus gateway uninstall   # Uninstall gateway service
    cronus setup               # Interactive setup wizard
    cronus logout              # Clear stored authentication
    cronus status              # Show status of all components
    cronus cron                # Manage cron jobs
    cronus cron list           # List cron jobs
    cronus cron status         # Check if cron scheduler is running
    cronus doctor              # Check configuration and dependencies
    cronus honcho setup                    # Configure Honcho AI memory integration
    cronus honcho status                   # Show Honcho config and connection status
    cronus honcho sessions                 # List directory → session name mappings
    cronus honcho map <name>               # Map current directory to a session name
    cronus honcho peer                     # Show peer names and dialectic settings
    cronus honcho peer --user NAME         # Set user peer name
    cronus honcho peer --ai NAME           # Set AI peer name
    cronus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    cronus honcho mode                     # Show current memory mode
    cronus honcho mode [hybrid|honcho|local]  # Set memory mode
    cronus honcho tokens                   # Show token budget settings
    cronus honcho tokens --context N       # Set session.context() token cap
    cronus honcho tokens --dialectic N     # Set dialectic result char cap
    cronus honcho identity                 # Show AI peer identity representation
    cronus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    cronus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Cronus + Honcho
    cronus version             Show version
    cronus update              Update to latest version
    cronus uninstall           Uninstall Cronus Agent
    cronus acp                 Run as an ACP server for editor integration
    cronus sessions browse     Interactive session picker with search

    cronus claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: cronus_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``cronus_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``cronus update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``cronus_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, cronus crashes on import and the user can't run
# ``cronus update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import cronus_bootstrap  # noqa: F401
...
# `entry.tsx`; this is just the earlier cousin. ``CRONUS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("CRONUS_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("CRONUS_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`cronus --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from cronus_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Cronus Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``cronus --version`` before config/logging imports on Termux."""
    if os.environ.get("CRONUS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to CRONUS_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (cronus tools, cronus setup, cronus model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'cronus {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any cronus module import.
#
# Many modules cache CRONUS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CRONUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.cronus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CRONUS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors cronus_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CRONUS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.cronus/profiles/coder or
    # /opt/data/profiles/coder).  If CRONUS_HOME points to the cronus root
    # instead (e.g. systemd hardcodes CRONUS_HOME=/root/.cronus), we must
    # still read active_profile — the user may have switched profiles via
    # `cronus profile use` and the gateway should honour that choice.
    # See issue #22502.
    cronus_home_env = os.environ.get("CRONUS_HOME", "")
    if profile_name is None and cronus_home_env:
        if Path(cronus_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the cronus root
    if profile_name is None:
        try:
            from cronus_constants import get_default_cronus_root

            active_path = get_default_cronus_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set CRONUS_HOME
    if profile_name is not None:
        try:
            from cronus_cli.profiles import resolve_profile_env

            cronus_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent cronus from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["CRONUS_HOME"] = cronus_home
...
# Load .env from ~/.cronus/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from cronus_cli.config import get_cronus_home
from cronus_cli.env_loader import load_cronus_dotenv

load_cronus_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → CRONUS_REDACT_SECRETS env
# var BEFORE cronus_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_cronus_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "CRONUS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["CRONUS_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `cronus` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from cronus_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from cronus_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if cronus_constants not importable yet
...
from cronus_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_cronus_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("CRONUS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("CRONUS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from cronus_cli.config import get_env_path, get_cronus_home, load_config
    from cronus_cli.auth import get_auth_status

    # Determine whether Cronus itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from cronus_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_cronus_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from cronus_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_cronus_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Cronus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Cronus to use their tokens.
    if _has_cronus_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from cronus_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, cronus_bin
        cli_args: the original CLI arguments (everything after 'cronus')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    cronus_bin = container_info["cronus_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo cronus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo cronus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  cronus --tui --resume {target}")
    if title:
        print(f'  cronus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @cronus/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@cronus" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/cronus-ink/package.json",
    "packages/cronus-ink/package-lock.json",
    "packages/cronus-ink/index.js",
    "packages/cronus-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``CRONUS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("CRONUS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``CRONUS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("CRONUS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    cronus_home = os.environ.get("CRONUS_HOME") or str(Path.home() / ".cronus")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CRONUS_HOME": cronus_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(cronus_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(cronus_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if cronus_cli_dir is None:
        cronus_cli_dir = Path(__file__).parent
    bundled = cronus_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (CRONUS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("CRONUS_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from cronus_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("CRONUS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with CRONUS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset CRONUS_TUI_DIR (e.g. `unset CRONUS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("CRONUS_QUIET"):
    ...
    if tui_dev:
        # Keep the local @cronus/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @cronus/ink resolves through
        # packages/cronus-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "cronus-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from cronus_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="cronus-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["CRONUS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["CRONUS_PYTHON_SRC_ROOT"] = os.environ.get(
        "CRONUS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("CRONUS_PYTHON", sys.executable)
    env.setdefault("CRONUS_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["CRONUS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["CRONUS_MODEL"] = model
        env["CRONUS_INFERENCE_MODEL"] = model
    if provider:
        env["CRONUS_TUI_PROVIDER"] = provider
        env["CRONUS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["CRONUS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["CRONUS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["CRONUS_TUI_SKILLS"] = value
    if query:
        env["CRONUS_TUI_QUERY"] = query
    if image:
        env["CRONUS_TUI_IMAGE"] = image
    if checkpoints:
        env["CRONUS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["CRONUS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["CRONUS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["CRONUS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["CRONUS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["CRONUS_ACCEPT_HOOKS"] = "1"
    ...
    # CRONUS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `cronus --tui` try to
    ...
    # still set CRONUS_TUI_RESUME themselves.
    env.pop("CRONUS_TUI_RESUME", None)
    if resume_session_id:
        env["CRONUS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `cronus update` so
    ...
    if code == 42:
        from cronus_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``CRONUS_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``cronus kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``cronus kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("CRONUS_KANBAN_BOARD"):
        return
    try:
        from cronus_cli.kanban_db import get_current_board

        os.environ["CRONUS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("CRONUS_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'cronus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from cronus_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from cronus_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'cronus doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Cronus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  cronus setup")
        print()

        from cronus_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'cronus setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from cronus_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["CRONUS_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.cronus/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["CRONUS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["CRONUS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["CRONUS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from cronus_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from cronus_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from cronus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Cronus?")
        print()
    ...
    # has no creds.json.  Every subsequent `cronus gateway` then paid a 30s
    ...
    session_dir = get_cronus_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: cronus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `cronus gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  cronus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Cronus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  cronus gateway")
        ...
        print()
        print("  Or install as a service: cronus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'cronus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from cronus_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from cronus_cli.config import stamp_install_method
    from cronus_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Cronus post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``cronus model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from cronus_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from cronus_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from cronus_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("CRONUS_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from cronus_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'cronus model' for "
                "available providers, or run 'cronus doctor' to diagnose config "
                "issues."
            )
    ...
    from cronus_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.cronus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.cronus/.env if the active provider is not 'custom'.
    ...
    """
    from cronus_cli.config import get_env_value, save_env_value, load_config
...
#
# Cronus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `cronus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `cronus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`cronus_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from cronus_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from cronus_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Cronus only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``cronus model`` flow, then route aux tasks to them here.
    """
    from cronus_cli.config import load_config
    from cronus_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from cronus_cli.auth import _prompt_model_selection
    from cronus_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from cronus_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from cronus_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from cronus_constants import OPENROUTER_BASE_URL
    from cronus_cli.auth import (
        ProviderConfig,
    ...
    from cronus_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.cronus/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `cronus setup` from scratch. OpenRouter
    ...
    from cronus_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from cronus_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from cronus_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `cronus auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from cronus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from cronus_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``cronus model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``cronus model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``cronus auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from cronus_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from cronus_cli.auth import _save_model_choice, deactivate_provider
    from cronus_cli.config import get_env_value, load_config, save_config
    from cronus_cli.secret_prompt import masked_secret_prompt
    ...
    from cronus_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `cronus model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from cronus_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Cronus URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from cronus_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from cronus_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from cronus_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Cronus will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Cronus will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from cronus_cli.curses_ui import flush_stdin
...
# downstream call sites read `cronus_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `cronus` invocation — including
# fast paths like `cronus --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from cronus_cli.config import save_env_value, load_config, save_config
    from cronus_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from cronus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from cronus_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from cronus_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Cronus turns to `copilot --acp`.")
    print("  Cronus currently starts its own ACP subprocess for each request.")
    print("  Cronus uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set CRONUS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``cronus setup`` / ``cronus model``.
    ...
    recover from a malformed paste without editing ``~/.cronus/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from cronus_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from cronus_cli.config import save_env_value
    from cronus_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `cronus setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from cronus_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so cronus knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Cronus typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Cronus, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from cronus_cli.auth import AuthError
        from cronus_cli.models import fetch_lmstudio_models
        ...
        from cronus_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from cronus_constants import display_cronus_home as _dhh_fn

            print(
                f"    Cronus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from cronus_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'cronus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Cronus CLI with a provider."""
    from cronus_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from cronus_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from cronus_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from cronus_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from cronus_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from cronus_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from cronus_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    Dispatches ``cronus slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: cronus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `cronus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from cronus_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from cronus_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from cronus_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from cronus_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `cronus security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from cronus_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from cronus_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from cronus_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from cronus_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Cronus home directory to a zip file."""
    if getattr(args, "quick", False):
        from cronus_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from cronus_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Cronus backup from a zip file."""
    from cronus_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from cronus_cli.banner import check_for_updates
        from cronus_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Cronus Agent."""
    _require_tty("uninstall")
    from cronus_cli.uninstall import run_uninstall
...
# Critical files that every ``cronus`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``cronus update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "cronus_cli/main.py",
    "cronus_cli/config.py",
    "cronus_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``cronus`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="cronus-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``cronus update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``cronus_cli/web_dist/`` (per vite.config.ts
    outDir: "../cronus_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "cronus_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``cronus`` launcher present but ``cronus_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``cronus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``cronus web``).
    ...
    # build path usable on a stock `py -m cronus_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (cronus web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``cronus dashboard`` processes other than ourselves.

    ``cronus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``cronus update`` replaces files on disk, the running
    ...
    patterns = [
        "cronus dashboard",
        "cronus_cli.main dashboard",
        "cronus_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "cronus.*dashboard"`) keeps us consistent
            # with `cronus_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `cronus update`.
    ...
    print("  Preview now:  cronus curator run --dry-run")
    print("  Pause it:     cronus curator pause")
    print(
        "  Docs:         https://cronus-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``cronus update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``cronus update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: cronus curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``cronus dashboard`` processes.

    Called at the end of ``cronus update`` (default ``reason``) and also
    from ``cronus dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    cronus dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Cronus Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `cronus update "
            f"--branch {branch}`, or update against main with `cronus update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/cronus-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="cronus-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"cronus-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for cronus-agent itself
        ...
        # GitHub ZIPs extract to cronus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"cronus-agent-{branch}")
    ...
    # optional — a failure here only affects ``cronus dashboard``. Make
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "cronus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Cronus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes cronus completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Cronus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Cronus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Cronus behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `cronus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/cronus-agent.git",
    "git@github.com:NousResearch/cronus-agent.git",
    "https://github.com/NousResearch/cronus-agent",
    "git@github.com:NousResearch/cronus-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/cronus-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_cronus_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_cronus_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Cronus repository.")
        print("  This means you may miss updates from NousResearch/cronus-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/cronus-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/cronus-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``cronus update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from cronus_constants import get_default_cronus_root

    default_home = get_default_cronus_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``cronus update`` is still progressing even if pip/uv itself is silent.
    """
...
def _cronus_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "cronus.exe",
        scripts_dir / "cronus-gateway.exe",
    ]


def _detect_concurrent_cronus_instances(
    ...
    The Cronus Desktop Electron app spawns ``cronus.EXE`` as a backend child,
    so during ``cronus update`` the user-invoked process and the desktop's
    ...
    shims (``cronus.exe`` / ``cronus-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``cronus update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``cronus.exe``) is a separate process from the Python
    ...
    # entire parent chain. On Windows the setuptools-generated cronus.exe
    ...
    # walking the parent chain, every ``cronus update`` reports its own
    ...
    for shim in _cronus_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "cronus.exe"
    lines = ["✗ Another cronus.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Cronus Desktop, exit any open `cronus` REPLs, and")
    lines.append("  stop the gateway (`cronus gateway stop`) before retrying.")
    lines.append("  Override with `cronus update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_cronus_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``cronus.exe``.
    ...
    when ``cronus update`` runs, ``cronus.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``cronus.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next cronus invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Cronus Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Cronus Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _cronus_exe_shims(scripts_dir):
        ...
        print(
            "    Close Cronus Desktop, exit other `cronus` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `cronus update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_cronus_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``cronus.exe.old.*`` left by prior updates.

    Called early on every cronus invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``cronus update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `cronus update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``cronus.exe`` / ``cronus-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_cronus_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_cronus_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from cronus_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.cronus/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``cronus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.cronus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``cronus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``cronus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.cronus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``cronus update --gateway``) the update is already
    ...
    try:
        ...
        # cronus_cli.config.get_cronus_home to simulate setup failure.
        from cronus_cli.config import get_cronus_home as _get_cronus_home

        logs_dir = _get_cronus_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``cronus update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from cronus_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``cronus update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from cronus_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from cronus_cli.config import recommended_update_command
        from cronus_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``cronus update`` without requiring a reinstall.  The
    ...
    ``cronus`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/cronus.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v cronus'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/cronus, code at /usr/local/lib/cronus-agent).
    fhs_link = Path("/usr/local/bin/cronus")
    ...
    path_comment = (
        "# Cronus Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of CRONUS_HOME before running the update.
    ...
    CRONUS_HOME directories.  The ``--backup`` flag on ``cronus update``
    ...
    try:
        from cronus_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_cronus_home so the user sees ~/.cronus/...
    try:
        from cronus_constants import get_cronus_home, display_cronus_home

        home = get_cronus_home()
        try:
            display_path = f"{display_cronus_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  cronus import {out_path}")
...
def cmd_update(args):
    """Update Cronus Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Cronus Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `cronus update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Cronus via pip (for PyPI installs)."""
    from cronus_cli import __version__
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "cronus-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "cronus-agent"]
    ...
    print("✓ Update complete! Restart cronus to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Cronus Agent...")
    print()

    # On Windows, abort early if another cronus.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_cronus_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/cronus-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # orphan merge-conflict markers in cronus_cli/config.py bricked
        # every user who ran ``cronus update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``cronus
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``cronus update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_cronus_home added to cronus_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload cronus_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_cronus_home() added since the last release.
        try:
            import importlib
            import cronus_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit CRONUS_HOME so
        # it is not affected by sync_skills()'s module-level CRONUS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CRONUS_HOME env var points at the default or a named profile.
        try:
            from cronus_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: cronus config migrate")
            else:
                print()
                print("Skipped. Run 'cronus config migrate' later to configure.")
        else:
        ...
        # check `cronus curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``cronus update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from cronus_cli.tools_config import install_cua_driver
        ...
        # When running as ``cronus update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_cronus_home() / ".update_exit_code"
            try:
        ...
        try:
            from cronus_cli.gateway import (
                is_macos,
            ...
            # Discover all cronus-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. cronus-gateway.service or cronus-gateway-coder.service
                            ...
                            # path in `cronus gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: cronus gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: cronus -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Cronus gateway unit files are still installed.
        # When both cronus.service (from a pre-rename install) and the
        # current cronus-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `cronus update` surfaces the issue until the user migrates.
        try:
            from cronus_cli.gateway import (
                has_legacy_cronus_units,
                _find_legacy_cronus_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_cronus_units():
                print()
                print("⚠ Legacy Cronus gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_cronus_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (cronus.service) fight the current")
                print("  cronus-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    cronus gateway migrate-legacy")
                print()
        ...
        print("  cronus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``cronus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `cronus profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_cronus_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → cronus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.cronus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  cronus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     cronus -p {name} chat")
                else:
        ...
        from cronus_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from cronus_constants import get_cronus_home as _hh
        ...
        from cronus_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `cronus profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from cronus_cli.profiles import profile_exists
        ...
        from cronus_cli.profiles import rename_profile
        ...
        from cronus_cli.profiles import export_profile
        ...
        from cronus_cli.profiles import import_profile
        ...
        import tempfile
        from cronus_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="cronus_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  cronus -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      cronus -p {plan.manifest.name} chat")
        ...
        from cronus_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `cronus profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"cronus -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from cronus_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("cronus_requires"):
            print(f"Requires:     Cronus {data['cronus_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from cronus_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.cronus_requires:
        print(f"  Requires: Cronus {mf.cronus_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``cronus dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``cronus dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No cronus dashboard processes running.")
        return 0

    print(f"{len(pids)} cronus dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No cronus dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `cronus update`.
    ...
    if "CRONUS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["CRONUS_WEB_DIST"])
            if "CRONUS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "cronus_cli" / "web_dist"
        )
    ...
    try:
        from cronus_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from cronus_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("CRONUS_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from cronus_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Cronus log files."""
    from cronus_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from cronus_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``cronus -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``cronus_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``cronus -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``cronus`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("CRONUS_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from cronus_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from cronus_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["CRONUS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["CRONUS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["CRONUS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `cronus --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("CRONUS_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("CRONUS_TUI") == "1"):
        return False
...
def main():
    """Main entry point for cronus CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from cronus_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``cronus.exe.old.*`` quarantine files left by previous
    # ``cronus update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_cronus_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: cronus-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from cronus_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://cronus-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `cronus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.cronus/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://cronus-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from cronus_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from cronus_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if CRONUS_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy cronus.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Cronus gateway unit files "
            "(e.g. cronus.service) left over from older installs. Profile "
            "units (cronus-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `cronus proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Cronus Agent with an interactive wizard. "
        "Run a specific section: cronus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'cronus setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Cronus.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $CRONUS_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Cronus")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from cronus_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Cronus CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: cronus-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Cronus with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set CRONUS_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Cronus Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.cronus/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Cronus profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.cronus/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Cronus profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from cronus_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from cronus_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.cronus/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.cronus/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Cronus Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `cronus "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Cronus "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.cronus/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Cronus Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Cronus setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Cronus Agent. Use 'cronus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    cronus debug share              Upload debug report and print URL
    cronus debug share --lines 500  Include more log lines
    cronus debug share --expire 30  Keep paste for 30 days
    cronus debug share --local      Print report locally (no upload)
    cronus debug share --no-redact  Disable upload-time secret redaction
    cronus debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'cronus debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Cronus home directory to a zip file",
        description="Create a zip archive of your entire Cronus configuration, "
        "skills, sessions, and data (excludes the cronus-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/cronus-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.cronus/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo cronus uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from cronus_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Cronus backup from a zip file",
        description="Extract a previously created Cronus backup into your "
        "Cronus home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Cronus Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from cronus_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.cronus/skills/.bundled_manifest) "
            "so future 'cronus update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from cronus_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from cronus_cli.skills_hub import skills_command
    ...
    from cronus_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/cronus-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `cronus plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from cronus_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``cronus --help``, ``cronus version``, ``cronus logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from cronus_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from cronus_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_cronus_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_cronus_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_cronus_home()}/memories/\n")
        else:
            from cronus_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'cronus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # cronus tools list [--platform cli]
    ...
    # cronus tools disable <name...> [--platform cli]
    ...
    # cronus tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from cronus_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from cronus_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `cronus computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `cronus tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: cronus computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: cronus computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Cronus as an MCP server",
        description=(
            "Manage MCP server connections and run Cronus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'cronus mcp add' to connect to a new server, or\n"
            "'cronus mcp serve' to expose Cronus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Cronus as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `cronus mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `cronus mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `cronus mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from cronus_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_cronus_home() / "sessions"
            ...
            # Launch cronus --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Cronus",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Cronus",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.cronus/ (by default a "
        "single restore-point archive is written to ~/.cronus/backups/ "
        "before apply; restorable with 'cronus import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from cronus_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Cronus Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'cronus config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, cronus will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another cronus.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Cronus Agent",
        description="Remove Cronus Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Cronus Agent as an ACP (Agent Client Protocol) server",
        description="Start Cronus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Cronus ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Cronus provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.cronus/node/ "
    ...
    def cmd_acp(args):
        """Launch Cronus Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Cronus instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `cronus update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `cronus profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Cronus profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Cronus Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `cronus --tui` via PTY/WebSocket). "
            "Alternatively set CRONUS_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `cronus dashboard` cmdlines and SIGTERM them directly — the same
    # path `cronus update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running cronus dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running cronus dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Cronus log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    cronus logs                    Show last 50 lines of agent.log
    cronus logs -f                 Follow agent.log in real time
    cronus logs errors             Show last 50 lines of errors.log
    cronus logs gateway -n 100     Show last 100 lines of gateway.log
    cronus logs --level WARNING    Only show WARNING and above
    cronus logs --session abc123   Filter by session ID
    cronus logs --component tools  Only show tool-related lines
    cronus logs --since 1h         Lines from the last hour
    cronus logs --since 30m -f     Follow, starting from 30 min ago
    cronus logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``cronus -c Pokemon Agent Dev`` → ``cronus -c 'Pokemon Agent Dev'``
    ...
    from cronus_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'cronus -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (cronus hooks list, cron
