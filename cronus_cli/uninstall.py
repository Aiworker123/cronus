"""
Cronus Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cronus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Cronus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing cronus-agent or cronus PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Cronus Agent" comment and following line
                if '# Cronus Agent' in line or '# cronus-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('cronus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing cronus
                if 'cronus' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the cronus wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "cronus",
        Path("/usr/local/bin/cronus"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains cronus_cli reference)
                content = wrapper.read_text()
                if 'cronus_cli' in content or 'cronus-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``cronus gateway run`` processes
    ...
    try:
        from cronus_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from cronus_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CRONUS_HOME`` and ``CRONUS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\cronus\git\cmd``, ``%LOCALAPPDATA%\cronus\git\bin``,
#      ``%LOCALAPPDATA%\cronus\git\usr\bin``, ``%LOCALAPPDATA%\cronus\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\cronus\git\`` and Node to
#      ``%LOCALAPPDATA%\cronus\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``cronus dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\cronus\gateway-service\`` and sometimes
...
def _cronus_path_markers(cronus_home: Path) -> list[str]:
    """Path-entry substrings that identify Cronus-owned User-PATH entries."""
    root = str(cronus_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare cronus-agent install dir.
    markers = [root + "\\cronus-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CRONUS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "cronus".  We don't want to catch
    # unrelated entries like "ccronus-foo" or "ephermeral", so we look for
    # backslash-cronus as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(cronus_home: Path) -> list[str]:
    """Strip Cronus-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _cronus_path_markers(cronus_home)
            kept: list[str] = []
...
def remove_cronus_env_vars_windows() -> list[str]:
    """Delete CRONUS_HOME and CRONUS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CRONUS_HOME", "CRONUS_GIT_BASH_PATH"):
                try:
...
def _is_default_cronus_home(cronus_home: Path) -> bool:
    """Return True when ``cronus_home`` points at the default (non-profile) root."""
    try:
        from cronus_constants import get_default_cronus_root
        return cronus_home.resolve() == get_default_cronus_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CRONUS_HOME directory.

    We shell out to ``cronus -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CRONUS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m cronus_cli.main` so we don't depend on a `cronus`
    #    wrapper that may be half-removed mid-uninstall.
    cronus_invocation = [_sys.executable, "-m", "cronus_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                cronus_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CRONUS_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cronus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cronus/ for future reinstall
    """
    project_root = get_project_root()
    cronus_home = get_cronus_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CRONUS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_cronus_home(cronus_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Cronus Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {cronus_home / 'config.yaml'}")
    print(f"  Secrets: {cronus_home / '.env'}")
    print(f"  Data:    {cronus_home / 'cron/'}, {cronus_home / 'sessions/'}, {cronus_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CRONUS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Cronus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Cronus code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in cronus_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\cronus\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(cronus_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Cronus-owned PATH entries in User environment")

        log_info("Removing CRONUS_HOME / CRONUS_GIT_BASH_PATH User env vars...")
        removed_env = remove_cronus_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Cronus-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing cronus command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cronus/, just remove the cronus-agent subdir
            if cronus_home in project_root.parents or project_root.parent == cronus_home:
    ...
    #     under CRONUS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(cronus_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(cronus_home)
    ...
    # 5. Optionally remove ~/.cronus/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CRONUS_HOME dirs live under
        ...
        try:
            if cronus_home.exists():
                shutil.rmtree(cronus_home)
                log_success(f"Removed {cronus_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {cronus_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {cronus_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/cronus-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/cronus-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Cronus Agent! ⚕")
    print()
