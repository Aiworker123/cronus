"""MCP picker — interactive `cronus mcp picker` (also the default `cronus mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``cronus mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `cronus plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from cronus_cli.cli_output import prompt_yes_no
from cronus_cli.curses_ui import curses_single_select
from cronus_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Cronus session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from cronus_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `cronus mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: cronus mcp install <name>    Picker: cronus mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Cronus is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Cronus — run `cronus update` "
...
def show_catalog() -> None:
    """`cronus mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`cronus mcp picker` (and default `cronus mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`cronus mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `cronus mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
