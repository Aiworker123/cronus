"""cronus webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.cronus/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _cronus_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: cronus gateway run
"""
...
def webhook_command(args):
    """Entry point for 'cronus webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: cronus webhook {subscribe|list|remove|test}")
        print("Run 'cronus webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (cronus gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: cronus webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from cronus webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (cronus gateway run)")
