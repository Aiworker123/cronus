"""
Dump command for cronus CLI.

Outputs a compact, plain-text summary of the user's Cronus setup
...
from cronus_cli.config import get_cronus_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.cronus_build_sha`` by the Dockerfile's
    ``CRONUS_GIT_SHA`` build-arg (see ``cronus_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``cronus dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from cronus_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(cronus_home: Path) -> int:
...
def _cron_summary(cronus_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = cronus_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"cronus_home:      {display_cronus_home()}")
    ...
    toolsets = config.get("toolsets", ["cronus-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(cronus_home)}")
    lines.append(f"  skills:             {_count_skills(cronus_home)}")
