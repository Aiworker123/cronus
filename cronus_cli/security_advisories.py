"""
Security advisory checker for Cronus Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``cronus doctor --ack <id>``; the ack is persisted to
...
1. ``cronus doctor`` (and ``cronus doctor --ack <id>``)
...
run in environments where the rest of Cronus failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including cronus when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.cronus/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: cronus doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Cronus requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'cronus doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.cronus/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_cronus_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``cronus doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `cronus doctor` on the gateway host for details.")
