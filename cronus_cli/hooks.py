"""cronus hooks — inspect and manage shell-script hooks.

Usage::

    cronus hooks list
    cronus hooks test <event> [--for-tool X] [--payload-file F]
    cronus hooks revoke <command>
    cronus hooks doctor

Consent records live under ``~/.cronus/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.cronus/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``cronus hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: cronus hooks {list|test|revoke|doctor}")
        print("Run 'cronus hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.cronus/config.yaml.")
        print("See `cronus hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `cronus hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `cronus hooks test` and `cronus hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Cronus wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `cronus hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `cronus hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `cronus hooks doctor`.")
