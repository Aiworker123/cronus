...
Used by ``cronus profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``cronus_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Cronus Agent kanban board.
...
Rules:
  ...
  - Never write "Cronus Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_cronus_home())
        else:
