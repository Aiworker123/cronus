...
# xAI's public API is OpenAI-compatible for the endpoints Cronus commonly
# uses. The Responses endpoint is included because Cronus' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Cronus-managed OAuth credentials."""

    auth_hint = "cronus auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`cronus auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`cronus auth reset xai-oauth` or re-authenticate with "
                    "`cronus auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `cronus auth add xai-oauth --type oauth`."
            )
