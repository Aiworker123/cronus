...
Pure logic: walks a Cronus config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `cronus doctor` and a future `cronus migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Cronus config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Cronus config and return retirement issues.
