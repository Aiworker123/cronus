"""Shell completion script generation for cronus CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_cronus_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Cronus Agent bash completion
# Add to ~/.bashrc:
#   eval "$(cronus completion bash)"

_cronus_profiles() {{
    local profiles_dir="$HOME/.cronus/profiles"
...
_cronus_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_cronus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _cronus_completion cronus
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef cronus
# Cronus Agent zsh completion
# Add to ~/.zshrc:
...
_cronus_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.cronus/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.cronus/profiles 2>/dev/null)}}")
    fi
...
_cronus() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_cronus_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'cronus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Cronus Agent fish completion",
        "# Add to your config:",
        "#   cronus completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __cronus_profiles",
        "    echo default",
        "    if test -d $HOME/.cronus/profiles",
        "        ls $HOME/.cronus/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c cronus -f -s p -l profile"
        " -d 'Profile name' -xa '(__cronus_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__cronus_profiles)' -d 'Profile name'"
                )
