...
``$CRONUS_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-cronus-setup
(the stage2 hook) has chowned the volume and seeded $CRONUS_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$CRONUS_HOME``,
    not under ``profiles/``). The dispatcher in ``cronus_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``cronus gateway start`` (no ``-p``) targets. Without it,
    bare ``cronus gateway start`` inside the container would land on
    ...
    ``$CRONUS_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        cronus_home: The container's CRONUS_HOME (typically /opt/data).
            Profiles live under ``<cronus_home>/profiles/<name>/``;
            the default profile lives at ``<cronus_home>`` itself.
    ...
    # named profiles).
    default_prior_state = _read_prior_state(cronus_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(cronus_home)
    ...
    profiles_root = cronus_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `cronus profile create` (config.yaml
            # is not — that comes later via `cronus setup`). Use it as the
            ...
            # via ``cronus -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(cronus_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `cronus -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit cronus
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the cronus user) won't hit EACCES. See
...
def _write_reconcile_log(
    cronus_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $CRONUS_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    cronus_home = Path(os.environ.get("CRONUS_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        cronus_home=cronus_home, scandir=scandir,
    )
    for a in actions:
        print(
