"""
cronus fallback — manage the fallback provider chain.
...
Subcommands:
  cronus fallback [list]   Show the current fallback chain (default when no subcommand)
  cronus fallback add      Pick provider + model via the same picker as `cronus model`,
                           then append the selection to the chain
  cronus fallback remove   Pick an entry to delete from the chain
  cronus fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.cronus/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from cronus_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `cronus model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  cronus fallback add")
        print()
        return
    ...
    print("  Docs: https://cronus-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cronus model`, then append the selection to the chain."""
    from cronus_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `cronus model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `cronus fallback list` to view, or `cronus fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cronus fallback [subcommand]``."""
