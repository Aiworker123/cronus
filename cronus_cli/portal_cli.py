"""``cronus portal`` — small CLI surface for Nous Portal users.
...
already in ``cronus auth`` or ``cronus tools``. It's a discovery + status
...
DOCS_URL = "https://cronus-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def portal_command(args) -> int:
    """Top-level dispatch for `cronus portal <subcommand>`."""
    ...
    print("Run `cronus portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `cronus portal` on the given argparse subparsers object."""
