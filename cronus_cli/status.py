"""
Status command for cronus CLI.

Shows the status of all Cronus Agent components.
"""
...
from cronus_cli.auth import AuthError, resolve_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``cronus config``'s
...
def show_status(args):
    """Show status of all Cronus Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Cronus Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: cronus auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: cronus model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: cronus auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: cronus auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: cronus model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from cronus_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from cronus_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   cronus gateway")
    ...
    jobs_file = get_cronus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_cronus_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'cronus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'cronus setup' to configure", Colors.DIM))
    print()
