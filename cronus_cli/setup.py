"""
Interactive setup wizard for Cronus Agent.
...
_DOCS_BASE = "https://cronus-agent.nousresearch.com/docs"
...
# display_cronus_home imported lazily at call sites (stale-module safety during cronus update)
...
from cronus_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from cronus_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Cronus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Cronus using environment variables or config commands:")
    print_info("  cronus config set model.provider custom")
    print_info("  cronus config set model.base_url http://localhost:8080/v1")
    print_info("  cronus config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'cronus setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from cronus_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from cronus_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'cronus setup')")


def _print_setup_summary(config: dict, cronus_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'cronus setup' to configure"))
    ...
    if subscription_features.image_gen.available:
        ...
        try:
            ...
            from cronus_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `cronus tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from cronus_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if tts_provider == "elevenlabs" and get_env_value("ELEVENLABS_API_KEY"):
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'cronus setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'cronus setup tts'"))
    else:
    ...
    # Spotify (OAuth via cronus auth spotify — check auth.json, not env vars)
    try:
        from cronus_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'cronus setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {cronus_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('cronus setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('cronus setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('cronus setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('cronus setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('cronus setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('cronus config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('cronus config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('cronus config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('cronus', Colors.GREEN)}              Start chatting")
    print(f"   {color('cronus gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('cronus doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `cronus tools` and `cronus setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``cronus model``)
    ...
    provider added to ``cronus model`` is automatically available here.
    ...
    # Delegate to the shared cronus model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from cronus_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: cronus model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Cronus can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Cronus can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'cronus setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``cronus model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from cronus_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.cronus/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Cronus auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via cronus setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'cronus setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Cronus runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CRONUS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CRONUS_MAX_ITERATIONS")
    ...
    print_info("  Run `cronus setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CRONUS_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Cronus delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `cronus update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://cronus-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CRONUS_HOME, and print
    ...
    """
    try:
        from cronus_cli.slack_cli import _build_full_manifest
        from cronus_constants import get_cronus_home

        manifest = _build_full_manifest(
            bot_name="Cronus",
            bot_description="Your Cronus agent on Slack",
        )
        target = Path(get_cronus_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `cronus slack manifest --write` anytime to refresh after "
            "Cronus adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Cronus delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Cronus to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from cronus_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://cronus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://cronus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  cronus config edit")
    print_info("   Open config in your editor:  cronus config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from cronus_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Cronus from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'cronus setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     cronus config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_cronus_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  cronus gateway")
        else:
            from cronus_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   cronus gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   cronus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `cronus setup tools` and `cronus tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``cronus_cli.auth`` — lists every supported
    ...
    """
    try:
        from cronus_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from cronus_cli.config
    ...
    if section_key == "model":
        ...
        from cronus_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_cronus.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_cronus migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_cronus", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Cronus.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Cronus to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Cronus at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Cronus at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Cronus at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Cronus at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Cronus equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Cronus config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Cronus.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Cronus's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(cronus_home: Path) -> bool:
    ...
    print_info("Cronus can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: cronus claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=cronus_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: cronus claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Cronus configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Cronus config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Cronus (use cronus claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``cronus setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``cronus setup --portal``)
    that gets a brand-new user from zero to a fully working Cronus session
    ...
    print(color("│     ⚕ Cronus Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `cronus setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `cronus auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `cronus auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    print_info("  Run `cronus portal status` to inspect routing.")
    print_info("  Run `cronus` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      cronus setup           — full or quick (auto-detected)
      cronus setup model     — just model/provider
      cronus setup tts       — just text-to-speech
      cronus setup terminal  — just terminal backend
      cronus setup gateway   — just messaging platforms
      cronus setup tools     — just tool configuration
      cronus setup agent     — just agent settings
    """
    from cronus_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Cronus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Cronus Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Cronus Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, cronus_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Cronus configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'cronus setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(cronus_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Cronus?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, cronus_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {cronus_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'cronus config edit'")
    ...
    _print_setup_summary(config, cronus_home)


def _run_first_time_quick_setup(config: dict, cronus_home, is_existing: bool):
    ...
    the user can customize later via ``cronus setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'cronus setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    cronus setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  cronus setup gateway")
    print()

    _print_setup_summary(config, cronus_home)


def _run_quick_setup(config: dict, cronus_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'cronus setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Cronus to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'cronus setup gateway'.")
