"""
Cronus Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m cronus_cli.main web          # Start on http://127.0.0.1:9119
    python -m cronus_cli.main web --port 8080
"""
...
try:
    ...
    # running `cronus dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["CRONUS_WEB_DIST"]) if "CRONUS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Cronus Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Cronus-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``cronus dashboard --tui``
# or CRONUS_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from cronus_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``cronus status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from cronus_cli.dashboard_auth import list_providers as _list_providers
...
# streamed to a per-action log file under ``~/.cronus/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_cronus_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "cronus-update": "cronus-update.log",
}
...
def _spawn_cronus_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``cronus <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``cronus_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "cronus_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "CRONUS_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``cronus gateway restart`` in the background."""
    try:
        proc = _spawn_cronus_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/cronus/update")
async def update_cronus():
    """Kick off ``cronus update`` in the background."""
    try:
        proc = _spawn_cronus_action(["update"], "cronus-update")
    except Exception as exc:
        _log.exception("Failed to spawn cronus update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Cronus supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in cronus_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.cronus/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``cronus auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Cronus resolves Anthropic creds in this order at runtime:
    1. ``~/.cronus/.anthropic_oauth.json`` — Cronus-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_cronus_oauth_credentials,
            read_claude_code_credentials,
            _CRONUS_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_cronus_oauth_credentials = None  # type: ignore
        _CRONUS_OAUTH_FILE = None  # type: ignore

    cronus_creds = None
    if read_cronus_oauth_credentials:
        try:
            cronus_creds = read_cronus_oauth_credentials()
        except Exception:
            cronus_creds = None
    if cronus_creds and cronus_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "cronus_pkce",
            "source_label": f"Cronus PKCE ({_CRONUS_OAUTH_FILE})",
            "token_preview": _truncate_token(cronus_creds.get("accessToken")),
            "expires_at": cronus_creds.get("expiresAt"),
            "has_refresh_token": bool(cronus_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Cronus even
    when they also have a separate Cronus-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "cronus auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "cronus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "cronus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "cronus auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "cronus auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from cronus_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("cronus_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Cronus-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _CRONUS_OAUTH_FILE
            if _CRONUS_OAUTH_FILE.exists():
                _CRONUS_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from cronus_cli.auth import clear_provider_auth
...
#          → persists to ~/.cronus/.anthropic_oauth.json AND credential pool
...
# Guarded so cronus web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Cronus file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``cronus auth add anthropic``.
    """
    from agent.anthropic_adapter import _CRONUS_OAUTH_FILE
    payload = {
    ...
    _CRONUS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _CRONUS_OAUTH_FILE.with_name(
        f"{_CRONUS_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _CRONUS_OAUTH_FILE)
        try:
            _CRONUS_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cronus-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("CRONUS_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from cronus_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from cronus_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_cronus_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from cronus_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from cronus_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, CRONUS_HOME)."""
    from cronus_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["cronus_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process CRONUS_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_cronus_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "cronus setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from cronus_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from cronus_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from cronus_cli.tools_config import (
...
#
# The endpoint spawns the same ``cronus --tui`` binary the CLI uses, behind
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from cronus_cli.dashboard_auth.audit import AuditEvent, audit_log
        from cronus_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``cronus --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``CRONUS_TUI_RESUME`` env var —
    matching what ``cronus_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``CRONUS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from cronus_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("CRONUS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("CRONUS_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["CRONUS_TUI_RESUME"] = resume

    if sidecar_url:
        env["CRONUS_TUI_SIDECAR_URL"] = sidecar_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from cronus_cli.dashboard_auth.ws_tickets import mint_ticket
...
# CRONUS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`cronus_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from cronus_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/cronus/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /cronus`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__CRONUS_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/cronus``)
        ...
        ``__CRONUS_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__CRONUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CRONUS_BASE_PATH__="{prefix}";'
                f"window.__CRONUS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__CRONUS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__CRONUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CRONUS_BASE_PATH__="{prefix}";'
                f"window.__CRONUS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/cronus`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Cronus backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Cronus Teal",         "description": "Classic dark teal — the canonical Cronus look"},
    {"name": "default-large", "label": "Cronus Teal (Large)", "description": "Cronus Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.cronus/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.cronus/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_cronus_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.cronus/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as cronus_cli.plugins):
    1. User plugins:    ~/.cronus/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.cronus/plugins/  (only if CRONUS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from cronus_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_cronus_home() / "plugins", "user"),
    ...
    # ``cronus_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("CRONUS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".cronus" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from cronus_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_cronus_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"cronus auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from cronus_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from cronus_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from cronus_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from cronus_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.cronus/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.cronus/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"cronus_dashboard_plugin_{plugin['name']}"
...
from cronus_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from cronus_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing CRONUS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Cronus Agent exposes a
    ...
    print(f"  Cronus Web UI → http://{host}:{port}")
