...
- ``cronus_cli.providers``        -- canonical provider identity + overlays
- ``cronus_cli.model_normalize``  -- per-provider name formatting
...
_CRONUS_MODEL_WARNING = (
    "Nous Research Cronus 3 & 4 models are NOT agentic and are not designed "
    "for use with Cronus Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Cronus 3 / Cronus 4 chat families.
# The previous substring check (`"cronus" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``cronus-brain:qwen3-14b-ctx16k`` that just
# happen to carry "cronus" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Cronus-3-Llama-3.1-70B, cronus-4-405b, openrouter/cronus3:70b
# Negative examples it must NOT match:
#   cronus-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_CRONUS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])cronus[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_cronus_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Cronus 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_CRONUS_NON_AGENTIC_RE.search(model_name))


def _check_cronus_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Cronus 3/4 chat model."""
    if is_nous_cronus_non_agentic(model_name):
        return _CRONUS_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``cronus config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from cronus_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'cronus model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'cronus doctor' — config issues detected:"
    ...
    # Mirrors the same logic in cronus_cli.runtime_provider.resolve_runtime_provider;
    ...
    cronus_warn = _check_cronus_model_warning(new_model)
    if cronus_warn:
        warnings.append(cronus_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from cronus_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from cronus_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by cronus provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://cronus-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Cronus release. Falls back to the in-repo
    ...
    # --- 1. Check Cronus-mapped providers ---
    for cronus_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(cronus_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CRONUS_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from cronus_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(cronus_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Cronus release.
        model_ids = curated.get(cronus_id, [])
        if cronus_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(cronus_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Cronus-only providers (nous, openai-codex, copilot, opencode-go) ---
    from cronus_cli.providers import CRONUS_OVERLAYS
    from cronus_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Cronus provider ID.
    # CRONUS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Cronus IDs ("copilot").
    _mdev_to_cronus = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CRONUS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Cronus slug — e.g. "github-copilot" → "copilot"
        cronus_slug = _mdev_to_cronus.get(pid, pid)
        if cronus_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(cronus_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, cronus_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or cronus_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(cronus_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", cronus_slug, exc)
        ...
        # configured.
        if not has_creds and cronus_slug == "anthropic":
            try:
                ...
                if (cronus_creds and cronus_creds.get("accessToken")) or \
        ...
        if cronus_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(cronus_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(cronus_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(cronus_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Cronus slug, fall back to overlay key
            model_ids = curated.get(cronus_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if cronus_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(cronus_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": cronus_slug,
            "name": get_label(cronus_slug),
            "is_current": cronus_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "cronus",
        ...
        seen_slugs.add(cronus_slug.lower())
        _record_builtin_endpoint(cronus_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CRONUS_OVERLAYS (keeps /model in sync
    # with `cronus model`).
    try:
        from cronus_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Cronus's canonical write key (matches
            ...
            # Cronus writes ``models:`` as a dict keyed by model id
            # (see cronus_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Cronus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Cronus's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, cronus_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`cronus_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from cronus_cli.models import fetch_openrouter_models
