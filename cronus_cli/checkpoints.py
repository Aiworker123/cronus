"""`cronus checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.cronus/checkpoints/``.  Actions:

    cronus checkpoints               # same as `status`
    cronus checkpoints status        # total size, project count, breakdown
    cronus checkpoints list          # per-project checkpoint counts + workdir
    cronus checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    cronus checkpoints clear [-f]    # nuke the entire base (asks first)
    cronus checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    cronus checkpoints
    cronus checkpoints prune --retention-days 3 --max-size-mb 200
    cronus checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: cronus checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``cronus checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `cronus checkpoints` → status
