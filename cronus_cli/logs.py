"""``cronus logs`` — view and filter Cronus log files.
...
Usage examples::
    ...
    cronus logs                    # last 50 lines of agent.log
    cronus logs -f                 # follow agent.log in real time
    cronus logs errors             # last 50 lines of errors.log
    cronus logs gateway -n 100    # last 100 lines of gateway.log
    cronus logs --level WARNING    # only WARNING+ lines
    cronus logs --session abc123   # filter by session ID substring
    cronus logs --component tools  # only tool-related lines
    cronus logs --since 1h         # lines from the last hour
    cronus logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_cronus_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Cronus runs — try 'cronus chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_cronus_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_cronus_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_cronus_home()}/logs/")
        return

    print(f"Log files in {display_cronus_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'cronus chat' to generate logs)")
