"""Slash command definitions and autocomplete for the Cronus CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Cronus config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Cronus works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Cronus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.cronus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Cronus Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`cronus_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/cronus`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Cronus
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``cronus skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``cronus skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``cronus skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/cronus <verb>`` subcommand.
    ...
    can still reach them via ``/cronus <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/cronus`` is always reserved as the first entry so the
    legacy ``/cronus <subcommand>`` form keeps working for anything that
    ...
    # Reserve /cronus as the catch-all top-level command.
    entries.append(("cronus", "Talk to Cronus or run a subcommand", "[subcommand] [args]"))
    seen.add("cronus")
...
def slack_app_manifest(request_url: str = "https://cronus-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /cronus handler.

    Maps both canonical names and aliases so /cronus bg do stuff works
    the same as /cronus background do stuff.

    Plugin-registered slash commands are included so ``/cronus <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from cronus_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
