...
on Japanese locales, etc.).  Cronus's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``cronus_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Cronus is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CRONUS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CRONUS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Cronus-managed Git install directories so
    ...
    # already-running shells don't see the change — which means cronus
    ...
    # install without Cronus-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Cronus's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Cronus's
    ``cronus config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Cronus (User env var in Windows Settings, or
    export in a PowerShell profile) and Cronus picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Cronus-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\cronus\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``cronus`` from right after install) retain their old PATH.

    Any subprocess Cronus spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Cronus-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "cronus", "git", "cmd"),
        os.path.join(local_appdata, "cronus", "git", "bin"),
        os.path.join(local_appdata, "cronus", "git", "usr", "bin"),
        # Cronus venv Scripts directory — host of the cronus.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes cronus, but harmless to include.
        os.path.join(local_appdata, "cronus", "cronus-agent", "venv", "Scripts"),
