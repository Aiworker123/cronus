"""CLI subcommand: ``cronus send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"cronus send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"cronus send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"cronus send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"cronus send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"cronus send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `cronus gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.cronus/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_cronus_env() -> None:
    """Populate ``os.environ`` from ``~/.cronus/.env`` AND bridge top-level
    ...
    startup that ``cronus send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.cronus/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.cronus/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``cronus send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.cronus/.env and ~/.cronus/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "cronus send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  cronus send --to telegram \"hello\"\n"
            "  cronus send --to discord:#ops --file report.md\n"
            "  cronus send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "cronus send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `cronus send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Cronus "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.cronus/.env + ~/.cronus/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `cronus send --list telegram`.",
    )
