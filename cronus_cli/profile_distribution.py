"""Profile distributions — shareable, packaged Cronus profiles via git.

A distribution is a Cronus profile published as a git repository (or
...
* ``cronus profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``cronus skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``cronus profile``, not a parallel tree):

    cronus profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "cronus-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            cronus_requires=str(data.get("cronus_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.cronus_requires:
            out["cronus_requires"] = self.cronus_requires
        if self.author:
...
def check_cronus_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Cronus {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``cronus profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Cronus profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Cronus distribution."
        )

    # Version check up-front so we fail fast
    check_cronus_requires(manifest.cronus_requires, cronus_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.cronus).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="cronus_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `cronus profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`cronus profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="cronus_dist_update_") as tmp:
        plan = plan_install(
