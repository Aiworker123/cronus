"""
Configuration management for Cronus Agent.

Config files are stored in ~/.cronus/ for easy access:
- ~/.cronus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.cronus/.env         - API keys and secrets

This module provides:
- cronus config          - Show current configuration
- cronus config edit     - Open config in editor
- cronus config set      - Set a specific value
- cronus config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.cronus/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``cronus logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  cronus config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Cronus makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Cronus itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``cronus update``, the TUI build.
...
#   on every plugin install / ``cronus update``.
...
# * ``CRONUS_HOME`` / ``CRONUS_PROFILE`` / ``CRONUS_CONFIG`` /
#   ``CRONUS_ENV`` — Cronus runtime location flags. Writing these into
...
#
# IMPORTANT: ``CRONUS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CRONUS_GEMINI_CLIENT_ID,
# CRONUS_LANGFUSE_PUBLIC_KEY, CRONUS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Cronus runtime location — never via dashboard env writer.
    # NOT a CRONUS_* blanket: integration credentials (CRONUS_GEMINI_*,
    # CRONUS_LANGFUSE_*, CRONUS_SPOTIFY_*, ...) ARE allowed.
    "CRONUS_HOME", "CRONUS_PROFILE", "CRONUS_CONFIG", "CRONUS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Cronus runtime location "
            "(CRONUS_HOME, CRONUS_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `cronus plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "CRONUS_LANGFUSE_ENV",
    "CRONUS_LANGFUSE_RELEASE",
    "CRONUS_LANGFUSE_SAMPLE_RATE",
    "CRONUS_LANGFUSE_MAX_CHARS",
    "CRONUS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from cronus_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CRONUS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_cronus_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Cronus is running in package-manager-managed mode.

    Two signals: the CRONUS_MANAGED env var (set by the systemd service),
    or a .managed marker file in CRONUS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade cronus-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Cronus was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.cronus/.install_method`` file (written by installers)
    2. CRONUS_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_cronus_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.cronus/.install_method."""
    stamp = get_cronus_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade cronus-agent"
    if method == "docker":
        return "docker pull nousresearch/cronus-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade cronus-agent"
        return "pip install --upgrade cronus-agent"
    return "cronus update"
...
# Long-form text for ``cronus update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# cronus_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Cronus, it doesn't update
...
✗ ``cronus update`` doesn't apply inside the Docker container.

Cronus Agent runs as a published image (nousresearch/cronus-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/cronus-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate cronus-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/cronus-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/cronus-agent/tags
  • Your config and session history live under ``$CRONUS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``cronus update`` inside Docker.
...
def format_managed_message(action: str = "modify this Cronus installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CRONUS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Cronus installation is managed by NixOS "
            f"(CRONUS_MANAGED={env_hint}).\n"
            "Edit services.cronus-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Cronus installation is managed by Homebrew "
            f"(CRONUS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade cronus-agent"
        )

    return (
        f"Cannot {action}: this Cronus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Cronus."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CRONUS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, cronus_bin
    or None if container mode is not active, we're already inside the
    container, or CRONUS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CRONUS_DEV") == "1":
        return None
    ...
    container_mode_file = get_cronus_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "cronus-agent")
    exec_user = info.get("exec_user", "cronus")
    cronus_bin = info.get("cronus_bin", "/data/current-package/bin/cronus")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "cronus_bin": cronus_bin,
    }
...
# Re-export from cronus_constants — canonical definition lives there.
from cronus_constants import get_cronus_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_cronus_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_cronus_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the cronus group can
    share state with the gateway service.

    The mode can be overridden via the CRONUS_HOME_MODE environment variable
    (e.g. CRONUS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CRONUS_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CRONUS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Cronus runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CRONUS_CONTAINER") or os.environ.get("CRONUS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CRONUS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CRONUS_HOME if the user doesn't have one yet."""
...
def ensure_cronus_home():
    """Ensure ~/.cronus directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_cronus_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_cronus_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CRONUS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Cronus-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Cronus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Cronus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Cronus
        ...
        # bundled Cronus image, which drops to the `cronus` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Cronus sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Cronus should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``cronus chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.cronus/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: cronus sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Cronus truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``cronus kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``cronus kanban decompose`` and the kanban
        ...
        # ``cronus profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `cronus model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `cronus --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `cronus -c` muscle memory.  Default off so existing
        # users aren't surprised.  CRONUS_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/cronus`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``CRONUS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CRONUS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # Public URL override (env: ``CRONUS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/cronus``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.cronus/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.cronus/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Cronus feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Cronus auto-pauses the goal and
    ...
    # always goes to ~/.cronus/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CRONUS_SKILL_DIR} and ${CRONUS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `cronus curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.cronus/skills/ into
        # ~/.cronus/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `cronus curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for cronus-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Cronus Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CRONUS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.cronus/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CRONUS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `cronus doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``cronus_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Cronus to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CRONUS_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `cronus -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `cronus` launches with
        ...
        # decomposition is manual via `cronus kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     cronus-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.cronus/logs/.
    ...
    # update model picker lists without shipping a cronus-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://cronus-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `cronus model` invocation; network failures
    ...
    "gateway": {
        ...
        # uploaded as native gateway attachments. Files inside the Cronus
        # cache (~/.cronus/cache/{documents,images,audio,video,screenshots})
        ...
        # to CRONUS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        # expanded.
        ...
        # CRONUS_MEDIA_TRUST_RECENT_FILES.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CRONUS_MEDIA_TRUST_RECENT_SECONDS.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.cronus/state.db.
    ...
    "sessions": {
        ...
        # default of ``cronus sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.cronus/sessions/session_{sid}.json`` on every turn
    ...
    # ``cronus update`` behaviour.
    "updates": {
        # Run a full ``cronus backup``-style zip of CRONUS_HOME before every
        # ``cronus update``.  Backups land in ``<CRONUS_HOME>/backups/`` and
        # can be restored with ``cronus import <path>``.  Off by default —
        # on large CRONUS_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<CRONUS_HOME>/lsp/bin/`` on first use.
    ...
    # `cronus tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.cronus/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.cronus/.env (or your shell) and never in config.yaml.
            ...
            # ~/.cronus/bin/ on first use.  When False you must install
            ...
            # `cronus secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'cronus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Cronus auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @cronus:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CRONUS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #cronus)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: cronus-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'cronus-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Cronus API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Cronus API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Cronus API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CRONUS_TOOL_PROGRESS and CRONUS_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "CRONUS_TOOL_PROGRESS": {
    ...
    "CRONUS_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.cronus.config`` entries, then checks
    ...
    try:
        ...
        # should never break `cronus update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Cronus versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``cronus_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'cronus setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Cronus won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'cronus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CRONUS_HOME", "~/.cronus")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CRONUS_TOOL_PROGRESS")
            old_mode = get_env_value("CRONUS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CRONUS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CRONUS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CRONUS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CRONUS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CRONUS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_cronus_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `cronus plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `cronus curator status` has no
    ...
    #   3. Creates `~/.cronus/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_cronus_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_cronus_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: cronus config set <key> <value>")
    ...
    # metadata.cronus.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _cronus_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.cronus/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.cronus/config.yaml.
    ...
    (which change ``CRONUS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — cronus auth) — OpenAI Codex
#   nous         (OAuth — cronus auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.cronus/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.cronus/.env.
    ...
    interactive menu render (`cronus tools`, `cronus setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `cronus tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Cronus env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.cronus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.cronus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.cronus/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.cronus/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Cronus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Cronus vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.cronus/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Cronus Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  cronus config edit     # Edit config file", Colors.DIM))
    print(color("  cronus config set <key> <value>", Colors.DIM))
    print(color("  cronus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: cronus config set <key> <value>")
            print()
            print("Examples:")
            print("  cronus config set model anthropic/claude-sonnet-4")
            print("  cronus config set terminal.backend docker")
            print("  cronus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'cronus config migrate' to add them")
        ...
        print("  cronus config           Show current configuration")
        print("  cronus config edit      Open config in editor")
        print("  cronus config set <key> <value>   Set a config value")
        print("  cronus config check     Check for missing/outdated config")
        print("  cronus config migrate   Update config with new options")
        print("  cronus config path      Show config file path")
        print("  cronus config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``cronus config`` UI so users
