...
``<root>`` is the **shared Cronus root** (the parent of any active
...
``cronus -p <profile>`` joins the same board as the dispatcher that
...
the profile's Cronus home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``CRONUS_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``CRONUS_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``cronus kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.cronus``. In Docker / custom
deployments where ``CRONUS_HOME`` points outside ``~/.cronus`` (e.g.
``/opt/cronus``), ``<root>`` is ``CRONUS_HOME``. Legacy env-var
overrides still work:

* ``CRONUS_KANBAN_DB`` — pin the database file path directly.
* ``CRONUS_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``CRONUS_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``CRONUS_KANBAN_DB``,
``CRONUS_KANBAN_WORKSPACES_ROOT``, and ``CRONUS_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/cronus-kanban-v1-spec.pdf`` for the full
...
# ``CRONUS_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``CRONUS_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("CRONUS_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``CRONUS_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("CRONUS_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``cronus-agent``
...
def kanban_home() -> Path:
    """Return the shared Cronus root that anchors the kanban board.

    Resolution order:

    1. ``CRONUS_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_cronus_root()``, which already returns ``<root>``
       when ``CRONUS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``CRONUS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``CRONUS_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("CRONUS_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from cronus_constants import get_default_cronus_root
    return get_default_cronus_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``cronus kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``CRONUS_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``cronus kanban boards
    ...
    """
    env = os.environ.get("CRONUS_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``cronus kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``CRONUS_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("CRONUS_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``CRONUS_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("CRONUS_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``cronus kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``CRONUS_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``CRONUS_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("CRONUS_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Cronus treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``CRONUS_KANBAN_DB`` env → ``CRONUS_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See cronus_state._WAL_INCOMPAT_MARKERS for detection logic.
                from cronus_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``cronus kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``CRONUS_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from cronus_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``cronus --skills ...`` alongside the built-in
    ...
    # `cronus --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``cronus kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `cronus kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``cronus kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``cronus kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``cronus kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``cronus kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``CRONUS_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Cronus' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Cronus-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``cronus kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Cronus
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``cronus kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Cronus profile.
    ...
    if not rows:
        return False
    try:
        from cronus_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Cronus profile.
        # `_default_spawn` invokes ``cronus -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Cronus
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `cronus kanban tail` — without
...
def _module_cronus_argv() -> list[str]:
    """Return the interpreter-bound Cronus CLI invocation."""
    # ``cronus_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``cronus`` package — there is no
    # ``cronus`` package to import.
    return [sys.executable, "-m", "cronus_cli.main"]


def _absolute_cronus_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Cronus shim."""
...
def _cronus_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Cronus executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_cronus_argv()
    return [_absolute_cronus_path(path)]


def _resolve_cronus_argv() -> list[str]:
    """Resolve the ``cronus`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$CRONUS_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("cronus")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\cronus.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m cronus_cli.main`` — fallback for setups where
       Cronus is launched from a venv and the ``cronus`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_cronus_bin`` for the same reason. Kept
    local (not imported from gateway) because ``cronus_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("CRONUS_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _cronus_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _cronus_path_argv(resolved_env_bin)
        return _module_cronus_argv()

    cronus_bin = _safe_which_no_cwd("cronus") if _IS_WINDOWS else shutil.which("cronus")
    if cronus_bin:
        return _cronus_path_argv(cronus_bin)
    return _module_cronus_argv()


def _kanban_worker_skill_available(cronus_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``cronus -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset CRONUS_HOME means the worker falls back to the default root
    # home (``~/.cronus``), which ships the bundled skill.
    base = _Path(cronus_home) if cronus_home else (_Path.home() / ".cronus")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``cronus -p <profile> chat -q ...`` subprocess.
    ...
    ``CRONUS_KANBAN_DB`` / ``CRONUS_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject CRONUS_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `cronus -p <name>` the
    # _apply_profile_override() runs *before* cronus_constants is imported.
    # If CRONUS_HOME is absent from the child's env, get_cronus_home() falls
    # back to Path.home() / ".cronus" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from cronus_cli.profiles import resolve_profile_env
    try:
        env["CRONUS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via CRONUS_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CRONUS_HOME never had profiles created.
        pass
    if task.tenant:
        env["CRONUS_TENANT"] = task.tenant
    env["CRONUS_KANBAN_TASK"] = task.id
    env["CRONUS_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["CRONUS_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["CRONUS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["CRONUS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`cronus -p <name>`
    # rewrites CRONUS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_cronus_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["CRONUS_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["CRONUS_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["CRONUS_KANBAN_BOARD"] = resolved_board
    # CRONUS_PROFILE is the author the kanban_comment tool defaults to.
    # `cronus -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["CRONUS_PROFILE"] = profile_arg

    cmd = [
        *_resolve_cronus_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped CRONUS_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("CRONUS_HOME")):
    ...
    # `cronus kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`cronus` executable not found on PATH. "
            "Install Cronus Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``cronus kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled CRONUS_PROFILE values like "cronus-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Cronus root exists

    Reads profile paths directly so this module has no import dependency on
    ``cronus_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from cronus_constants import get_default_cronus_root
        default_root = get_default_cronus_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``cronus kanban assignees`` for the terminal.
