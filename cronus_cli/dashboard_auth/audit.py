...
Profile-aware location: ``$CRONUS_HOME/logs/dashboard-auth.log``.
...
from ``cronus_constants`` or other cronus_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CRONUS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``cronus_constants.get_cronus_home`` semantics: env var wins,
    else ``~/.cronus``. A local copy avoids an import cycle with the
    middleware which lives below ``cronus_cli``.
    """
    home = os.environ.get("CRONUS_HOME") or str(Path.home() / ".cronus")
