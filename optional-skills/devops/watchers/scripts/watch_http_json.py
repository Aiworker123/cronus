...
Usage (via cron with --no-agent):
    ...
    cronus cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CRONUS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Cronus-Watcher/1.0"})
