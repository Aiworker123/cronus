...
Usage (via cron with --no-agent):
    ...
    cronus cron create cronus-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CRONUS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name cronus-issues --repo NousResearch/cronus-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.cronus/.env to avoid the 60 req/hr
