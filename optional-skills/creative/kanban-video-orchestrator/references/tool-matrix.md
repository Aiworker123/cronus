...
Maps each role archetype to the Cronus skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public cronus-agent
...
## Cronus skills relevant to video production

### Visual / rendering skills (`cronus-agent/skills/creative/`)
...
### Design / pre-production skills (`cronus-agent/skills/creative/`)
...
### Audio / media skills (`cronus-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`cronus-agent/skills/devops/`)
...
These are **not** Cronus skills but external CLIs / APIs that profiles invoke.
...
## Built-in Cronus tools for media review

These are native Cronus tools — not invoked via terminal but through their own
...
key is present in `~/.cronus/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Cronus profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.cronus/.env` → environment variable.
...
When a new Cronus-public video skill ships:
