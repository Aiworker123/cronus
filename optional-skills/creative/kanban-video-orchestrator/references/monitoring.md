...
cronus kanban watch --tenant <project-slug>

# Snapshot of the board
cronus kanban list --tenant <project-slug>
cronus kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
cronus kanban stats --tenant <project-slug>

# Visual dashboard (browser)
cronus dashboard

# Inspect a specific task (includes comments + events)
cronus kanban show <task-id>

# Follow a single task's event stream
cronus kanban tail <task-id>
```

Verify available subcommands with `cronus kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `cronus kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `cronus kanban block <id>` then `cronus kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `cronus kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `cronus kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `cronus profile list` (profile exists?), `cronus status` (gateway/dispatcher up?), and verify tenant. |
...
cronus kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
cronus kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(cronus kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
cronus kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
cronus kanban block <task-id>
cronus kanban archive <task-id>

# Diagnose what happened
cronus kanban show <task-id>      # task body, comments, recent events
cronus kanban tail <task-id>      # follow the live event stream
cronus kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `cronus kanban create` for the director
...
while true; do
    clear
    cronus kanban list --tenant <slug>
    echo "---"
    cronus kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `cronus kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `cronus profile list` and start
