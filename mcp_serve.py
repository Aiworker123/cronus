"""
Cronus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Cronus-specific extra)

Usage:
    cronus mcp serve
    cronus mcp serve --verbose
...
logger = logging.getLogger("cronus.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CRONUS_HOME."""
    try:
        from cronus_constants import get_cronus_home
        return get_cronus_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CRONUS_HOME", Path.home() / ".cronus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from cronus_constants import get_cronus_home
        directory_file = get_cronus_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CRONUS_HOME", Path.home() / ".cronus")
...
class EventBridge:
    ...
    This is the Cronus equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from cronus_constants import get_cronus_home
            db_file = get_cronus_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CRONUS_HOME", Path.home() / ".cronus")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Cronus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "cronus",
        instructions=(
            "Cronus Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Cronus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
