...
# 5. Symlinks the 'cronus' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Cronus Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/cronus-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink cronus into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up cronus command..."

CRONUS_BIN="$SCRIPT_DIR/venv/bin/cronus"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CRONUS_BIN" "$COMMAND_LINK_DIR/cronus"
echo -e "${GREEN}✓${NC} Symlinked cronus → $COMMAND_LINK_DISPLAY_DIR/cronus"
...
# ============================================================================
# Seed bundled skills into ~/.cronus/skills/
# ============================================================================

CRONUS_SKILLS_DIR="${CRONUS_HOME:-$HOME/.cronus}/skills"
mkdir -p "$CRONUS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.cronus/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CRONUS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  cronus status        # Check configuration"
if is_termux; then
    echo "  cronus gateway       # Run gateway in foreground"
else
    echo "  cronus gateway install # Install gateway service (messaging + cron)"
fi
echo "  cronus cron list     # View scheduled jobs"
echo "  cronus doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m cronus_cli.main setup
fi
