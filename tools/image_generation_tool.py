...
selectable via ``cronus tools`` → Image Generation; the active model is
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`cronus setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `cronus tools` "
        "→ Image Generation (run `cronus plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping cronus with a FAL key configured
