...
running under the dispatcher (env var ``CRONUS_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``cronus chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``cronus kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``cronus kanban complete …``
   inside the container, where ``cronus`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.cronus/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``cronus kanban …``), the dashboard
(``cronus dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``CRONUS_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``cronus chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("CRONUS_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("CRONUS_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("CRONUS_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``CRONUS_KANBAN_TASK`` set
    ...
    ``CRONUS_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``CRONUS_KANBAN_DB`` → ``CRONUS_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Cronus.
    """
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set CRONUS_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # CRONUS_KANBAN_CLAIM_LOCK in the worker env at spawn time
            # (see _default_spawn in kanban_db.py); falling back to the
            # default _claimer_id() covers locally-driven workers that
            # never went through the dispatcher path.
            claim_lock = os.environ.get("CRONUS_KANBAN_CLAIM_LOCK")
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``cronus-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("CRONUS_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("CRONUS_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets CRONUS_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("CRONUS_SESSION_ID")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("CRONUS_PROFILE") or "worker",
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to CRONUS_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: CRONUS_KANBAN_DB env → "
    "CRONUS_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to CRONUS_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
