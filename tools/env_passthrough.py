...
def _is_cronus_provider_credential(name: str) -> bool:
    """True if ``name`` is a Cronus-managed provider credential (API key,
    token, or similar) per ``_CRONUS_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Cronus API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _CRONUS_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _CRONUS_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Cronus-managed provider credentials (from
    ``_CRONUS_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Cronus-managed
    ...
    Non-Cronus third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_cronus_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Cronus provider "
                "credential %r (blocked by _CRONUS_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Cronus-managed provider credentials must not be passed
                ...
                if _is_cronus_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Cronus "
                        "provider credential %r from config.yaml (blocked "
                        "by _CRONUS_PROVIDER_ENV_BLOCKLIST). Operator "
