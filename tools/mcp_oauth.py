...
This module provides the glue:
    - ``CronusTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Cronus Agent"
"""
...
from cronus_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__cronus_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses CRONUS_HOME so each profile gets its own OAuth tokens.
    Layout: ``CRONUS_HOME/mcp-tokens/``
    """
    try:
        from cronus_constants import get_cronus_home
        base = Path(get_cronus_home())
    except ImportError:
        base = Path(os.environ.get("CRONUS_HOME", str(Path.home() / ".cronus")))
...
# ---------------------------------------------------------------------------
# CronusTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class CronusTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CRONUS_HOME/mcp-tokens/<server_name>.json         -- tokens
        CRONUS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CRONUS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Cronus records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Cronus.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://cronus-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `cronus mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = CronusTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Cronus Agent")
    scope = cfg.get("scope")
