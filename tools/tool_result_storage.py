...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/cronus-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/cronus-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/cronus-results"
HEREDOC_MARKER = "CRONUS_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/cronus-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"CRONUS_PERSIST_{uuid.uuid4().hex[:8]}"
