...
def _load_cronus_env_vars() -> dict[str, str]:
    """Load ~/.cronus/.env values without failing Docker command execution."""
    try:
        from cronus_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``CRONUS_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("CRONUS_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using CRONUS_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'cronus'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.cronus/sandboxes/). Non-persistent
        ...
        container_name = f"cronus-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Cronus secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _CRONUS_PROVIDER_ENV_BLOCKLIST)
        cronus_env = _load_cronus_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = cronus_env.get(key)
