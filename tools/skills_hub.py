#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Cronus Skills Hub.
...
Used by cronus_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            cronus_meta = metadata.get("cronus", {})
            if isinstance(cronus_meta, dict):
                tags = cronus_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                cronus_meta = meta_block.get("cronus", {})
                if isinstance(cronus_meta, dict):
                    tags = cronus_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Cronus centralized index source
# ---------------------------------------------------------------------------

CRONUS_INDEX_URL = "https://cronus-agent.nousresearch.com/docs/api/skills-index.json"
CRONUS_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "cronus-index.json"
CRONUS_INDEX_TTL = 6 * 3600  # 6 hours


def _load_cronus_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CRONUS_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if CRONUS_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - CRONUS_INDEX_CACHE_FILE.stat().st_mtime
            if age < CRONUS_INDEX_TTL:
                return json.loads(CRONUS_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(CRONUS_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Cronus index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Cronus index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        CRONUS_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        CRONUS_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class CronusIndexSource(SkillSource):
    """Skill source backed by the centralized Cronus Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_cronus_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "cronus-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "cronus-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        CronusIndexSource(auth=auth), # Centralized index (search + resolved install paths)
