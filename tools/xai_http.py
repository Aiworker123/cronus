...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``cronus tools`` repaint, tool-registration scans,
    ...
    2. ``~/.cronus/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_cronus_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.cronus/.env`` first, then ``os.environ``.

    Wraps :func:`cronus_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from cronus_cli.config import get_env_value as _cronus_get_env_value

        value = _cronus_get_env_value(name)
...
def cronus_xai_user_agent() -> str:
    """Return a stable Cronus-specific User-Agent for xAI HTTP calls."""
    try:
        from cronus_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Cronus-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Cronus-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``cronus_cli.config.get_env_value`` so keys
    stored in ``~/.cronus/.env`` (the standard Cronus location) are honored —
