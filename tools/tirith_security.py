...
it is automatically downloaded from GitHub releases to $CRONUS_HOME/bin/tirith.
...
def _get_cronus_home() -> str:
    """Return the Cronus home directory, respecting CRONUS_HOME env var."""
    return str(get_cronus_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_cronus_home(), ".tirith-install-failed")
...
def _cronus_bin_dir() -> str:
    """Return $CRONUS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_cronus_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CRONUS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_cronus_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CRONUS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CRONUS_HOME/bin/tirith
    ...
    cronus_bin = os.path.join(_cronus_bin_dir(), "tirith")
    if os.path.isfile(cronus_bin) and os.access(cronus_bin, os.X_OK):
        _resolved_path = cronus_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return cronus_bin
