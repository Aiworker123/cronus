...
provider** — turn on a backend by enabling a plugin (``cronus plugins
enable video_gen/<name>``) and selecting it in ``cronus tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/cronus-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `cronus tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `cronus plugins list` to see "
            f"installed video gen backends, or `cronus tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `cronus tools` → "
...
# `cronus tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`cronus tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `cronus tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
