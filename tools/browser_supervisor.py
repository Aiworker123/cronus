...
One ``CDPSupervisor`` runs per Cronus ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "cronus-dialog-bridge.invalid"
...
(() => {
  if (window.__cronusDialogBridgeInstalled) return;
  window.__cronusDialogBridgeInstalled = true;
  const ENDPOINT = "http://cronus-dialog-bridge.invalid/";
