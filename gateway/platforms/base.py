...
# Audio file extensions Cronus recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Cronus' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Cronus-created Telegram private-chat
...
from cronus_constants import get_cronus_dir, get_cronus_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.cronus/.env manually."
)
...
# Default location: {CRONUS_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_cronus_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; CronusAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_cronus_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_cronus_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_cronus_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_cronus_dir("cache/screenshots", "browser_screenshots")
_CRONUS_HOME = get_cronus_home()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "CRONUS_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "CRONUS_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "CRONUS_MEDIA_TRUST_RECENT_SECONDS"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CRONUS_HOME / "image_cache",
    _CRONUS_HOME / "audio_cache",
    _CRONUS_HOME / "video_cache",
    _CRONUS_HOME / "document_cache",
    _CRONUS_HOME / "browser_screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The Cronus home itself contains credentials (auth.json, .env) — only the
    # cache subdirectories under it are explicitly allowlisted above.
    denied.append(_CRONUS_HOME / ".env")
    denied.append(_CRONUS_HOME / "auth.json")
    denied.append(_CRONUS_HOME / "credentials")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    existing regular files under Cronus-managed media caches, or roots the
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?cronus\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+cronus[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("CRONUS_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "CRONUS_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "CRONUS_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from cronus_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          CRONUS_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          CRONUS_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          CRONUS_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("CRONUS_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("CRONUS_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("CRONUS_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _cronus_run_generation is set on
