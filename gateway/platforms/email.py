"""
Email platform adapter for the Cronus gateway.

Allows users to interact with Cronus by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "cronus-agent" "version" "{_cronus_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Cronus Agent")
        ...
        msg_id = f"<cronus-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
