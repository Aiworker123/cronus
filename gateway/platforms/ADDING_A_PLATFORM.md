...
There are two ways to add a platform to the Cronus gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.cronus/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Cronus code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `cronus gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `cronus_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Cronus core.
...
And add it to the `cronus-gateway` composite:

```python
"cronus-gateway": {
    "includes": [..., "cronus-your-platform"]
}
```
...
## 12. Status Display (`cronus_cli/status.py`)
...
## 13. Gateway Setup Wizard (`cronus_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ cronus_cli/ toolsets.py \
  --include="*.py" -l | sort -u
