...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_cronus_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /cronus prefix). A single regex
            ...
            # manifest (see `cronus slack manifest`). In Socket Mode, Slack
            ...
            from cronus_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/cronus$")

            @self._app.command(_slash_pattern)
            async def handle_cronus_command(ack, command):
            ...
            for _action_id in (
                "cronus_approve_once",
                "cronus_approve_session",
                "cronus_approve_always",
                "cronus_deny",
            ):
            ...
            for _action_id in (
                "cronus_confirm_once",
                "cronus_confirm_always",
                "cronus_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Cronus handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Cronus session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from cronus_cli.commands import is_gateway_known_command
                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@cronus`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "cronus_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "cronus_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "cronus_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "cronus_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "cronus_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "cronus_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "cronus_confirm_once": "once",
            "cronus_confirm_always": "always",
            "cronus_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "cronus_approve_once": "once",
            "cronus_approve_session": "session",
            "cronus_approve_always": "always",
            "cronus_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/cronus <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/cronus
        ...
        if slash_name in {"cronus", ""}:
            # Legacy /cronus <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from cronus_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/cronus   ``).
        ...
        # "Running /cmd…" ack shown by handle_cronus_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/cronus <question>" must produce public replies so
