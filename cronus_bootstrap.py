"""Windows UTF-8 bootstrap for Cronus entry points.
...
should be imported at the very top of every Cronus entry point
(``cronus``, ``cronus-agent``, ``cronus-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import cronus_bootstrap``
# (or ``from cronus_bootstrap import apply_windows_utf8_bootstrap``) at
