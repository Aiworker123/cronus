...
appropriate.  All installs go to a Cronus-owned bin staging dir,
``<CRONUS_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``cronus lsp
  status``.
...
# ``<CRONUS_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def cronus_lsp_bin_dir() -> Path:
    """Return the Cronus-owned bin staging dir for LSP servers."""
    home = os.environ.get("CRONUS_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".cronus")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = cronus_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = cronus_lsp_bin_dir().parent  # <CRONUS_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = cronus_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a cronus-owned target dir.
    ...
    console script.
    """
    pip_target = cronus_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = cronus_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``cronus lsp status`` CLI to give users a quick
