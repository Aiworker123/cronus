...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Cronus with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.cronus/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Cronus
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cronus tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``cronus setup`` for
