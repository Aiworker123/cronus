...
See the ``cronus-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Cronus, e.g. 'cronus-agent').\n"
    "  • Hub-installed skills (installed via 'cronus skills install').\n"
    "Pinned skills (marked via 'cronus curator pin') CAN be improved — "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("CRONUS_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Cronus' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so Cronus
            ...
            # scratch (fresh _cronus_now() timestamp, fresh
