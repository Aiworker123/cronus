...
Storage
-------
Bundles live in ``~/.cronus/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CRONUS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``cronus bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CRONUS_HOME.

    Honors ``CRONUS_BUNDLES_DIR`` for tests; falls back to
    ``<CRONUS_HOME>/skill-bundles``.
    """
    override = os.environ.get("CRONUS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_cronus_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `cronus bundles` CLI subcommand.
# ---------------------------------------------------------------------------
