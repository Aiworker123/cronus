...
``auth.json``.  They intentionally have no dependency on ``cronus_cli.auth`` so
...
# Sources Cronus owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Cronus-owned OAuth/device-code state)
