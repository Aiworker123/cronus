...
Owns one Codex thread per Cronus session. Drives `turn/start`, consumes
...
# Cronus' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CRONUS_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Cronus/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Cronus session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CRONUS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CRONUS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="cronus",
            client_title="Cronus Agent",
            client_version=_get_cronus_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Cronus' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Cronus' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own cronus-tools callback we
            # auto-accept — the user already approved Cronus' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Cronus approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Cronus returns 'once', 'session', 'always', or 'deny'.
...
def _get_cronus_version() -> str:
    """Best-effort Cronus version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("cronus-agent")
