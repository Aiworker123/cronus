...
upstream prefix cache warm.  See ``cronus-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Cronus never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CRONUS_HOME persona while keeping
    ...
    # Pointer to the cronus-agent skill + docs for user questions about Cronus itself.
    stable_parts.append(CRONUS_AGENT_HELP_GUIDANCE)
    ...
    # CRONUS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Cronus profile the agent is running
    # under so it doesn't conflate ~/.cronus/skills/ (default profile) with
    # ~/.cronus/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Cronus profile: default. Other profiles (if any) live "
            "under ~/.cronus/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Cronus profile: {active_profile}. This session reads "
            f"and writes ~/.cronus/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.cronus/skills/, ~/.cronus/plugins/, "
            f"~/.cronus/cron/, ~/.cronus/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the cronus-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Cronus never rebuilds or reinjects parts of it
