...
as ``kind: backend``) or ``~/.cronus/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``cronus tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CRONUS_HOME/cache/videos/``, creating parents as needed."""
    from cronus_constants import get_cronus_home

    path = get_cronus_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CRONUS_HOME/cache/videos/``.
