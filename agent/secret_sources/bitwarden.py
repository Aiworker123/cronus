...
Cronus pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.cronus/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<cronus_home>/bin/bws`` on
  first use.  Cronus pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.cronus/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``cronus`` invocations don't
  hammer the API.
* Failures NEVER block Cronus startup.  Missing binary, no network,
...
# In-process cache so repeated load_cronus_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `cronus chat -q ...`
...
# <cronus_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.cronus/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under cronus_home/cache/.

    `home_path` is what `load_cronus_dotenv()` already resolved; falling back
    to `$CRONUS_HOME` / `~/.cronus` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("CRONUS_HOME", Path.home() / ".cronus"))
...
def _cronus_bin_dir() -> Path:
    """Where Cronus stores its managed binaries.  Profile-aware."""
    from cronus_constants import get_cronus_home

    return get_cronus_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<cronus_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _cronus_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``cronus secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _cronus_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="cronus-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "cronus-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<cronus_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from cronus_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_cronus_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `cronus secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `cronus secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `cronus secrets bitwarden setup` to install."
        )
        return result
