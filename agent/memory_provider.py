...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - cronus_home (str): The active CRONUS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.cronus``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "cronus").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'cronus memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], cronus_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'cronus memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``cronus_home`` is the active CRONUS_HOME directory path.
