...
from cronus_constants import display_cronus_home as _dhh_fn, PARTIAL_STREAM_STUB_ID
from cronus_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Cronus tool use: "
    ...
    return (
        ...
        f"context, but Cronus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Cronus config, set `model.ollama_num_ctx: 65536` "
...
def run_conversation(
    ...
    # ``cronus logs --session <id>`` can filter a single conversation.
    from cronus_logging import set_session_context
    ...
    # are reused.  The system prompt is Cronus's territory; plugins
    ...
    # all run inside Codex). Default Cronus path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Cronus internals.
        #
        # Cronus invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Cronus tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("CRONUS_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: cronus auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `cronus doctor` for credential-chain diagnostics, or")
                    ...
                    from cronus_constants import display_cronus_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Cronus-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: cronus config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: cronus config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Cronus).  A 429 can
                ...
                # which Cronus' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Cronus' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`cronus",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `cronus auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `cronus model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: cronus auth add nous --type oauth", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: cronus setup", force=True)
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("CRONUS_KANBAN_TASK")
        if _kanban_task:
            try:
