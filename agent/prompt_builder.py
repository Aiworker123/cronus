...
from cronus_constants import get_cronus_home, get_skills_dir, is_wsl
...
_CRONUS_MD_NAMES = (".cronus.md", "CRONUS.md")


def _find_cronus_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.cronus.md`` or ``CRONUS.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Cronus Agent, an intelligent AI assistant created by Nous Research. "
...
CRONUS_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Cronus Agent "
    "itself, load the `cronus-agent` skill with skill_view(name='cronus-agent') "
    "before answering. Docs: https://cronus-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.cronus/kanban.db`. Your task id is in "
    "`$CRONUS_KANBAN_TASK`; your workspace is `$CRONUS_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CRONUS_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `cronus kanban <verb>` for board operations. Use "
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- If the user has explicitly stated something about their environment (e.g. "
    "'I have WSL installed', 'I'm on Ubuntu', 'I already have X'), treat it as "
    "ground truth and act on it directly — do not re-verify what they told you.\n"
    "</prerequisite_checks>\n"
    "\n"
    "<verification>\n"
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Cronus WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Cronus itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Cronus runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Cronus itself is running. The host OS, home, and cwd "
                f"of the Cronus process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Cronus "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_cronus_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.cronus/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("CRONUS_PLATFORM")
        or get_session_env("CRONUS_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Cronus Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `cronus-agent` skill "
            "first. It has the actual commands (e.g. `cronus config set …`, `cronus tools`, "
            "`cronus setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Nous subscription managed tools are disabled. Returns empty string."""
    return ""
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from CRONUS_HOME and return its content, or None.
    ...
    """
    try:
        from cronus_cli.config import ensure_cronus_home
        ensure_cronus_home()
    except Exception as e:
        logger.debug("Could not ensure CRONUS_HOME before loading SOUL.md: %s", e)

    soul_path = get_cronus_home() / "SOUL.md"
...
def _load_cronus_md(cwd_path: Path) -> str:
    """.cronus.md / CRONUS.md — walk to git root."""
    cronus_md_path = _find_cronus_md(cwd_path)
    if not cronus_md_path:
        return ""
    try:
        content = cronus_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = cronus_md_path.name
        try:
            rel = str(cronus_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".cronus.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", cronus_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .cronus.md / CRONUS.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CRONUS_HOME is independent and always included when present.
    ...
    # SOUL.md from CRONUS_HOME only — skip when already loaded as identity
    if not skip_soul:
