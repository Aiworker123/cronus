...
Every Cronus request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Cronus Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=cronus-agent",
        "client=cronus-client-v<__version__>",
    ]

The version is sourced live from ``cronus_cli.__version__`` so it auto-aligns
...
``cronus_cli.__version__`` is the canonical source of truth.
"""
...
def _cronus_version() -> str:
    """Return the current Cronus release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``cronus_cli`` cannot be imported (should
...
def cronus_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=cronus-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=cronus-client-v{_cronus_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=cronus-agent", cronus_client_tag()]
