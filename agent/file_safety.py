...
def _cronus_home_path() -> Path:
    """Resolve the active CRONUS_HOME (profile-aware) without circular imports."""
    try:
        from cronus_constants import get_cronus_home  # local import to avoid cycles
        return get_cronus_home()
    except Exception:
        return Path(os.path.expanduser("~/.cronus"))


def _cronus_root_path() -> Path:
    """Resolve the Cronus root dir (always the parent of any profile, never per-profile)."""
    try:
        from cronus_constants import get_default_cronus_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    cronus_home = _cronus_home_path()
    cronus_root = _cronus_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(cronus_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(cronus_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved CRONUS_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("CRONUS_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Cronus control-plane files: block both the ACTIVE profile's view
    # (cronus_home) AND the global root view. Without the root pass, a
    ...
    cronus_dirs = []
    for base in (_cronus_home_path(), _cronus_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in cronus_dirs:
                cronus_dirs.append(real)
        except Exception:
            continue

    for base_real in cronus_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Cronus path.

    Three categories are blocked:

      * Internal Cronus cache files under ``CRONUS_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CRONUS_HOME and the global Cronus
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.cronus/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CRONUS_HOME (profile-aware) AND the global
    # Cronus root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CRONUS_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    cronus_dirs: list[Path] = []
    for base in (_cronus_home_path(), _cronus_root_path()):
        try:
            real = base.resolve()
            if real not in cronus_dirs:
    ...
    for hd in cronus_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Cronus cache file "
    ...
    for hd in cronus_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Cronus credential store "
    ...
    for hd in cronus_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Cronus MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Cronus MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Cronus profiles are separate CRONUS_HOME dirs under
...
#
# Reference: May 2026 incident where a cronus-security profile session
# edited skills under both ``~/.cronus/profiles/cronus-security/skills/``
# AND ``~/.cronus/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CRONUS_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CRONUS_HOME.
    ...
    """
    try:
        home_real = _cronus_home_path().resolve()
        root_real = _cronus_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Cronus scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Cronus entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Cronus profile {info['target_profile']!r}, but the "
