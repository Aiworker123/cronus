...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Cronus starts and finds ``~/.openclaw/``.

    Points users at ``cronus claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``cronus claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Cronus, run "
        "`cronus claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `cronus claw cleanup` (renames it to ~/.openclaw.pre-migration — "
