...
as ``kind: backend``) or ``~/.cronus/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cronus tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``cronus tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``cronus tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$CRONUS_HOME/cache/images/``, creating parents as needed."""
    from cronus_constants import get_cronus_home

    path = get_cronus_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$CRONUS_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$CRONUS_HOME/cache/images/``.
