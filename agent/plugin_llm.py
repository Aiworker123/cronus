...
Plugins built on Cronus Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Cronus
...
:class:`~cronus_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Cronus' main config
...
handles every provider, fallback chain, and per-task override Cronus
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`cronus_cli.plugins.PluginContext`
