...
def _state_file() -> Path:
    return get_cronus_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.cronus/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``cronus update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``cronus curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `cronus curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Cronus' background skill CURATOR. This is an "
    ...
    "into ~/.cronus/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: cronus-config-*, cronus-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.cronus/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.cronus/logs/curator/``)
    ...
    authored skill data in ``~/.cronus/skills/``.

    ``ensure_cronus_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_cronus_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every cronus install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: cronus curator status
        keep an umbrella stable: cronus curator pin document-tools
    ...
    lines.append("full report: cronus curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: cronus curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.cronus/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.cronus/skills/.archive/` for "
            "safety and can be restored via `cronus curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.cronus/skills/.archive/`. "
            "Restore any via `cronus curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `cronus curator restore <name>`")
    lines.append("- All archives live under `~/.cronus/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `cronus curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `cronus curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``cronus model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `cronus model` → auxiliary
