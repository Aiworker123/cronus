"""Persistent slash-command worker — one CronusCLI per TUI session.
...
def _run(cli: CronusCLI, command: str) -> str:
...
def main():
    ...
    os.environ["CRONUS_SESSION_KEY"] = args.session_key
    os.environ["CRONUS_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = CronusCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
