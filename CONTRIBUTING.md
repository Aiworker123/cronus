# Contributing to Cronus Agent

Thank you for contributing to Cronus Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Cronus to work everywhere.
...
Bundled skills (in `skills/`) ship with every Cronus install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `cronus skills browse` (labeled "official") and install it with `cronus skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `cronus skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.cronus/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(cronus_home, config)` for setup-wizard integration
...
- Integrate with `cronus memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/cronus-agent.git
cd cronus-agent
...
```bash
mkdir -p ~/.cronus/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.cronus/config.yaml
touch ~/.cronus/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.cronus/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/cronus" ~/.local/bin/cronus
...
├── cli.py                    # CronusCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (cronus-cli, cronus-telegram, etc.)
├── cronus_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── cronus_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.cronus/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (cronus-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.cronus/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.cronus/`)

| Path | Purpose |
|------|---------|
| `~/.cronus/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.cronus/.env` | API keys and secrets |
| `~/.cronus/auth.json` | OAuth credentials (Nous Portal) |
| `~/.cronus/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.cronus/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.cronus/state.db` | SQLite session database |
| `~/.cronus/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.cronus/cron/` | Scheduled job data |
| `~/.cronus/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`cronus_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.cronus/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_CRONUS_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.cronus`:
...
The user may skip setup and keep loading the skill. Cronus only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `cronus setup` or update `~/.cronus/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Cronus Agent" is the secondary collaborator. If the contributor's commit shows "Cronus Agent" as author because they used Cronus to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Cronus tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `cronus --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Cronus uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.cronus/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `cronus_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `cronus_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Cronus runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the cronus wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `cronus_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `cronus_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `cronus_cli/gateway_windows.py::
...
Cronus has terminal access. Security matters.
...
2. **Test manually**: Run `cronus` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/cronus-agent/issues)
- Include: OS, Python version, Cronus version (`cronus version`), full error traceback
